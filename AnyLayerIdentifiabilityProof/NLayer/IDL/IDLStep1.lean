...
/-- One step past the finite depth, the concrete preactivation is the real bias.

When the gate index is exactly the finite depth, `paramStream θ'` supplies a zero
attention matrix to `formalPhi`; the scaled formal-phi term vanishes.  This is one step
past the last split-boundary successor gate, not the `j + 2 = L` boundary gate itself. -/
theorem fixedOStar_concretePrimedTierSystem_outOfRange_H_eq_real_log
    {L d r : Nat} {O : Set (ProbePair d)}
    {θ θ' : Params L d}
    (p : FixedOStarProbe r L d O θ θ')
    {j : Nat} (hj : j + 1 = L) :
    p.concretePrimedTierSystem.stratification.H (j + 1) =
      fun _ : ℂ => (Real.log (r : ℝ) : ℂ) := by
  subst L
  funext z
  simp [FixedOStarProbe.concretePrimedTierSystem,
    ...
    constantProbeGateArgument, formalPhi, matC, Matrix.mulVec, dotProduct]

/-- One-past-depth concrete preactivations never hit the sigmoid pole set. -/
theorem fixedOStar_concretePrimedTierSystem_outOfRange_H_not_mem_oddPiI
    {L d r : Nat} {O : Set (ProbePair d)}
    {θ θ' : Params L d}
    (p : FixedOStarProbe r L d O θ θ')
    {j : Nat} (hj : j + 1 = L) (z : ℂ) :
    p.concretePrimedTierSystem.stratification.H (j + 1) z ∉ oddPiI := by
  rw [fixedOStar_concretePrimedTierSystem_outOfRange_H_eq_real_log (p := p) hj]
  exact ofReal_not_mem_oddPiI (Real.log (r : ℝ))
...
/-- Conversely, because the boundary formal-phi lead is identically zero, the existing
last-step lead-avoidance field forces eventual absence of regular boundary pole hits.

This is the precise obstruction left by the current split Claim-B surface. -/
theorem fixedOStarRegularPoleLastStepNoPoleHitOfDepthGTOne_of_leadAvoidance
    {L d r : Nat} {O : Set (ProbePair d)}
    {θ θ' : Params L d}
    {hr : 2 <= r}
    {hStanding : Step1StandingAssumptions r L d O θ θ'}
    ...
    FixedOStarRegularPoleLastStepNoPoleHitDataOfDepthGTOne hr hStanding := by
  intro hDepth p j τ hj hlast hτ hOmega hH
  have havoid := hlastAvoid hDepth p hj hlast hτ hOmega hH
  have hleadZero :
      (p.concretePrimedTierSystem.nestedFamily.step (j + 1)).lead = 0 :=
    fixedOStar_concretePrimedTierSystem_boundary_stepLead_eq_zero
      (p := p) (m := j + 1) (by omega)
  filter_upwards [havoid] with z hz hhit
  have hne := hz hhit
  rw [hleadZero] at hne
  exact hne rfl

/-- On the current formal-phi boundary tower, last-step lead avoidance is exactly the
absence of regular boundary pole hits.

The forward direction uses the identically-zero boundary lead; the reverse direction is
the vacuous packaging used by the existing compatibility surface. -/
theorem fixedOStarRegularPoleLastStepLeadAvoidance_iff_noPoleHit
    {L d r : Nat} {O : Set (ProbePair d)}
    {θ θ' : Params L d}
    {hr : 2 <= r}
    {hStanding : Step1StandingAssumptions r L d O θ θ'} :
    FixedOStarRegularPoleLastStepLeadAvoidanceOnPoleHitDataOfDepthGTOne
        hr hStanding ↔
      FixedOStarRegularPoleLastStepNoPoleHitDataOfDepthGTOne hr hStanding :=
  ⟨fixedOStarRegularPoleLastStepNoPoleHitOfDepthGTOne_of_leadAvoidance,
...
/-- The propagation nested region is exactly the coordinatewise exterior polydomain.

This exposes a key limitation of the current propagation-only tower: because each
peeled propagation presentation has degree zero in the newly peeled variable, its
thresholds do not depend on the propagation polynomials and are all equal to `1`.
Consequently, membership in this region alone cannot encode zero-freeness of the
visible polynomial `g`; that has to come from an additional visible-polynomial region
or a real bridge theorem. -/
theorem step1PropagationNestedTailFamily_mem_region_iff_coordinatewise_large {d : Nat}
    (θ : Nat -> Matrix (Fin d) (Fin d) ℝ × Matrix (Fin d) (Fin d) ℝ)
    (w : Fin d -> ℝ) :
    ∀ {m : Nat} (z : Fin m -> ℂ),
      z ∈ (step1PropagationNestedTailFamily θ w).region m ↔
        ∀ i : Fin m, 1 < ‖z i‖ := by
  intro m
  induction m with
  | zero =>
  ...
  | succ m ih =>
      ...
      · intro hz i
        ...
        · have hlarge :=
            (step1PropagationNestedTailFamily θ w).nestedLast_large_of_mem_region hz
          simpa [nestedLast,
            step1PropagationNestedTailFamily_threshold_eq_one θ w m (nestedInit z)]
            using hlarge
        · have hinit :
              nestedInit z ∈ (step1PropagationNestedTailFamily θ w).region m :=
            (step1PropagationNestedTailFamily θ w).nestedInit_mem_of_mem_region hz
          have hj := (ih (nestedInit z)).mp hinit j
          simpa [nestedInit] using hj
      · intro hcoord
        have hinit :
            nestedInit z ∈ (step1PropagationNestedTailFamily θ w).region m := by
          exact (ih (nestedInit z)).mpr (by
            intro j
            simpa [nestedInit] using hcoord j.castSucc)
        have hlast :
            (step1PropagationNestedTailFamily θ w).threshold m (nestedInit z) <
              ‖nestedLast z‖ := by
          simpa [nestedLast,
            step1PropagationNestedTailFamily_threshold_eq_one θ w m (nestedInit z)]
            using hcoord (Fin.last m)
        exact ⟨hinit, hlast⟩
...
/-- Claim-B pole-hit-plus-avoidance reduced to the two genuine depth-`> 1` split
obligations. -/
theorem fixedOStarRegularPoleHitAvoidanceData_of_depth_gt_one_preimage_and_leadAvoidance
    {L d r : Nat} {O : Set (ProbePair d)}
    {θ θ' : Params L d}
    {hr : 2 <= r}
    {hStanding : Step1StandingAssumptions r L d O θ θ'}
  ...
  intro p j τ hj hτ hOmega hH
  have hpreimageAll :
      FixedOStarRegularPolePreimageData hr hStanding :=
    fixedOStarRegularPolePreimageData_of_depth_gt_one_preimageData hpreimage
  have hleadAll :
    ...
    fixedOStarRegularPoleLeadAvoidanceOnPoleHitData_of_depth_gt_one_leadAvoidanceData
      hlead
  have hpoleFreq := hpreimageAll p hj hτ hH
  have hleadEventual := hleadAll p hj hτ hOmega hH
  exact ((hpoleFreq.and_eventually hOmega).and_eventually hleadEventual).mono
    fun z hz => by
      rcases hz with ⟨⟨hpolez, hregular⟩, hleadz⟩
...
/-- The older level-set Claim-B interface implies the current at-tier interface.

The extra punctured-regularity and blow-up hypotheses required by the older interface
are formal consequences of membership in the current zero-free tier and the compiled
scaled polynomial tower. -/
theorem fixedOStarZeroFreeLevelSetLeadAtTierData_of_levelSetLeadData
    {L d r : Nat} {O : Set (ProbePair d)}
    {θ θ' : Params L d}
    {hr : 2 <= r}
    {hStanding : Step1StandingAssumptions r L d O θ θ'}
  ...
  have H : ZeroFreeScaledPropagationAnalyticData A := by
    simpa [A] using
      fixedOStar_zeroFreeScaledPropagationAnalyticData_of_levelSetLead_frequently
        hr hStanding p (hlevel p)
  have hBridge : ZeroFreeScaledPropagationBridgeData A :=
    H.toZeroFreeScaledPropagationBridgeData
  have hH : BlowsUpAt (A.stratification.H (j + 1)) τ :=
    ZeroFreeScaledPropagationBridgeData.gateArgument_blowsUpAt hBridge hj hτA
  simpa [A] using hlevel p hj hτ (by simpa [A] using hOmega) (by simpa [A] using hH)

/-- The at-tier and older level-set forms of fixed-`O_star` Claim B are equivalent in
the concrete Step 1 setup. -/
theorem fixedOStarZeroFreeLevelSetLeadData_iff_atTierData
    {L d r : Nat} {O : Set (ProbePair d)}
    {θ θ' : Params L d}
    {hr : 2 <= r}
    {hStanding : Step1StandingAssumptions r L d O θ θ'} :
    FixedOStarZeroFreeLevelSetLeadData hr hStanding ↔
      FixedOStarZeroFreeLevelSetLeadAtTierData hr hStanding :=
  ⟨fixedOStarZeroFreeLevelSetLeadAtTierData_of_levelSetLeadData,
    fixedOStarZeroFreeLevelSetLeadData_of_atTierData⟩
...
/-- The combined propagation-visible zero-free region directly supplies the visible
factor nonvanishing field. -/
theorem step1VisiblePolynomialNonzeroOnPropagationVisibleLastRegionAssumptionsForDepth
    {L d : Nat}
    {θ : Nat -> Matrix (Fin d) (Fin d) ℝ × Matrix (Fin d) (Fin d) ℝ}
    {w : Fin d -> ℝ} {iota : Fin d} :
  ...
  intro hDepth z hz
  exact
    eval_step1VisiblePoly_ne_zero_of_propagationVisibleZeroFreeRegion
      (θ := θ) (w := w) (iota := iota) hDepth hz
...
/-- The combined propagation-visible zero-free region directly supplies the propagation
factor nonvanishing needed at positive tail levels. -/
theorem step1PropagationPositiveTailSliceNonzeroOnPropagationVisibleRegionAssumptionsForDepth
    {L d : Nat}
    {θ : Nat -> Matrix (Fin d) (Fin d) ℝ × Matrix (Fin d) (Fin d) ℝ}
    {w : Fin d -> ℝ} {iota : Fin d} :
    Step1PropagationPositiveTailSliceNonzeroOnPropagationVisibleRegionAssumptionsForDepth
      L d θ w iota := by
  intro m _hm_pos _hm z hz
  exact
    eval_step1PropagationTailPoly_ne_zero_of_propagationVisibleZeroFreeRegion_succ
      (K := m) (θ := θ) (w := w) (iota := iota) (z := z) hz
...
/-- Exact missing tower-compatibility bridge for the visible-region Claim-C wrapper.

Concrete last-tier membership supplies membership in the propagation nested family's
last threshold region.  Claim C needs membership in the visible polynomial's zero-free
nested region instead.  This provider records precisely that inclusion at the last
depth, for each fixed `O_star` probe. -/
abbrev FixedOStarLastTierVisibleZeroFreeBridgeData
    {L d r : Nat} {O : Set (ProbePair d)}
    {θ θ' : Params L d}
    (_hr : 2 <= r)
    (_hStanding : Step1StandingAssumptions r L d O θ θ') : Prop :=
  (p : FixedOStarProbe r L d O θ θ') ->
    p.concretePrimedTierSystem.nestedFamily.region (L - 1) ⊆
      (step1VisibleNestedTailData (paramStream θ') p.probe.1 p.iota).zeroFreeRegion
        (L - 1)

/-- Depth-sharpened version of the visible zero-free bridge.

The depth `L <= 1` case of Claim C is already closed by
`fixedOStarLastTierVisiblePolynomialNonzeroData_of_depth_le_one`.  This interface asks
for the tower-compatibility bridge only in the genuinely remaining `1 < L` case. -/
abbrev FixedOStarLastTierVisibleZeroFreeBridgeDataOfDepthGTOne
    {L d r : Nat} {O : Set (ProbePair d)}
    {θ θ' : Params L d}
    (_hr : 2 <= r)
    (_hStanding : Step1StandingAssumptions r L d O θ θ') : Prop :=
...
/-- Pointwise form of the last-tier bridge: propagation-region inclusion at the last
depth is sufficient to put every concrete last-tier prefix in the visible zero-free
region. -/
theorem fixedOStar_lastTierVisibleNestedRegion_of_visibleZeroFreeBridge
    {L d r : Nat} {O : Set (ProbePair d)}
    {θ θ' : Params L d}
    (p : FixedOStarProbe r L d O θ θ')
    (hbridge :
    ...
    ∀ {τ : ℂ}, 0 < L -> τ ∈ p.concretePrimedTierSystem.T (L - 1) ->
      gatePrefix p.concretePrimedTierSystem.stratification (L - 1) τ ∈
        (step1VisibleNestedTailData (paramStream θ') p.probe.1 p.iota).zeroFreeRegion
          (L - 1) := by
  intro τ _hL hτ
  exact hbridge (p.concretePrimedTierSystem.mem_region hτ)

/-- Compile the exact tower-compatibility bridge into the previous visible nested-region
Claim-C interface. -/
theorem fixedOStarLastTierVisibleNestedRegionData_of_visibleZeroFreeBridgeData
    {L d r : Nat} {O : Set (ProbePair d)}
    {θ θ' : Params L d}
    {hr : 2 <= r}
    {hStanding : Step1StandingAssumptions r L d O θ θ'}
    (hbridge : FixedOStarLastTierVisibleZeroFreeBridgeData hr hStanding) :
    FixedOStarLastTierVisibleNestedRegionData hr hStanding := by
  intro p τ hL hτ
  exact fixedOStar_lastTierVisibleNestedRegion_of_visibleZeroFreeBridge
    p (hbridge p) hL hτ

/-- A concrete equality version of the bridge: if, at the last depth, the propagation
nested threshold region agrees with the visible polynomial zero-free region, then the
visible nested-region Claim-C obligation follows. -/
theorem fixedOStarLastTierVisibleNestedRegionData_of_lastRegion_eq_visibleZeroFreeRegion
    {L d r : Nat} {O : Set (ProbePair d)}
    {θ θ' : Params L d}
    {hr : 2 <= r}
    {hStanding : Step1StandingAssumptions r L d O θ θ'}
      ...
      (p : FixedOStarProbe r L d O θ θ') ->
        p.concretePrimedTierSystem.nestedFamily.region (L - 1) =
          (step1VisibleNestedTailData (paramStream θ') p.probe.1 p.iota).zeroFreeRegion
            (L - 1)) :
    FixedOStarLastTierVisibleNestedRegionData hr hStanding := by
  apply fixedOStarLastTierVisibleNestedRegionData_of_visibleZeroFreeBridgeData
  intro p z hz
  simpa [heq p] using hz
...
/-- On the combined propagation-visible zero-free last tier, the visible polynomial is
pointwise nonzero. -/
theorem fixedOStar_visiblePoly_ne_zero_of_mem_propagationVisibleTierSystem_T0
    {L d r : Nat} {O : Set (ProbePair d)}
    {θ θ' : Params L d}
    (p : FixedOStarProbe r L d O θ θ')
    (hDepth : 1 < L) {τ : ℂ}
    (hτ : τ ∈ p.propagationVisibleTierSystem.T0 (L - 1)) :
    MvPolynomial.eval
        (gatePrefix p.concretePrimedTierSystem.stratification (L - 1) τ)
        (step1VisiblePoly (K := L - 1) (paramStream θ') p.probe.1 p.iota) ≠ 0 := by
  have hz :
      gatePrefix p.concretePrimedTierSystem.stratification (L - 1) τ ∈
        PolynomialNestedTailData.zeroFreeRegion
          (step1PropagationVisibleNestedTailData (paramStream θ') p.probe.1 p.iota)
          (L - 1) := by
    have hzA :=
      p.propagationVisibleTierSystem.T0_mem_zeroFreeRegion hτ
    simpa [FixedOStarProbe.propagationVisibleTierSystem,
      PolynomialNestedTailData.zeroFreeRegion] using hzA
  exact
    eval_step1VisiblePoly_ne_zero_of_propagationVisibleZeroFreeRegion
      (θ := paramStream θ') (w := p.probe.1) (iota := p.iota) hDepth hz
...
/-- Decoupled Claim-B data for the combined propagation-visible tier system, reduced to
the single genuine analytic input: the *combined* level-set step that frequently produces
successor-stratum points where the combined next leading coefficient is nonzero.

All other fields are discharged: blow-up of `H'_{j+1}` from the formal-phi system, gate
prefix continuity from the shared stratification, openness of the combined zero-free
regions, and combined-threshold boundedness from continuity. -/
theorem fixedOStar_propagationVisible_zeroFreeDecoupledPropagationData_of_levelSetLead
    {L d r : Nat} {O : Set (ProbePair d)}
    {θ θ' : Params L d}
    (hStanding : Step1StandingAssumptions r L d O θ θ')
    (p : FixedOStarProbe r L d O θ θ')
    (hlevelLead :
      ∀ {j : Nat} {τ : ℂ}, j + 1 < L -> τ ∈ p.propagationVisibleTierSystem.T0 j ->
        (∀ᶠ z in puncturedNhds τ,
          z ∈ p.propagationVisibleTierSystem.stratification.omega (j + 1)) ->
        BlowsUpAt (p.propagationVisibleTierSystem.stratification.H (j + 1)) τ ->
        ∃ᶠ z in puncturedNhds τ,
          z ∈ p.propagationVisibleTierSystem.stratification.S (j + 1)
            ∧ (p.propagationVisibleTierSystem.nestedFamily.step (j + 1)).lead
              (gatePrefix p.propagationVisibleTierSystem.stratification (j + 1) z) ≠ 0) :
    ZeroFreeDecoupledPropagationData p.propagationVisibleTierSystem where
  gateArgument_blowsUpAt := by
    intro j τ hj hτ
    exact fixedOStar_propagationVisible_H_blowsUpAt hStanding p hj hτ
  ...
  zeroFreeRegion_isOpen := by
    ...
    simpa [FixedOStarProbe.propagationVisibleTierSystem,
      PolynomialNestedTailData.zeroFreeRegion] using
      (step1PropagationVisibleNestedTailData (paramStream θ') p.probe.1
        p.iota).isOpen_zeroFreeRegion j
  threshold_puncturedBounded := by
    ...
    have hthreshold_contD :
        ContinuousAt
          ...
          (gatePrefix p.concretePrimedTierSystem.stratification j τ) :=
      ((step1PropagationVisibleNestedTailData (paramStream θ') p.probe.1
        p.iota).continuousOn_thresholds_zeroFreeRegion j).continuousAt
        (((step1PropagationVisibleNestedTailData (paramStream θ') p.probe.1
          p.iota).isOpen_zeroFreeRegion j).mem_nhds hτprefixD)
    have hcontC :
        ContinuousAt
          (fun z : ℂ =>
            (((step1PropagationVisibleNestedTailData (paramStream θ') p.probe.1
  ...
  levelSetLead_frequently := by
    intro j τ hj hτ hOmega hH
    exact hlevelLead hj hτ hOmega hH

/-- Claim B for the combined propagation-visible tier chain, reduced to the single genuine
analytic input (the combined level-set step).  This is the corrected TeX Claim-B target:
it does **not** route through any graph/no-pole boundary surface. -/
theorem fixedOStar_propagationVisible_zeroFreeTierPropagation_of_levelSetLead
    {L d r : Nat} {O : Set (ProbePair d)}
    {θ θ' : Params L d}
    (hStanding : Step1StandingAssumptions r L d O θ θ')
    (p : FixedOStarProbe r L d O θ θ')
      ...
      ∀ {j : Nat} {τ : ℂ}, j + 1 < L -> τ ∈ p.propagationVisibleTierSystem.T0 j ->
        ...
        ∃ᶠ z in puncturedNhds τ,
          z ∈ p.propagationVisibleTierSystem.stratification.S (j + 1)
            ∧ (p.propagationVisibleTierSystem.nestedFamily.step (j + 1)).lead
              (gatePrefix p.propagationVisibleTierSystem.stratification (j + 1) z) ≠ 0) :
    ZeroFreeTierPropagation p.propagationVisibleTierSystem :=
  (fixedOStar_propagationVisible_zeroFreeDecoupledPropagationData_of_levelSetLead
    hStanding p hlevelLead).toZeroFreeTierPropagation

/-- On the combined propagation-visible zero-free last tier, the actual last-layer
visible coefficient is pointwise nonzero. -/
theorem fixedOStar_lastLayer_visibleCoeff_ne_zero_of_mem_propagationVisibleTierSystem_T0
    {L d r : Nat} {O : Set (ProbePair d)}
    {θ θ' : Params L d}
    (p : FixedOStarProbe r L d O θ θ')
    (hDepth : 1 < L) {τ : ℂ}
    (hτ : τ ∈ p.propagationVisibleTierSystem.T0 (L - 1)) :
    p.concretePrimedTierSystem.stratification.lastLayer.coeff (L - 1) τ ≠ 0 := by
  rw [fixedOStar_lastLayer_visibleCoeff_eq_step1VisiblePoly p (by omega) τ]
  exact
    fixedOStar_visiblePoly_ne_zero_of_mem_propagationVisibleTierSystem_T0
      p hDepth hτ
...
/-- Compose the current strongest Claim-B and Claim-C reductions into the Step 1
provider package. -/
theorem fixedOStarStep1ProviderData_of_regularPoleAndVisibleNestedRegion
    {L d r : Nat} {O : Set (ProbePair d)}
    {θ θ' : Params L d}
    {hr : 2 <= r}
    {hStanding : Step1StandingAssumptions r L d O θ θ'}
  ...
  fixedOStarStep1ProviderData_of_visibleNestedRegion hr hStanding
    (fixedOStarZeroFreeLevelSetLeadAtTierData_of_regularPoleLeadAtTierData hpole)
    hregion
...
the existing Claim-C visible nested-region interface. -/
theorem fixedOStarStep1ProviderData_of_poleHitAvoidanceAndVisibleNestedRegion
    {L d r : Nat} {O : Set (ProbePair d)}
    {θ θ' : Params L d}
    {hr : 2 <= r}
    {hStanding : Step1StandingAssumptions r L d O θ θ'}
    ...
    (hregion : FixedOStarLastTierVisibleNestedRegionData hr hStanding) :
    FixedOStarStep1ProviderData hr hStanding :=
  fixedOStarStep1ProviderData_of_regularPoleAndVisibleNestedRegion
...
/-- Direct bridge from the visible zero-free tower inclusion to the pointwise
visible-polynomial Claim-C field. -/
theorem fixedOStarLastTierVisiblePolynomialNonzeroData_of_visibleZeroFreeBridgeData
    {L d r : Nat} {O : Set (ProbePair d)}
    {θ θ' : Params L d}
    (hr : 2 <= r)
    (hStanding : Step1StandingAssumptions r L d O θ θ')
  ...
  fixedOStarLastTierVisiblePolynomialNonzeroData_of_visibleNestedRegion
    hr hStanding
    (fixedOStarLastTierVisibleNestedRegionData_of_visibleZeroFreeBridgeData hbridge)

/-- Depth-gated direct bridge from the visible zero-free tower inclusion to the actual
`visiblePolynomialNonzero_depth_gt_one` provider field. -/
theorem fixedOStarLastTierVisiblePolynomialNonzeroDataOfDepthGTOne_of_visibleZeroFreeBridgeData
    {L d r : Nat} {O : Set (ProbePair d)}
    {θ θ' : Params L d}
    (hr : 2 <= r)
    (hStanding : Step1StandingAssumptions r L d O θ θ')
    (hbridge :
      FixedOStarLastTierVisibleZeroFreeBridgeDataOfDepthGTOne hr hStanding) :
    FixedOStarLastTierVisiblePolynomialNonzeroDataOfDepthGTOne hr hStanding := by
  intro hDepth
  exact fixedOStarLastTierVisiblePolynomialNonzeroData_of_visibleZeroFreeBridgeData
    hr hStanding (hbridge hDepth)

/-- The visible zero-free tower bridge also supplies the exact last-region Claim-C
field used by the reduced frontier.

This keeps the last-region surface as weak as possible while still allowing callers with
the older visible-zero-free inclusion package to construct it directly. -/
theorem fixedOStarVisibleNonzeroOnLastRegionOfDepthGTOne_of_visibleZeroFreeBridgeData
    {L d r : Nat} {O : Set (ProbePair d)}
    {θ θ' : Params L d}
    (hr : 2 <= r)
    (hStanding : Step1StandingAssumptions r L d O θ θ')
  ...
  exact
    eval_step1VisiblePoly_ne_zero_of_visibleZeroFreeRegion
      (K := L - 1) hpoly (hbridge hDepth p hz)

/-- Claim C reduced to the exact remaining depth branch.

For `L <= 1`, the fixed `O_star` visible-coordinate choice already proves the visible
polynomial is nonzero.  For `1 < L`, it suffices to provide the visible zero-free bridge
at the last depth. -/
theorem fixedOStarLastTierVisiblePolynomialNonzeroData_of_depth_gt_one_visibleZeroFreeBridgeData
    {L d r : Nat} {O : Set (ProbePair d)}
    {θ θ' : Params L d}
    (hr : 2 <= r)
    (hStanding : Step1StandingAssumptions r L d O θ θ')
  ...
  · exact fixedOStarLastTierVisiblePolynomialNonzeroData_of_depth_le_one
      hr hStanding hDepth
  · have hDepthGt : 1 < L := by omega
    exact fixedOStarLastTierVisiblePolynomialNonzeroData_of_visibleZeroFreeBridgeData
      hr hStanding (hbridge hDepthGt)

/-- Direct provider constructor from the current bridge-shaped Claim-C obligation. -/
theorem fixedOStarStep1ProviderData_of_poleHitAvoidanceAndVisibleZeroFreeBridge
    {L d r : Nat} {O : Set (ProbePair d)}
    {θ θ' : Params L d}
    {hr : 2 <= r}
    {hStanding : Step1StandingAssumptions r L d O θ θ'}
  ...
  fixedOStarStep1ProviderData_of_poleHitAvoidanceAndVisiblePolynomialNonzero
    hhit
    (fixedOStarLastTierVisiblePolynomialNonzeroData_of_visibleZeroFreeBridgeData
      hr hStanding hbridge)

/-- Provider constructor matching the depth-sharpened Claim-C surface: Claim B remains
the pole-hit-plus-avoidance statement, while Claim C only asks for the visible zero-free
bridge when `1 < L`. -/
theorem fixedOStarStep1ProviderData_of_poleHitAvoidanceAndDepthGTOneVisibleZeroFreeBridge
    {L d r : Nat} {O : Set (ProbePair d)}
    {θ θ' : Params L d}
    {hr : 2 <= r}
    {hStanding : Step1StandingAssumptions r L d O θ θ'}
  ...
  fixedOStarStep1ProviderData_of_poleHitAvoidanceAndVisiblePolynomialNonzero
    hhit
    (fixedOStarLastTierVisiblePolynomialNonzeroData_of_depth_gt_one_visibleZeroFreeBridgeData
      hr hStanding hbridge)
...
/-- In depth at most one, the fixed-`O_star` pointwise Step 1 provider is already
discharged: Claim B is vacuous and Claim C is the depth-one visible-coordinate case. -/
theorem fixedOStarPointwiseStep1ProviderData_of_depth_le_one
    {L d r : Nat} {O : Set (ProbePair d)}
    {θ θ' : Params L d}
    {hr : 2 <= r}
    {hStanding : Step1StandingAssumptions r L d O θ θ'}
  ...
  poleHitAvoidance :=
    fixedOStarRegularPoleHitAvoidanceData_of_regularPolePreimage_and_leadAvoidance
      (fixedOStarRegularPolePreimageData_of_depth_le_one hDepth)
      (fixedOStarRegularPoleLeadAvoidanceOnPoleHitData_of_depth_le_one hDepth)
  visiblePolynomialNonzero :=
...
/-- Depth-gated provider whose Claim C is the pointwise visible-polynomial
nonvanishing field actually consumed by the last-tier coefficient constructor. -/
structure FixedOStarDepthGatedPreimagePointwiseStep1ProviderData
    {L d r : Nat} {O : Set (ProbePair d)}
    {θ θ' : Params L d}
    (hr : 2 <= r)
    (hStanding : Step1StandingAssumptions r L d O θ θ') : Prop where
...
/-- Pointwise Claim-C version of the split-lead depth-gated provider. -/
structure FixedOStarDepthGatedPreimageSplitLeadPointwiseStep1ProviderData
    {L d r : Nat} {O : Set (ProbePair d)}
    {θ θ' : Params L d}
    (hr : 2 <= r)
    (hStanding : Step1StandingAssumptions r L d O θ θ') : Prop where
...
/-- Constructor for the current last-region frontier from older Claim-B level-set data,
the static interior region-nonzero field, the boundary no-pole-hit field, and the
visible zero-free tower bridge. -/
theorem fixedOStarSplitLeadNoPoleLastRegionData_of_levelSet_regionLead_noPole_visibleBridge
    {L d r : Nat} {O : Set (ProbePair d)}
    {θ θ' : Params L d}
    {hr : 2 <= r}
    {hStanding : Step1StandingAssumptions r L d O θ θ'}
  ...
  fixedOStarSplitLeadNoPoleLastRegionData_of_regionLeadNoPoleLastRegionData
    { regularPolePreimage_depth_gt_one :=
      ...
      regularPoleInteriorPropagationLeadingCoeffNonzeroOnRegion_depth_gt_one := hregion
      regularPoleLastStepNoPoleHit_depth_gt_one := hnoPole
      visiblePolynomialNonzeroOnLastRegion_depth_gt_one :=
        fixedOStarVisibleNonzeroOnLastRegionOfDepthGTOne_of_visibleZeroFreeBridgeData
          hr hStanding hvisibleBridge }

/-- Constructor for the current static-region lead frontier from older Claim-B
level-set data, per-fixed-probe nested largeness, boundary no-pole-hit, and the visible
zero-free tower bridge. -/
theorem fixedOStarRegionLeadNoPoleLastRegionData_of_levelSet_nestedLargeness_noPole_visibleBridge
    {L d r : Nat} {O : Set (ProbePair d)}
    {θ θ' : Params L d}
    {hr : 2 <= r}
    {hStanding : Step1StandingAssumptions r L d O θ θ'}
...
/-- Constructor for the current split-lead/no-pole last-region frontier from older
Claim-B level-set data, per-fixed-probe nested largeness, boundary no-pole-hit, and the
visible zero-free tower bridge. -/
theorem fixedOStarSplitLeadNoPoleLastRegionData_of_levelSet_nestedLargeness_noPole_visibleBridge
    {L d r : Nat} {O : Set (ProbePair d)}
    {θ θ' : Params L d}
    {hr : 2 <= r}
    {hStanding : Step1StandingAssumptions r L d O θ θ'}
    ...
    FixedOStarDepthGatedPreimageSplitLeadNoPoleLastRegionStep1ProviderData
      hr hStanding :=
  fixedOStarSplitLeadNoPoleLastRegionData_of_regionLeadNoPoleLastRegionData
    (fixedOStarRegionLeadNoPoleLastRegionData_of_levelSet_nestedLargeness_noPole_visibleBridge
      hlevel hnested hnoPole hvisibleBridge)

/-- Constructor for the static-region lead frontier using the exact last-region
visible-polynomial nonvanishing object instead of the stronger visible zero-free bridge. -/
theorem fixedOStarRegionLeadNoPoleLastRegionData_of_levelSet_nestedLargeness_noPole_visibleNonzeroOnLastRegion
    {L d r : Nat} {O : Set (ProbePair d)}
    {θ θ' : Params L d}
    {hr : 2 <= r}
    {hStanding : Step1StandingAssumptions r L d O θ θ'}
...
/-- Constructor for the split-lead/no-pole last-region frontier using the exact
last-region visible-polynomial nonvanishing object instead of the stronger visible
zero-free bridge. -/
theorem fixedOStarSplitLeadNoPoleLastRegionData_of_levelSet_nestedLargeness_noPole_visibleNonzeroOnLastRegion
    {L d r : Nat} {O : Set (ProbePair d)}
    {θ θ' : Params L d}
    {hr : 2 <= r}
    {hStanding : Step1StandingAssumptions r L d O θ θ'}
    ...
    FixedOStarDepthGatedPreimageSplitLeadNoPoleLastRegionStep1ProviderData
      hr hStanding :=
  fixedOStarSplitLeadNoPoleLastRegionData_of_regionLeadNoPoleLastRegionData
    (fixedOStarRegionLeadNoPoleLastRegionData_of_levelSet_nestedLargeness_noPole_visibleNonzeroOnLastRegion
      hlevel hnested hnoPole hvisibleRegion)

/-- Constructor for the current static-region lead frontier from older Claim-B
level-set data, per-fixed-probe nested largeness, pointwise boundary no-pole data, and
the visible zero-free tower bridge.

This is the pointwise-no-pole variant of
`fixedOStarRegionLeadNoPoleLastRegionData_of_levelSet_nestedLargeness_noPole_visibleBridge`;
the only conversion is the already-proved pointwise-to-eventual no-pole wrapper. -/
theorem fixedOStarRegionLeadNoPoleLastRegionData_of_levelSet_nestedLargeness_pointwiseNoPole_visibleBridge
    {L d r : Nat} {O : Set (ProbePair d)}
    {θ θ' : Params L d}
    {hr : 2 <= r}
    {hStanding : Step1StandingAssumptions r L d O θ θ'}
...
/-- Constructor for the current split-lead/no-pole last-region frontier from older
Claim-B level-set data, per-fixed-probe nested largeness, pointwise boundary no-pole
data, and the visible zero-free tower bridge.

This packages the exact Step 1 frontier with the boundary field stated pointwise rather
than as an eventual no-pole-hit hypothesis. -/
theorem fixedOStarSplitLeadNoPoleLastRegionData_of_levelSet_nestedLargeness_pointwiseNoPole_visibleBridge
    {L d r : Nat} {O : Set (ProbePair d)}
    {θ θ' : Params L d}
    {hr : 2 <= r}
    {hStanding : Step1StandingAssumptions r L d O θ θ'}
    ...
    FixedOStarDepthGatedPreimageSplitLeadNoPoleLastRegionStep1ProviderData
      hr hStanding :=
  fixedOStarSplitLeadNoPoleLastRegionData_of_regionLeadNoPoleLastRegionData
    (fixedOStarRegionLeadNoPoleLastRegionData_of_levelSet_nestedLargeness_pointwiseNoPole_visibleBridge
      hlevel hnested hpoint hvisibleBridge)

/-- Constructor for the static-region lead frontier using the exact last-region
visible-polynomial nonvanishing object instead of the stronger visible zero-free bridge. -/
theorem fixedOStarRegionLeadNoPoleLastRegionData_of_levelSet_nestedLargeness_pointwiseNoPole_visibleNonzeroOnLastRegion
    {L d r : Nat} {O : Set (ProbePair d)}
    {θ θ' : Params L d}
    {hr : 2 <= r}
    {hStanding : Step1StandingAssumptions r L d O θ θ'}
...
last-region visible-polynomial nonvanishing object instead of the stronger visible
zero-free bridge. -/
theorem fixedOStarSplitLeadNoPoleLastRegionData_of_levelSet_nestedLargeness_pointwiseNoPole_visibleNonzeroOnLastRegion
    {L d r : Nat} {O : Set (ProbePair d)}
    {θ θ' : Params L d}
    {hr : 2 <= r}
    {hStanding : Step1StandingAssumptions r L d O θ θ'}
    ...
    FixedOStarDepthGatedPreimageSplitLeadNoPoleLastRegionStep1ProviderData
      hr hStanding :=
  fixedOStarSplitLeadNoPoleLastRegionData_of_regionLeadNoPoleLastRegionData
    (fixedOStarRegionLeadNoPoleLastRegionData_of_levelSet_nestedLargeness_pointwiseNoPole_visibleNonzeroOnLastRegion
      hlevel hnested hpoint hvisibleRegion)
...
/-- Constructor for the split-lead/no-pole last-region frontier using generic
propagation-region visible-polynomial nonvanishing. -/
theorem fixedOStarSplitLeadNoPoleLastRegionData_of_step1PropagationVisibleNonzero
    {L d r : Nat} {O : Set (ProbePair d)}
    {θ θ' : Params L d}
    {hr : 2 <= r}
    {hStanding : Step1StandingAssumptions r L d O θ θ'}
  ...
  fixedOStarSplitLeadNoPoleLastRegionData_of_regionLeadNoPoleLastRegionData
    (fixedOStarRegionLeadNoPoleLastRegionData_of_step1PropagationVisibleNonzero
      hlevel hnested hnoPole hvisible)

/-- Pointwise-no-pole variant of the generic propagation-region Claim-C constructor for
the static-region lead frontier. -/
theorem fixedOStarRegionLeadNoPoleLastRegionData_of_pointwiseVisiblePropagationNonzero
    {L d r : Nat} {O : Set (ProbePair d)}
    {θ θ' : Params L d}
    {hr : 2 <= r}
    {hStanding : Step1StandingAssumptions r L d O θ θ'}
...
/-- Pointwise-no-pole variant of the generic propagation-region Claim-C constructor for
the split-lead/no-pole last-region frontier. -/
theorem fixedOStarSplitLeadNoPoleLastRegionData_of_pointwiseVisiblePropagationNonzero
    {L d r : Nat} {O : Set (ProbePair d)}
    {θ θ' : Params L d}
    {hr : 2 <= r}
    {hStanding : Step1StandingAssumptions r L d O θ θ'}
    ...
    FixedOStarDepthGatedPreimageSplitLeadNoPoleLastRegionStep1ProviderData
      hr hStanding :=
  fixedOStarSplitLeadNoPoleLastRegionData_of_regionLeadNoPoleLastRegionData
    (fixedOStarRegionLeadNoPoleLastRegionData_of_pointwiseVisiblePropagationNonzero
      hlevel hnested hpoint hvisible)
...
/-- Fully reduced constructor for the current split-lead/no-pole last-region frontier:
Claim B uses regular pole hits at tier points, the propagation assumptions are bundled
per probe, and the boundary no-pole input is stated directly at layer `L - 1`. -/
theorem fixedOStarSplitLeadNoPoleLastRegionData_of_regularPole_step1PropagationPrimitive_lastBoundaryNoPole
    {L d r : Nat} {O : Set (ProbePair d)}
    {θ θ' : Params L d}
    {hr : 2 <= r}
    {hStanding : Step1StandingAssumptions r L d O θ θ'}
...
/-- Variant of the current split-lead/no-pole last-region frontier where the Claim-B
regular-pole field is supplied by the narrower depth-gated preimage/lead-avoidance
split. -/
theorem fixedOStarSplitLeadNoPoleLastRegionData_of_regularPolePreimage_leadAvoidance_step1PropagationPrimitive_lastBoundaryNoPole
    {L d r : Nat} {O : Set (ProbePair d)}
    {θ θ' : Params L d}
    {hr : 2 <= r}
    {hStanding : Step1StandingAssumptions r L d O θ θ'}
    ...
    FixedOStarDepthGatedPreimageSplitLeadNoPoleLastRegionStep1ProviderData
      hr hStanding :=
  fixedOStarSplitLeadNoPoleLastRegionData_of_regularPole_step1PropagationPrimitive_lastBoundaryNoPole
    (fixedOStarRegularPoleLeadAtTierData_of_regularPolePreimage_leadAvoidance_depth_gt_one
      hpreimage hlead)
    hprop hboundary

/-- Variant of the current split-lead/no-pole last-region frontier where the per-probe
propagation primitive is supplied by polynomial-family data plus the narrow tail-slice
and visible-region nonvanishing witnesses. -/
theorem fixedOStarSplitLeadNoPoleLastRegionData_of_regularPole_step1PropagationPolynomialWitness_lastBoundaryNoPole
    {L d r : Nat} {O : Set (ProbePair d)}
    {θ θ' : Params L d}
    {hr : 2 <= r}
    {hStanding : Step1StandingAssumptions r L d O θ θ'}
...
/-- Fully split variant of the current frontier: Claim B is supplied by depth-gated
preimage plus lead avoidance, and the propagation primitive is supplied by the
polynomial-family/tail-slice/visible-region witness package. -/
theorem fixedOStarSplitLeadNoPoleLastRegionData_of_regularPolePreimage_leadAvoidance_step1PropagationPolynomialWitness_lastBoundaryNoPole
    {L d r : Nat} {O : Set (ProbePair d)}
    {θ θ' : Params L d}
    {hr : 2 <= r}
    {hStanding : Step1StandingAssumptions r L d O θ θ'}
    ...
    FixedOStarDepthGatedPreimageSplitLeadNoPoleLastRegionStep1ProviderData
      hr hStanding :=
  fixedOStarSplitLeadNoPoleLastRegionData_of_regularPolePreimage_leadAvoidance_step1PropagationPrimitive_lastBoundaryNoPole
    hpreimage hlead
    (fun p => step1PropagationPrimitiveAssumptionsForDepth_of_polynomialWitness
      (hprop p))
    hboundary
...
/-- Compile the split-lead pointwise provider to the pointwise depth-gated provider. -/
theorem fixedOStarDepthGatedPreimagePointwiseStep1ProviderData_of_splitLeadPointwiseData
    {L d r : Nat} {O : Set (ProbePair d)}
    {θ θ' : Params L d}
    {hr : 2 <= r}
    {hStanding : Step1StandingAssumptions r L d O θ θ'}
    (D : FixedOStarDepthGatedPreimageSplitLeadPointwiseStep1ProviderData hr hStanding) :
    FixedOStarDepthGatedPreimagePointwiseStep1ProviderData hr hStanding where
  regularPolePreimage_depth_gt_one :=
    D.regularPolePreimage_depth_gt_one
  regularPoleLeadAvoidance_depth_gt_one :=
    fixedOStarRegularPoleLeadAvoidanceOfDepthGTOne_of_interiorPropagation_and_lastStep
...
/-- Compile the pointwise depth-gated provider to the decomposed pointwise package whose
Claim-B fields are always-on. -/
theorem fixedOStarPreimagePointwiseStep1ProviderData_of_depthGatedPreimagePointwiseData
    {L d r : Nat} {O : Set (ProbePair d)}
    {θ θ' : Params L d}
    {hr : 2 <= r}
    {hStanding : Step1StandingAssumptions r L d O θ θ'}
    (D : FixedOStarDepthGatedPreimagePointwiseStep1ProviderData hr hStanding) :
...
/-- Compile the pointwise depth-gated decomposed Step 1 package to the pointwise
provider surface. -/
theorem fixedOStarPointwiseStep1ProviderData_of_depthGatedPreimagePointwiseData
    {L d r : Nat} {O : Set (ProbePair d)}
    {θ θ' : Params L d}
    {hr : 2 <= r}
    {hStanding : Step1StandingAssumptions r L d O θ θ'}
    ...
    FixedOStarPointwiseStep1ProviderData hr hStanding :=
  fixedOStarPointwiseStep1ProviderData_of_preimagePointwiseData
    (fixedOStarPreimagePointwiseStep1ProviderData_of_depthGatedPreimagePointwiseData D)
...
/-- Compile the decomposed pointwise Step 1 surface directly to the endpoint provider
package. -/
theorem fixedOStarStep1ProviderData_of_preimagePointwiseData
    {L d r : Nat} {O : Set (ProbePair d)}
    {θ θ' : Params L d}
    {hr : 2 <= r}
    {hStanding : Step1StandingAssumptions r L d O θ θ'}
  ...
  fixedOStarStep1ProviderData_of_pointwiseData
    (fixedOStarPointwiseStep1ProviderData_of_preimagePointwiseData D)

/-- Compile the depth-gated decomposed Step 1 package directly to the endpoint provider
package. -/
theorem fixedOStarStep1ProviderData_of_preimageDepthBridgeData
    {L d r : Nat} {O : Set (ProbePair d)}
    {θ θ' : Params L d}
    {hr : 2 <= r}
    {hStanding : Step1StandingAssumptions r L d O θ θ'}
    ...
    FixedOStarStep1ProviderData hr hStanding :=
  fixedOStarStep1ProviderData_of_pointwiseData
    (fixedOStarPointwiseStep1ProviderData_of_preimageDepthBridgeData D)
...
/-- Compile the pointwise depth-gated decomposed Step 1 package directly to the endpoint
provider package. -/
theorem fixedOStarStep1ProviderData_of_depthGatedPreimagePointwiseData
    {L d r : Nat} {O : Set (ProbePair d)}
    {θ θ' : Params L d}
    {hr : 2 <= r}
    {hStanding : Step1StandingAssumptions r L d O θ θ'}
    ...
    FixedOStarStep1ProviderData hr hStanding :=
  fixedOStarStep1ProviderData_of_pointwiseData
    (fixedOStarPointwiseStep1ProviderData_of_depthGatedPreimagePointwiseData D)
...
/-- The standing `O_star` genericity package makes the input projection nonempty. -/
theorem oStarInputProjection_nonempty_of_generic
    {L d : Nat} {θ' : Params L d} {O : Set (ProbePair d)}
    (hgeneric : OStarGenericAssumptions L d θ' O) :
    (OStarInputProjection θ' O).Nonempty := by
  rcases hgeneric.nonempty_O_star with ⟨probe, hp⟩
  exact ⟨probe.1, probe.2, hp⟩

/-- Projection-level polynomial-family data.

The nonzero kappa and visible-coordinate facts needed by the Step 1 polynomial family
are already part of membership in `O_star`; no extra Step 1 analytic witness is needed
for this algebraic package. -/
abbrev Step1OStarProjectionPolynomialFamilyData
      ...
      w ∈ OStarInputProjection θ' O ->
        (iota : Fin d) ->
          visibleTailCoord θ' w iota ≠ 0 ->
            Step1PolynomialFamilyAssumptionsForDepth L d (paramStream θ') w iota

/-- Build the projection-level polynomial-family data directly from `O_star`
membership. -/
theorem step1OStarProjectionPolynomialFamilyData_of_oStarProjection
    {L d : Nat} {θ' : Params L d} {O : Set (ProbePair d)} :
    Step1OStarProjectionPolynomialFamilyData L d θ' O := by
  intro hL w hw iota hvisible
  rcases hw with ⟨v, hp⟩
  exact step1PolynomialFamilyAssumptionsForDepth_of_OStar hL hp hvisible

/-- Algebraic projection data supplied by the existing `O_star` generic package.

This records the part of the Step 1 polynomial-family setup that is already forced by
matrix-level genericity: the projected probe domain is nonempty, and every projected
`O_star` input with a visible coordinate has the concrete Step 1 polynomial-family
certificate.  No Claim-B/Claim-C analytic witness is part of this package. -/
structure Step1OStarProjectionAlgebraicData
    (L d : Nat) (θ' : Params L d) (O : Set (ProbePair d)) : Prop where
  inputProjection_nonempty : (OStarInputProjection θ' O).Nonempty
  polynomialFamily : Step1OStarProjectionPolynomialFamilyData L d θ' O

/-- Compile `OStarGenericAssumptions` to the algebraic projection data used by Step 1.

The proof is only a repackaging of existing `O_star` facts; it deliberately does not
claim any of the remaining positive-region, visible-region, boundary no-pole, or
pole-preimage obligations. -/
theorem step1OStarProjectionAlgebraicData_of_oStarGenericAssumptions
    {L d : Nat} {θ' : Params L d} {O : Set (ProbePair d)}
    (hgeneric : OStarGenericAssumptions L d θ' O) :
    Step1OStarProjectionAlgebraicData L d θ' O where
  inputProjection_nonempty :=
    oStarInputProjection_nonempty_of_generic hgeneric
  polynomialFamily :=
    step1OStarProjectionPolynomialFamilyData_of_oStarProjection

/-- The TeX matrix-level generic clauses supply the algebraic `O_star` projection data
for the Step 1 polynomial family. -/
theorem step1OStarProjectionAlgebraicData_of_texGenericStepClauses
    {L d : Nat} {tailGeneric : Params L d -> Prop}
    {certificate : Params (L + 1) d -> Prop}
    {θ' : Params (L + 1) d} {O : Set (ProbePair d)}
    (hstep : TexGenericStepClauses L d tailGeneric certificate θ')
    (hOpenO : IsOpen O) (hNonemptyO : O.Nonempty) :
    Step1OStarProjectionAlgebraicData (L + 1) d θ' O :=
  step1OStarProjectionAlgebraicData_of_oStarGenericAssumptions
    (hstep.toOStarGenericAssumptions hOpenO hNonemptyO)

/-- Convenience projection of the TeX generic compiler to the polynomial-family
certificate alone. -/
theorem step1OStarProjectionPolynomialFamilyData_of_texGenericStepClauses
    {L d : Nat} {tailGeneric : Params L d -> Prop}
    {certificate : Params (L + 1) d -> Prop}
    {θ' : Params (L + 1) d} {O : Set (ProbePair d)}
    (hstep : TexGenericStepClauses L d tailGeneric certificate θ')
    (hOpenO : IsOpen O) (hNonemptyO : O.Nonempty) :
    Step1OStarProjectionPolynomialFamilyData (L + 1) d θ' O :=
  (step1OStarProjectionAlgebraicData_of_texGenericStepClauses
    hstep hOpenO hNonemptyO).polynomialFamily

/-- The projection-level polynomial-family data gives actual nonzeroness of each
propagation polynomial as a polynomial.  This is the algebraic consequence of the
`O_star` kappa clauses; it is weaker than pointwise zero-freeness on a recursive
region. -/
theorem step1OStarProjectionPropagationTailPoly_ne_zero_of_polynomialFamilyData
    {L d : Nat} {θ' : Params L d} {O : Set (ProbePair d)}
    (hdata : Step1OStarProjectionPolynomialFamilyData L d θ' O)
    (hL : 0 < L)
    {w : Fin d -> ℝ}
    (hw : w ∈ OStarInputProjection θ' O)
    {iota : Fin d}
    (hvisible : visibleTailCoord θ' w iota ≠ 0)
    {m : Nat} (hm : m < L - 1) :
    step1PropagationTailPoly (paramStream θ') w m ≠ 0 :=
  (hasDominantTopCoeff_step1PropagationTailPoly_of_depth
    (hdata hL w hw iota hvisible) hm).ne_zero

/-- The same algebraic projection data gives nonzeroness of the extracted propagation
leading coefficient as a polynomial.  Pointwise nonvanishing on the nested region is the
separate positive-tail slice obligation below. -/
theorem step1OStarProjectionPropagationLeadingCoeffPoly_ne_zero_of_polynomialFamilyData
    {L d : Nat} {θ' : Params L d} {O : Set (ProbePair d)}
    (hdata : Step1OStarProjectionPolynomialFamilyData L d θ' O)
    (hL : 0 < L)
    {w : Fin d -> ℝ}
    ...
    (hvisible : visibleTailCoord θ' w iota ≠ 0)
    {m : Nat} (hm : m < L - 1) :
    step1PropagationLeadingCoeffPoly (paramStream θ') w m ≠ 0 :=
  step1PropagationLeadingCoeffPoly_ne_zero_of_depth
    (hdata hL w hw iota hvisible) hm

/-- The projection-level polynomial-family data also gives nonzeroness of the visible
last polynomial as a polynomial.  This records exactly the consequence of the visible
coordinate clause before any last-region zero-free assertion is added. -/
theorem step1OStarProjectionVisiblePoly_ne_zero_of_polynomialFamilyData
    {L d : Nat} {θ' : Params L d} {O : Set (ProbePair d)}
    (hdata : Step1OStarProjectionPolynomialFamilyData L d θ' O)
    (hL : 0 < L)
    {w : Fin d -> ℝ}
  ...
  (hdata hL w hw iota hvisible).hasDominantTopCoeff_visible.ne_zero

/-- TeX generic step clauses supply polynomial nonzeroness of each projected
propagation polynomial. -/
theorem step1OStarProjectionPropagationTailPoly_ne_zero_of_texGenericStepClauses
    {L d : Nat} {tailGeneric : Params L d -> Prop}
    {certificate : Params (L + 1) d -> Prop}
    {θ' : Params (L + 1) d} {O : Set (ProbePair d)}
    (hstep : TexGenericStepClauses L d tailGeneric certificate θ')
    ...
    {m : Nat} (hm : m < (L + 1) - 1) :
    step1PropagationTailPoly (paramStream θ') w m ≠ 0 :=
  step1OStarProjectionPropagationTailPoly_ne_zero_of_polynomialFamilyData
    (step1OStarProjectionPolynomialFamilyData_of_texGenericStepClauses
      hstep hOpenO hNonemptyO)
    (Nat.succ_pos L) hw hvisible hm

/-- TeX generic step clauses supply polynomial nonzeroness of each extracted projected
propagation leading coefficient. -/
theorem step1OStarProjectionPropagationLeadingCoeffPoly_ne_zero_of_texGenericStepClauses
    {L d : Nat} {tailGeneric : Params L d -> Prop}
    {certificate : Params (L + 1) d -> Prop}
    {θ' : Params (L + 1) d} {O : Set (ProbePair d)}
    (hstep : TexGenericStepClauses L d tailGeneric certificate θ')
    ...
    {m : Nat} (hm : m < (L + 1) - 1) :
    step1PropagationLeadingCoeffPoly (paramStream θ') w m ≠ 0 :=
  step1OStarProjectionPropagationLeadingCoeffPoly_ne_zero_of_polynomialFamilyData
    (step1OStarProjectionPolynomialFamilyData_of_texGenericStepClauses
      hstep hOpenO hNonemptyO)
    (Nat.succ_pos L) hw hvisible hm

/-- TeX generic step clauses supply polynomial nonzeroness of the projected visible
last polynomial. -/
theorem step1OStarProjectionVisiblePoly_ne_zero_of_texGenericStepClauses
    {L d : Nat} {tailGeneric : Params L d -> Prop}
    {certificate : Params (L + 1) d -> Prop}
    {θ' : Params (L + 1) d} {O : Set (ProbePair d)}
    (hstep : TexGenericStepClauses L d tailGeneric certificate θ')
    ...
    {iota : Fin d}
    (hvisible : visibleTailCoord θ' w iota ≠ 0) :
    step1VisiblePoly (K := (L + 1) - 1) (paramStream θ') w iota ≠ 0 :=
  step1OStarProjectionVisiblePoly_ne_zero_of_polynomialFamilyData
    (step1OStarProjectionPolynomialFamilyData_of_texGenericStepClauses
      hstep hOpenO hNonemptyO)
    (Nat.succ_pos L) hw hvisible
...
/-- The combined propagation-visible zero-free regions supply the projection-level
positive-tail propagation-factor witness. -/
theorem step1PropagationPositiveTailSliceNonzeroOnOStarProjectionPropagationVisibleDataOfDepthGTTwo
    {L d : Nat} {θ' : Params L d} {O : Set (ProbePair d)} :
    Step1PropagationPositiveTailSliceNonzeroOnOStarProjectionPropagationVisibleDataOfDepthGTTwo
      L d θ' O := by
  intro _hDepth w _hw iota _hvisible
  exact
    step1PropagationPositiveTailSliceNonzeroOnPropagationVisibleRegionAssumptionsForDepth
      (L := L) (d := d) (θ := paramStream θ') (w := w) (iota := iota)
...
/-- The combined propagation-visible zero-free last region supplies the projection-level
visible-polynomial witness. -/
theorem step1VisiblePolynomialNonzeroOnOStarProjectionPropagationVisibleLastRegionData
    {L d : Nat} {θ' : Params L d} {O : Set (ProbePair d)} :
    Step1VisiblePolynomialNonzeroOnOStarProjectionPropagationVisibleLastRegionData
      L d θ' O := by
  intro w _hw iota _hvisible
  exact
    step1VisiblePolynomialNonzeroOnPropagationVisibleLastRegionAssumptionsForDepth
      (L := L) (d := d) (θ := paramStream θ') (w := w) (iota := iota)
...
/-- Raw probe/coordinate pole-preimage witness.

This is the remaining analytic pole-hit statement stripped of the fixed-probe
tail-agreement package: for every raw `O_star` probe and visible coordinate, blow-up of
the concrete successor preactivation at a zero-free tier point has frequent preimages of
the sigmoid pole set. -/
abbrev Step1OStarProbeCoordRegularPolePreimageDataOfDepthGTOne
    (L d r : Nat) (θ' : Params L d) (O : Set (ProbePair d)) : Prop :=
  1 < L ->
    (probe : ProbePair d) ->
      probe ∈ O_star θ' O ->
        (iota : Fin d) ->
          visibleTailCoord θ' probe.1 iota ≠ 0 ->
            ...
            ∀ {j : Nat} {τ : ℂ}, j + 1 < L -> τ ∈ A.T0 j ->
              BlowsUpAt (A.stratification.H (j + 1)) τ ->
              ∃ᶠ z in puncturedNhds τ,
                A.stratification.H (j + 1) z ∈ oddPiI

/-- Reduced `O_star` pole-preimage surface, stated over raw probes and visible
coordinates instead of `FixedOStarProbe` packages. -/
structure Step1OStarProbeCoordRegularPolePreimageData
    (L d r : Nat) (θ' : Params L d) (O : Set (ProbePair d)) : Prop where
  regularPolePreimageOnProbeCoord_depth_gt_one :
    Step1OStarProbeCoordRegularPolePreimageDataOfDepthGTOne L d r θ' O

/-- Pointwise raw probe/coordinate regular-pole preimage theorem from an explicit
analytic reciprocal normal form.

This is the concrete bridge to
`frequently_mem_oddPiI_of_analyticAt_reciprocal`: once the successor preactivation
`A.stratification.H (j+1)` has been put in a local reciprocal normal form `R` at the
tier point, frequent hits of the sigmoid pole set follow without using blow-up alone. -/
theorem fixedOStarProbeCoord_regularPolePreimage_of_analyticAt_reciprocal
    {L d r : Nat} {θ' : Params L d} {O : Set (ProbePair d)}
    {probe : ProbePair d} (_hp : probe ∈ O_star θ' O)
    {iota : Fin d} (_hvisible : visibleTailCoord θ' probe.1 iota ≠ 0)
    {j : Nat} {τ : ℂ} (_hj : j + 1 < L)
    (_hτ :
      τ ∈ (fixedOStarProbeCoordTierSystem (r := r) (θ' := θ') probe iota).T0 j)
    (R : ℂ -> ℂ)
    (hR : AnalyticAt ℂ R τ)
    (hRτ : R τ = 0)
    ...
    ∃ᶠ z in puncturedNhds τ,
      ((fixedOStarProbeCoordTierSystem (r := r) (θ' := θ') probe iota).stratification.H
        (j + 1) z) ∈ oddPiI :=
  frequently_mem_oddPiI_of_analyticAt_reciprocal hR hRτ hRnonconst hrecip

/-- Pointwise raw probe/coordinate regular-pole preimage theorem from a local analytic
reciprocal normal form on the punctured neighborhood. -/
theorem fixedOStarProbeCoord_regularPolePreimage_of_analyticAt_eventually_reciprocal
    {L d r : Nat} {θ' : Params L d} {O : Set (ProbePair d)}
    {probe : ProbePair d} (_hp : probe ∈ O_star θ' O)
    {iota : Fin d} (_hvisible : visibleTailCoord θ' probe.1 iota ≠ 0)
    {j : Nat} {τ : ℂ} (_hj : j + 1 < L)
...
/-- Raw probe/coordinate preimage data from explicit reciprocal normal forms at every
non-vacuous zero-free tier point.

The hypothesis is not a new provider field: it is the concrete normal-form obligation
needed at the exact point where the existing preimage target asks for pole hits. -/
theorem step1OStarProbeCoordRegularPolePreimageData_of_analyticAt_reciprocalNormalForms
    {L d r : Nat} {θ' : Params L d} {O : Set (ProbePair d)}
    (hnormal :
      ∀ (_hDepth : 1 < L) (probe : ProbePair d), probe ∈ O_star θ' O ->
        ∀ (iota : Fin d), visibleTailCoord θ' probe.1 iota ≠ 0 ->
          let A := fixedOStarProbeCoordTierSystem (r := r) (θ' := θ') probe iota
          ∀ {j : Nat} {τ : ℂ}, j + 1 < L -> τ ∈ A.T0 j ->
            BlowsUpAt (A.stratification.H (j + 1)) τ ->
            ∃ R : ℂ -> ℂ,
              AnalyticAt ℂ R τ
                ∧ R τ = 0
                ∧ (¬ ∀ᶠ z in nhds τ, R z = R τ)
                ∧ ∀ z, z ≠ τ -> R z * A.stratification.H (j + 1) z = 1) :
    Step1OStarProbeCoordRegularPolePreimageData L d r θ' O where
  regularPolePreimageOnProbeCoord_depth_gt_one := by
    intro hDepth probe hp iota hvisible
    let A := fixedOStarProbeCoordTierSystem (r := r) (θ' := θ') probe iota
    change ∀ {j : Nat} {τ : ℂ}, j + 1 < L -> τ ∈ A.T0 j ->
      BlowsUpAt (A.stratification.H (j + 1)) τ ->
      ∃ᶠ z in puncturedNhds τ, A.stratification.H (j + 1) z ∈ oddPiI
    intro j τ hj hτ hH
    rcases hnormal hDepth probe hp iota hvisible (j := j) (τ := τ) hj hτ hH with
      ⟨R, hR, hRτ, hRnonconst, hrecip⟩
    exact
      fixedOStarProbeCoord_regularPolePreimage_of_analyticAt_reciprocal
        (r := r) (θ' := θ') (O := O) hp hvisible hj hτ R hR hRτ hRnonconst hrecip

/-- Raw probe/coordinate preimage data from explicit local reciprocal normal forms at
every non-vacuous zero-free tier point. -/
theorem step1OStarProbeCoordRegularPolePreimageData_of_analyticAt_eventually_reciprocalNormalForms
    {L d r : Nat} {θ' : Params L d} {O : Set (ProbePair d)}
    (hnormal :
      ∀ (_hDepth : 1 < L) (probe : ProbePair d), probe ∈ O_star θ' O ->
        ∀ (iota : Fin d), visibleTailCoord θ' probe.1 iota ≠ 0 ->
          ...
          ∀ {j : Nat} {τ : ℂ}, j + 1 < L -> τ ∈ A.T0 j ->
            ...
            ∃ R : ℂ -> ℂ,
              AnalyticAt ℂ R τ
                ...
                ∧ (¬ ∀ᶠ z in nhds τ, R z = R τ)
                ∧ ∀ᶠ z in puncturedNhds τ,
                  R z * A.stratification.H (j + 1) z = 1) :
    Step1OStarProbeCoordRegularPolePreimageData L d r θ' O where
  regularPolePreimageOnProbeCoord_depth_gt_one := by
    intro hDepth probe hp iota hvisible
    let A := fixedOStarProbeCoordTierSystem (r := r) (θ' := θ') probe iota
    ...
    rcases hnormal hDepth probe hp iota hvisible (j := j) (τ := τ) hj hτ hH with
      ⟨R, hR, hRτ, hRnonconst, hrecip⟩
    exact
      fixedOStarProbeCoord_regularPolePreimage_of_analyticAt_eventually_reciprocal
        (r := r) (θ' := θ') (O := O) hp hvisible hj hτ R hR hRτ hRnonconst
        hrecip

/-- Raw probe/coordinate regular-pole preimage package from the concrete scaled
quadratic normal form.

No boundary graph or no-pole hypothesis is needed: the `O_star` and visible-coordinate
arguments already present in the raw callback supply the polynomial-family facts needed
for the quadratic step. -/
theorem step1OStarProbeCoordRegularPolePreimageData_of_OStar_genericity
    {L d r : Nat} {θ' : Params L d} {O : Set (ProbePair d)} :
    Step1OStarProbeCoordRegularPolePreimageData L d r θ' O :=
  step1OStarProbeCoordRegularPolePreimageData_of_analyticAt_eventually_reciprocalNormalForms
    (r := r) (θ' := θ') (O := O) (by
      intro _hDepth probe hp iota hvisible
      let A := fixedOStarProbeCoordTierSystem (r := r) (θ' := θ') probe iota
      change ∀ {j : Nat} {τ : ℂ}, j + 1 < L -> τ ∈ A.T0 j ->
        BlowsUpAt (A.stratification.H (j + 1)) τ ->
        ∃ R : ℂ -> ℂ,
      ...
      intro j τ hj hτ _hH
      have hH_scaled :
          A.stratification.H (j + 1)
            =ᶠ[puncturedNhds τ] scaledTierPhiPath A j := by
        simpa [A] using
          fixedOStarProbeCoordTierSystem_H_eventuallyEq_scaledTierPhiPath
            (r := r) (θ' := θ') probe iota j τ
      have hdegree : (A.nestedFamily.step j).degree = 2 := by
        simpa [A] using
          fixedOStarProbeCoordTierSystem_step_degree_eq_two_of_OStar
            (r := r) (θ' := θ') (O := O) hp hvisible hj
      have hτS : τ ∈ A.stratification.S j := A.T0_mem_stratum hτ
      have hτω : τ ∈ A.stratification.omega j :=
        A.stratification.mem_omega_of_mem_stratum (by omega) hτS
      have hτ0 : τ ≠ 0 := A.T0_ne_zero (by omega) hτ
      have hlead0 :
          (A.nestedFamily.step j).lead (gatePrefix A.stratification j τ) ≠ 0 :=
        A.T0_lead_ne hτ
      have hcoeff :=
        fixedOStarProbeCoordTierSystem_coeffPaths_analyticAt
          (r := r) (θ' := θ') probe iota (by simpa [A] using hτω)
      exact
        fixedOStarProbeCoord_H_localReciprocalNormalForm_of_scaled_quadratic
          (r := r) (θ' := θ') probe iota hj
          (by simpa [A] using hτS) hτ0 (by simpa [A] using hlead0)
          (by simpa [A] using hH_scaled)
          (by simpa [A] using hdegree)
          (by simpa [A] using hcoeff.1)
          (by simpa [A] using hcoeff.2))

/-- The raw probe/coordinate pole-preimage package is vacuous in depth at most one. -/
theorem step1OStarProbeCoordRegularPolePreimageData_of_depth_le_one
    {L d r : Nat} {θ' : Params L d} {O : Set (ProbePair d)}
    (hDepth : L <= 1) :
    Step1OStarProbeCoordRegularPolePreimageData L d r θ' O where
  regularPolePreimageOnProbeCoord_depth_gt_one := by
...
/-- No-pole-on-region witnesses are contravariant in the region. -/
theorem step1LastBoundaryFormalPhiNoPoleOnRegionAssumptionsForDepth_mono
    {L d r : Nat}
    {θ : Nat -> Matrix (Fin d) (Fin d) ℝ × Matrix (Fin d) (Fin d) ℝ}
    {w v : Fin d -> ℝ}
    {R S : Set (ℂ × (Fin (L - 1) -> ℂ))}
    ...
    Step1LastBoundaryFormalPhiNoPoleOnRegionAssumptionsForDepth
      L d r θ w v R := by
  intro hDepth q hq
  exact hS hDepth q (hRS hq)

/-- The active raw graph boundary witness is vacuous through depth one. -/
theorem step1LastBoundaryFormalPhiNoPoleOnOStarProbeCoordGraphData_of_depth_le_one
    {L d r : Nat} {O : Set (ProbePair d)} {θ' : Params L d}
    (hDepth : L <= 1) :
    Step1LastBoundaryFormalPhiNoPoleOnOStarProbeCoordGraphData
      (L := L) (d := d) (O := O) (r := r) θ' := by
  intro _probe _hp _iota _hvisible hDepth_gt
  have hfalse : False := by omega
  exact False.elim hfalse

/-- A no-pole witness on any region containing the raw probe/coordinate boundary graph
supplies the active raw graph boundary witness. -/
theorem step1LastBoundaryFormalPhiNoPoleOnOStarProbeCoordGraphData_of_regionSuperset
    {L d r : Nat} {O : Set (ProbePair d)} {θ' : Params L d}
    (R : ProbePair d -> Fin d -> Set (ℂ × (Fin (L - 1) -> ℂ)))
    (hGraph_subset :
      ∀ probe, probe ∈ O_star θ' O ->
        ∀ iota, visibleTailCoord θ' probe.1 iota ≠ 0 ->
          fixedOStarLastBoundaryRegularDomainGraphOfProbeCoord
              (r := r) (θ' := θ') probe iota ⊆ R probe iota)
    (hboundary :
      ∀ probe, probe ∈ O_star θ' O ->
        ∀ iota, visibleTailCoord θ' probe.1 iota ≠ 0 ->
          Step1LastBoundaryFormalPhiNoPoleOnRegionAssumptionsForDepth
            L d r (paramStream θ') probe.1 probe.2 (R probe iota)) :
    Step1LastBoundaryFormalPhiNoPoleOnOStarProbeCoordGraphData
      (L := L) (d := d) (O := O) (r := r) θ' := by
  intro probe hp iota hvisible
  exact
    step1LastBoundaryFormalPhiNoPoleOnRegionAssumptionsForDepth_mono
      (hGraph_subset probe hp iota hvisible)
      (hboundary probe hp iota hvisible)

/-- A raw ambient no-pole witness on the full product region supplies the active raw
probe/coordinate boundary graph witness. -/
theorem step1LastBoundaryFormalPhiNoPoleOnOStarProbeCoordGraphData_of_univ
    {L d r : Nat} {O : Set (ProbePair d)} {θ' : Params L d}
    (hboundary :
      ∀ probe, probe ∈ O_star θ' O ->
        ∀ iota, visibleTailCoord θ' probe.1 iota ≠ 0 ->
          Step1LastBoundaryFormalPhiNoPoleOnRegionAssumptionsForDepth
            L d r (paramStream θ') probe.1 probe.2
            (Set.univ : Set (ℂ × (Fin (L - 1) -> ℂ)))) :
    Step1LastBoundaryFormalPhiNoPoleOnOStarProbeCoordGraphData
      (L := L) (d := d) (O := O) (r := r) θ' := by
  intro probe hp iota hvisible hDepth q _hq
  exact hboundary probe hp iota hvisible hDepth q trivial
...
/-- The projected visible-polynomial obligation is empty through depth one. -/
theorem step1ProjectionVisibleLastRegionPolynomialNonzeroObligations_of_depth_le_one
    {L d : Nat} {θ' : Params L d} {O : Set (ProbePair d)}
    (hDepth : L <= 1) :
    Step1ProjectionVisibleLastRegionPolynomialNonzeroObligations L d θ' O := by
  intro hDepth_gt
  have hfalse : False := by omega
  exact False.elim hfalse

/-- The active visible-last-region obligation is exactly nonvanishing on the exterior
polydomain `∀ i, 1 < ‖z i‖`.

This is a diagnostic proof, not a replacement target: it shows that the current
propagation-only region does not remember the visible polynomial `g`.  Any proof of the
active obligation must therefore prove nonvanishing on this whole exterior polydomain,
or the target must be changed to use a region whose thresholds include `g`. -/
theorem step1ProjectionVisibleLastRegionObligations_iff_exteriorPolydomain
    {L d : Nat} {θ' : Params L d} {O : Set (ProbePair d)} :
            ...
            (iota : Fin d) ->
              visibleTailCoord θ' w iota ≠ 0 ->
                ∀ z : Fin (L - 1) -> ℂ,
                  (∀ j : Fin (L - 1), 1 < ‖z j‖) ->
                    MvPolynomial.eval z
                      (step1VisiblePoly (K := L - 1) (paramStream θ') w iota) ≠
                        0 := by
  constructor
  · intro h hDepth w hw iota hvisible z hz
    exact h hDepth w hw iota hvisible z
      ((step1PropagationNestedTailFamily_mem_region_iff_coordinatewise_large
        (paramStream θ') w z).mpr hz)
  · intro h hDepth w hw iota hvisible z hz
    exact h hDepth w hw iota hvisible z
      ((step1PropagationNestedTailFamily_mem_region_iff_coordinatewise_large
        (paramStream θ') w z).mp hz)
...
/-- Concrete raw graph no-pole obligation for the last boundary.

For every raw `O_star` probe and visible coordinate, the scalar preactivation assembled
from the last in-range formal-phi polynomial must avoid the sigmoid pole set on the
probe/coordinate regular-domain graph. -/
abbrev Step1OStarProbeCoordGraphLastBoundaryNoPoleObligations
    {L d r : Nat} {O : Set (ProbePair d)}
    (θ' : Params L d) : Prop :=
  1 < L ->
    (probe : ProbePair d) ->
      probe ∈ O_star θ' O ->
        (iota : Fin d) ->
          visibleTailCoord θ' probe.1 iota ≠ 0 ->
            ∀ q ∈ fixedOStarLastBoundaryRegularDomainGraphOfProbeCoord
                (r := r) (θ' := θ') probe iota,
              q.1 *
                  MvPolynomial.eval q.2
                    (formalPhiPoly (K := L - 1) (paramStream θ') (L - 1)
...
/-- The raw graph no-pole obligation is empty through depth one. -/
theorem step1OStarProbeCoordGraphLastBoundaryNoPoleObligations_of_depth_le_one
    {L d r : Nat} {O : Set (ProbePair d)} {θ' : Params L d}
    (hDepth : L <= 1) :
    Step1OStarProbeCoordGraphLastBoundaryNoPoleObligations
      (L := L) (d := d) (O := O) (r := r) θ' := by
  intro hDepth_gt
  have hfalse : False := by omega
  exact False.elim hfalse

/-- Compile the concrete raw graph no-pole obligation to the active graph witness. -/
theorem step1LastBoundaryFormalPhiNoPoleOnOStarProbeCoordGraphData_of_concrete
    {L d r : Nat} {O : Set (ProbePair d)} {θ' : Params L d}
    (h :
      Step1OStarProbeCoordGraphLastBoundaryNoPoleObligations
        (L := L) (d := d) (O := O) (r := r) θ') :
    Step1LastBoundaryFormalPhiNoPoleOnOStarProbeCoordGraphData
      (L := L) (d := d) (O := O) (r := r) θ' := by
  intro probe hp iota hvisible hDepth q hq
  exact h hDepth probe hp iota hvisible q hq

/-- The active graph witness supplies the older raw graph no-pole obligation.

The two formulations have the same pointwise content; this direction only changes the
order in which the depth and raw probe/coordinate arguments are supplied. -/
theorem step1OStarProbeCoordGraphLastBoundaryNoPoleObligations_of_graphData
    {L d r : Nat} {O : Set (ProbePair d)} {θ' : Params L d}
    (h :
      Step1LastBoundaryFormalPhiNoPoleOnOStarProbeCoordGraphData
        (L := L) (d := d) (O := O) (r := r) θ') :
    Step1OStarProbeCoordGraphLastBoundaryNoPoleObligations
      (L := L) (d := d) (O := O) (r := r) θ' := by
  intro hDepth probe hp iota hvisible q hq
  exact h probe hp iota hvisible hDepth q hq
...
/-- The older combined concrete frontier supplies the split pointwise
region-nonvanishing package. -/
theorem step1OStarProjectionConcretePointwiseRegionNonzeroObligations_of_concrete
    {L d r : Nat} {θ' : Params L d} {O : Set (ProbePair d)}
  ...
  propagationPositiveTailSlices_depth_gt_two := fun _hDepth =>
    D.propagationPositiveTailSlices
  visibleLastRegion := D.visibleLastRegion

/-- The older combined concrete frontier supplies the depth-sharpened split frontier. -/
theorem step1OStarProjectionGraphConcreteByDepthObligations_of_concrete
    {L d r : Nat} {θ' : Params L d} {O : Set (ProbePair d)}
    (D : Step1OStarProjectionGraphConcretePolynomialObligations L d r θ' O) :
    Step1OStarProjectionGraphConcreteByDepthObligations L d r θ' O where
  pointwiseRegionNonzero :=
    step1OStarProjectionConcretePointwiseRegionNonzeroObligations_of_concrete D
  lastBoundaryNoPole := D.lastBoundaryNoPole

/-- In depth at most one, the concrete projection/graph polynomial frontier has no
remaining obligations. -/
theorem step1OStarProjectionGraphConcretePolynomialObligations_of_depth_le_one
    {L d r : Nat} {θ' : Params L d} {O : Set (ProbePair d)}
    (hDepth : L <= 1) :
  ...
  propagationPositiveTailSlices :=
    step1ProjectionPositiveTailSlicePolynomialNonzeroObligations_of_depth_le_two
      (by omega)
  visibleLastRegion :=
    step1ProjectionVisibleLastRegionPolynomialNonzeroObligations_of_depth_le_one
      hDepth
  lastBoundaryNoPole :=
    step1LastBoundaryFormalPhiNoPoleOnOStarProbeCoordGraphData_of_concrete
      (step1OStarProbeCoordGraphLastBoundaryNoPoleObligations_of_depth_le_one
        hDepth)
...
/-- In depth at most one, the depth-sharpened split frontier has no remaining
obligations. -/
theorem step1OStarProjectionGraphConcreteByDepthObligations_of_depth_le_one
    {L d r : Nat} {θ' : Params L d} {O : Set (ProbePair d)}
    (hDepth : L <= 1) :
    Step1OStarProjectionGraphConcreteByDepthObligations L d r θ' O :=
  step1OStarProjectionGraphConcreteByDepthObligations_of_concrete
    (step1OStarProjectionGraphConcretePolynomialObligations_of_depth_le_one
      (L := L) (d := d) (r := r) (θ' := θ') (O := O) hDepth)
...
/-- Compile the already-split concrete pointwise region obligations to the split
projection-level base-tail zero-free witness. -/
theorem step1OStarProjectionSplitBaseTailZeroFreeWitnessData_of_pointwiseRegionNonzero
    {L d : Nat} {θ' : Params L d} {O : Set (ProbePair d)}
    (D : Step1OStarProjectionConcretePointwiseRegionNonzeroObligations L d θ' O) :
    Step1OStarProjectionSplitBaseTailZeroFreeWitnessData L d θ' O where
  propagationPositiveTailSliceNonzeroOnProjection_depth_gt_two := by
    intro hDepth
    exact
      step1PropagationPositiveTailSliceNonzeroOnOStarProjectionDataOfDepthGTTwo_of_indexed
        (D.propagationPositiveTailSlices_depth_gt_two hDepth) hDepth
  propagationPositiveTailSliceNonzeroOnProjectionPropagationVisible_depth_gt_two :=
    step1PropagationPositiveTailSliceNonzeroOnOStarProjectionPropagationVisibleDataOfDepthGTTwo
  visiblePolynomialNonzeroOnProjectionLastRegion :=
...
/-- A bundled projection-level base-tail zero-free witness supplies the active split
base-tail source. -/
theorem step1OStarProjectionSplitBaseTailZeroFreeWitnessData_of_baseTailZeroFree
    {L d : Nat} {θ' : Params L d} {O : Set (ProbePair d)}
    (hzeroFree :
      Step1OStarProjectionBaseTailZeroFreeWitnessData L d θ' O) :
    Step1OStarProjectionSplitBaseTailZeroFreeWitnessData L d θ' O :=
  step1OStarProjectionSplitBaseTailZeroFreeWitnessData_of_pointwiseRegionNonzero
...
/-- Build the split base-tail graph frontier from the bundled base-tail zero-free
witness and the active raw graph no-pole source. -/
theorem step1SplitBaseTailGraphObligations_of_baseTailZeroFree_graphNoPole
    {L d r : Nat} {θ' : Params L d} {O : Set (ProbePair d)}
    (baseTailZeroFree :
      Step1OStarProjectionBaseTailZeroFreeWitnessData L d θ' O)
    (lastBoundaryNoPole :
  ...
  step1SplitBaseTailGraphObligations_of_graphNoPole
    (step1OStarProjectionSplitBaseTailZeroFreeWitnessData_of_baseTailZeroFree
      baseTailZeroFree)
    lastBoundaryNoPole

/-- Compile split pointwise region nonvanishing and the fixed-probe formal-phi
boundary package to the split base-tail graph frontier. -/
theorem step1SplitBaseTailGraphObligations_of_pointwiseRegionNonzero_formalPhiNoPole
    {L d r : Nat} {θ' : Params L d} {O : Set (ProbePair d)}
    {θ : Params L d}
    {hr : 2 <= r}
    {hStanding : Step1StandingAssumptions r L d O θ θ'}
...
/-- Compile the depth-sharpened split concrete frontier to the active
generic-polynomial witness record. -/
theorem step1OStarProjectionGraphGenericPolynomialWitnessData_of_byDepthConcreteObligations
    {L d r : Nat} {θ' : Params L d} {O : Set (ProbePair d)}
    (D : Step1OStarProjectionGraphConcreteByDepthObligations L d r θ' O) :
  ...
  step1OStarProjectionGraphGenericPolynomialWitnessData_of_concreteObligations
    (step1OStarProjectionGraphConcretePolynomialObligations_of_byDepth D)
...
/-- Compile bundled base-tail zero-freeness and raw graph no-pole directly to the
active generic-polynomial witness record. -/
theorem step1OStarProjectionGraphGenericPolynomialWitnessData_of_baseTailZeroFree_graphNoPole
    {L d r : Nat} {θ' : Params L d} {O : Set (ProbePair d)}
    (baseTailZeroFree :
      Step1OStarProjectionBaseTailZeroFreeWitnessData L d θ' O)
    (lastBoundaryNoPole :
...
/-- In depth at most one, the active projection/graph generic-polynomial witness has
no remaining obligations. -/
theorem step1OStarProjectionGraphGenericPolynomialWitnessData_of_depth_le_one
    {L d r : Nat} {θ' : Params L d} {O : Set (ProbePair d)}
    (hDepth : L <= 1) :
    Step1OStarProjectionGraphGenericPolynomialWitnessData L d r θ' O :=
  step1OStarProjectionGraphGenericPolynomialWitnessData_of_concreteObligations
...
/-- Raw probe/coordinate pole-preimage data supplies the existing fixed-probe family
field.

The proof only repackages the concrete primed tier system: the raw system in the
assumption is definitionally the same `ConstantProbeConcreteData.ofConcrete` system used
inside `FixedOStarProbe.concretePrimedTierSystem`. -/
theorem step1OStarRegularPolePreimageFamilyData_of_probeCoordRegularPolePreimageData
    {L d r : Nat} {O : Set (ProbePair d)}
    {θ θ' : Params L d}
    {hr : 2 <= r}
    (hpreimage :
    ...
    Step1OStarRegularPolePreimageFamilyData
      (L := L) (d := d) (r := r) (O := O) (θ := θ) (θ' := θ') hr where
  regularPolePreimage_depth_gt_one := by
    intro _hStanding hDepth p j τ hj hτ hH
    let A := fixedOStarProbeCoordTierSystem (r := r) (θ' := θ') p.probe p.iota
    have hτA : τ ∈ A.T0 j := by
      simpa [A, fixedOStarProbeCoordTierSystem,
        FixedOStarProbe.concretePrimedTierSystem, FixedOStarProbe.concretePrimedStratification,
        FixedOStarProbe.concretePrimedData] using hτ
    have hHA : BlowsUpAt (A.stratification.H (j + 1)) τ := by
      simpa [A, fixedOStarProbeCoordTierSystem,
        FixedOStarProbe.concretePrimedTierSystem, FixedOStarProbe.concretePrimedStratification,
        FixedOStarProbe.concretePrimedData] using hH
    have hfreq :=
      hpreimage.regularPolePreimageOnProbeCoord_depth_gt_one
        hDepth p.probe p.mem_O_star p.iota p.visible_iota_ne hj hτA hHA
    simpa [A, fixedOStarProbeCoordTierSystem,
      FixedOStarProbe.concretePrimedTierSystem, FixedOStarProbe.concretePrimedStratification,
      FixedOStarProbe.concretePrimedData] using hfreq

/-- Family-level regular-pole preimage data from raw probe/coordinate reciprocal normal
forms.

This combines the pointwise reciprocal bridge with the existing raw-to-fixed-probe
repackaging theorem. -/
theorem step1OStarRegularPolePreimageFamilyData_of_probeCoord_analyticAt_reciprocalNormalForms
    {L d r : Nat} {O : Set (ProbePair d)}
    {θ θ' : Params L d}
    {hr : 2 <= r}
    (hnormal :
  ...
  step1OStarRegularPolePreimageFamilyData_of_probeCoordRegularPolePreimageData
    (step1OStarProbeCoordRegularPolePreimageData_of_analyticAt_reciprocalNormalForms
      (r := r) (θ' := θ') (O := O) hnormal)

/-- Family-level regular-pole preimage data from raw probe/coordinate local reciprocal
normal forms. -/
theorem step1OStarRegularPolePreimageFamilyData_of_probeCoord_analyticAt_eventually_reciprocalNormalForms
    {L d r : Nat} {O : Set (ProbePair d)}
    {θ θ' : Params L d}
    {hr : 2 <= r}
    (hnormal :
...
/-- Family-level regular-pole preimage package from the raw probe-coordinate scaled
quadratic normal form. -/
theorem step1OStarRegularPolePreimageFamilyData_of_OStar_genericity
    {L d r : Nat} {O : Set (ProbePair d)}
    {θ θ' : Params L d}
    {hr : 2 <= r} :
    Step1OStarRegularPolePreimageFamilyData
      (L := L) (d := d) (r := r) (O := O) (θ := θ) (θ' := θ') hr :=
  step1OStarRegularPolePreimageFamilyData_of_probeCoordRegularPolePreimageData
    (step1OStarProbeCoordRegularPolePreimageData_of_OStar_genericity
      (L := L) (d := d) (r := r) (θ' := θ') (O := O))

/-- A family-level fixed-probe regular-pole preimage package supplies the raw
probe/coordinate preimage package once a standing-assumptions package is fixed. -/
theorem step1OStarProbeCoordRegularPolePreimageData_of_regularPolePreimageFamilyData
    {L d r : Nat} {O : Set (ProbePair d)}
    {θ θ' : Params L d}
    {hr : 2 <= r}
    (hpreimage :
  ...
  regularPolePreimageOnProbeCoord_depth_gt_one := by
    ...
    change ∀ {j : Nat} {τ : ℂ}, j + 1 < L -> τ ∈ A.T0 j ->
      ...
      ∃ᶠ z in puncturedNhds τ, A.stratification.H (j + 1) z ∈ oddPiI
    intro j τ hj hτ hH
    have hτp : τ ∈ p.concretePrimedTierSystem.T0 j := by
      simpa [p, A, fixedOStarProbeOfOStarVisibleCoord,
        fixedOStarProbeCoordTierSystem,
        FixedOStarProbe.concretePrimedTierSystem, FixedOStarProbe.concretePrimedStratification,
        FixedOStarProbe.concretePrimedData] using hτ
    have hHp :
      ...
      simpa [p, A, fixedOStarProbeOfOStarVisibleCoord,
        ...
        FixedOStarProbe.concretePrimedData] using hH
    have hfreq :=
      hpreimage.regularPolePreimage_depth_gt_one hStanding hDepth p hj hτp hHp
    simpa [p, A, fixedOStarProbeOfOStarVisibleCoord,
      fixedOStarProbeCoordTierSystem,
      FixedOStarProbe.concretePrimedTierSystem, FixedOStarProbe.concretePrimedStratification,
      FixedOStarProbe.concretePrimedData] using hfreq

/-- A fixed-standing-assumptions pole-preimage source supplies the raw
probe/coordinate pole-preimage package for that same standing package. -/
theorem step1OStarProbeCoordRegularPolePreimageData_of_fixedRegularPolePreimageDataOfDepthGTOne
    {L d r : Nat} {O : Set (ProbePair d)}
    {θ θ' : Params L d}
    {hr : 2 <= r}
    (hStanding : Step1StandingAssumptions r L d O θ θ')
  ...
  regularPolePreimageOnProbeCoord_depth_gt_one := by
    ...
    have hHp :
      ...
      simpa [p, A, fixedOStarProbeOfOStarVisibleCoord,
        ...
        FixedOStarProbe.concretePrimedTierSystem, FixedOStarProbe.concretePrimedStratification,
        FixedOStarProbe.concretePrimedData] using hH
    have hfreq := hpreimage hDepth p hj hτp hHp
    simpa [p, A, fixedOStarProbeOfOStarVisibleCoord,
      fixedOStarProbeCoordTierSystem,
      FixedOStarProbe.concretePrimedTierSystem, FixedOStarProbe.concretePrimedStratification,
      FixedOStarProbe.concretePrimedData] using hfreq

/-- The older fixed-probe zero-free level-set Claim-B source supplies the raw
probe/coordinate pole-preimage package for the same standing assumptions. -/
theorem step1OStarProbeCoordRegularPolePreimageData_of_fixedZeroFreeLevelSetLeadData
    {L d r : Nat} {O : Set (ProbePair d)}
    {θ θ' : Params L d}
    {hr : 2 <= r}
    {hStanding : Step1StandingAssumptions r L d O θ θ'}
    (hlevel : FixedOStarZeroFreeLevelSetLeadData hr hStanding) :
    Step1OStarProbeCoordRegularPolePreimageData L d r θ' O :=
  step1OStarProbeCoordRegularPolePreimageData_of_fixedRegularPolePreimageDataOfDepthGTOne
    hStanding
    (fixedOStarRegularPolePreimageDataOfDepthGTOne_of_zeroFreeLevelSetLeadData
      hlevel)

/-- A family of older fixed-probe zero-free level-set Claim-B sources supplies the
active family-level regular-pole preimage source. -/
theorem step1OStarRegularPolePreimageFamilyData_of_zeroFreeLevelSetLeadData
    {L d r : Nat} {O : Set (ProbePair d)}
    {θ θ' : Params L d}
    {hr : 2 <= r}
    (hlevel :
...
/-- Raw probe/coordinate pole-preimage data plus the shared generic-polynomial witness
recovers the active regular-pole-at-tier family. -/
theorem fixedOStarRegularPoleLeadAtTierData_family_of_probeCoordPreimage_and_genericPolynomials
    {L d r : Nat} {O : Set (ProbePair d)}
    {θ θ' : Params L d}
    {hr : 2 <= r}
    (hpreimage : Step1OStarProbeCoordRegularPolePreimageData L d r θ' O)
    (hgeneric :
      Step1OStarProjectionGraphGenericPolynomialWitnessData L d r θ' O) :
    ∀ hStanding : Step1StandingAssumptions r L d O θ θ',
      FixedOStarRegularPoleLeadAtTierData hr hStanding :=
...
/-- Compile raw probe/coordinate pole-preimage data plus the bundled base-tail
zero-free graph frontier directly to the fixed-`O_star` Step 1 provider. -/
theorem fixedOStarStep1ProviderData_of_probeCoordPreimage_baseTailZeroFreeConcrete
    {L d r : Nat} {O : Set (ProbePair d)}
    {θ θ' : Params L d}
    {hr : 2 <= r}
    {hStanding : Step1StandingAssumptions r L d O θ θ'}
    (hpreimage : Step1OStarProbeCoordRegularPolePreimageData L d r θ' O)
    (D : Step1OStarProjectionGraphBaseTailZeroFreeConcreteObligations L d r θ' O) :
    FixedOStarStep1ProviderData hr hStanding := by
  let genericPolynomials :
      Step1OStarProjectionGraphGenericPolynomialWitnessData L d r θ' O :=
    step1OStarProjectionGraphGenericPolynomialWitnessData_of_baseTailZeroFreeConcrete D
  exact
    fixedOStarStep1ProviderData_of_graphBoundaryBaseTailLeadAtTierProjectionGraphReducedStep1ProviderData
      (fixedOStarProjectionGraphReducedProviderData_of_regularPole_and_genericPolynomials
        ((fixedOStarRegularPoleLeadAtTierData_family_of_probeCoordPreimage_and_genericPolynomials
          (θ := θ) (hr := hr) hpreimage genericPolynomials) hStanding)
        genericPolynomials)

set_option linter.style.longLine false in
/-- Compile raw probe/coordinate pole-preimage data plus the split base-tail graph
frontier directly to the fixed-`O_star` Step 1 provider. -/
theorem fixedOStarStep1ProviderData_of_probeCoordPreimage_splitBaseTailZeroFreeConcrete
    {L d r : Nat} {O : Set (ProbePair d)}
    {θ θ' : Params L d}
    {hr : 2 <= r}
    {hStanding : Step1StandingAssumptions r L d O θ θ'}
    ...
    (D :
      Step1OStarProjectionGraphSplitBaseTailZeroFreeConcreteObligations
        L d r θ' O) :
    FixedOStarStep1ProviderData hr hStanding :=
  fixedOStarStep1ProviderData_of_probeCoordPreimage_baseTailZeroFreeConcrete
    hpreimage
    (step1OStarProjectionGraphBaseTailZeroFreeConcreteObligations_of_splitBaseTail D)

/-- Compile raw probe/coordinate pole-preimage data plus split base-tail zero-freeness
and raw graph no-pole directly to the fixed-`O_star` Step 1 provider. -/
theorem fixedOStarStep1ProviderData_of_probeCoordPreimage_splitBaseTail_graphNoPole
    {L d r : Nat} {O : Set (ProbePair d)}
    {θ θ' : Params L d}
    {hr : 2 <= r}
    {hStanding : Step1StandingAssumptions r L d O θ θ'}
    ...
    (lastBoundaryNoPole :
      Step1LastBoundaryFormalPhiNoPoleOnOStarProbeCoordGraphData
        (L := L) (d := d) (O := O) (r := r) θ') :
    FixedOStarStep1ProviderData hr hStanding :=
  fixedOStarStep1ProviderData_of_probeCoordPreimage_splitBaseTailZeroFreeConcrete
    hpreimage
    (step1SplitBaseTailGraphObligations_of_graphNoPole
      baseTailZeroFree lastBoundaryNoPole)

/-- Compile family-level regular-pole preimage data plus split base-tail zero-freeness
and raw graph no-pole directly to the fixed-`O_star` Step 1 provider. -/
theorem fixedOStarStep1ProviderData_of_regularPolePreimage_splitBaseTail_graphNoPole
    {L d r : Nat} {O : Set (ProbePair d)}
    {θ θ' : Params L d}
    {hr : 2 <= r}
    {hStanding : Step1StandingAssumptions r L d O θ θ'}
    ...
    (lastBoundaryNoPole :
      Step1LastBoundaryFormalPhiNoPoleOnOStarProbeCoordGraphData
        (L := L) (d := d) (O := O) (r := r) θ') :
    FixedOStarStep1ProviderData hr hStanding :=
  fixedOStarStep1ProviderData_of_probeCoordPreimage_splitBaseTail_graphNoPole
    (step1OStarProbeCoordRegularPolePreimageData_of_regularPolePreimageFamilyData
      hpreimage hStanding)
    baseTailZeroFree lastBoundaryNoPole

/-- Compile family-level regular-pole preimage data, split base-tail zero-freeness,
and the fixed-probe formal-phi no-pole package directly to the fixed-`O_star`
Step 1 provider.

The boundary field is first transported to the raw probe/coordinate graph, so this is
the formal-phi source variant of
`fixedOStarStep1ProviderData_of_regularPolePreimage_splitBaseTail_graphNoPole`. -/
theorem fixedOStarStep1ProviderData_of_regularPolePreimage_splitBaseTail_formalPhiNoPole
    {L d r : Nat} {O : Set (ProbePair d)}
    {θ θ' : Params L d}
    {hr : 2 <= r}
    {hStanding : Step1StandingAssumptions r L d O θ θ'}
    ...
    FixedOStarStep1ProviderData hr hStanding :=
  fixedOStarStep1ProviderData_of_regularPolePreimage_splitBaseTail_graphNoPole
    hpreimage baseTailZeroFree
    (step1LastBoundaryFormalPhiNoPoleOnOStarProbeCoordGraphData_of_lastBoundaryFormalPhiNoPole
      lastBoundaryFormalPhiNoPole)

/-- Compile the split-lead pointwise provider directly to the endpoint provider package. -/
theorem fixedOStarStep1ProviderData_of_splitLeadPointwiseData
    {L d r : Nat} {O : Set (ProbePair d)}
    {θ θ' : Params L d}
    {hr : 2 <= r}
    {hStanding : Step1StandingAssumptions r L d O θ θ'}
    (D : FixedOStarDepthGatedPreimageSplitLeadPointwiseStep1ProviderData hr hStanding) :
    FixedOStarStep1ProviderData hr hStanding :=
  fixedOStarStep1ProviderData_of_depthGatedPreimagePointwiseData
    (fixedOStarDepthGatedPreimagePointwiseStep1ProviderData_of_splitLeadPointwiseData D)

/-- Exact remaining analytic inputs needed by the compiled Step 1 provider surface.

The standing `O_star` genericity assumptions only package the polynomial/nonempty
facts used to choose generic probes.  The recursive fixed-probe proof still needs
these two pointwise/frequent zero-free obligations. -/
structure FixedOStarStep1ProviderCoreData
    {L d r : Nat} {O : Set (ProbePair d)}
    {θ θ' : Params L d}
    (hr : 2 <= r)
    (hStanding : Step1StandingAssumptions r L d O θ θ') : Prop where
  regularPoleLeadAtTier : FixedOStarRegularPoleLeadAtTierData hr hStanding
  visibleNestedRegion : FixedOStarLastTierVisibleNestedRegionData hr hStanding

/-- Constructor from the isolated core analytic inputs to the provider package
consumed by the Step 1 endpoint. -/
theorem fixedOStarStep1ProviderData_of_coreData
    {L d r : Nat} {O : Set (ProbePair d)}
    {θ θ' : Params L d}
    {hr : 2 <= r}
    {hStanding : Step1StandingAssumptions r L d O θ θ'}
    (D : FixedOStarStep1ProviderCoreData hr hStanding) :
    FixedOStarStep1ProviderData hr hStanding :=
  fixedOStarStep1ProviderData_of_regularPoleAndVisibleNestedRegion
    D.regularPoleLeadAtTier D.visibleNestedRegion

/-- Extract the Claim-B provider expected by the endpoint constructors. -/
theorem fixedOStarZeroFreeLevelSetLeadData_of_step1ProviderData
    {L d r : Nat} {O : Set (ProbePair d)}
    {θ θ' : Params L d}
    {hr : 2 <= r}
    {hStanding : Step1StandingAssumptions r L d O θ θ'}
    ...
    FixedOStarZeroFreeLevelSetLeadData hr hStanding :=
  fixedOStarZeroFreeLevelSetLeadData_of_atTierData D.levelSetLeadAtTier
...
/-- Lower last-layer coefficients are continuous at points of the corrected combined
zero-free last tier.

The proof is the same regularity argument as for `p.concretePrimedTierSystem`: the
combined tier system has the same concrete stratification, and membership in its last
zero-free tier still puts the point in the last stratum and hence outside all earlier
strata. -/
theorem fixedOStar_propagationVisibleTierSystem_lowerCoeff_continuousAt_of_T0
    {L d r : Nat} {O : Set (ProbePair d)}
    {θ θ' : Params L d}
    (p : FixedOStarProbe r L d O θ θ') :
    ∀ {τ : ℂ}, 0 < L ->
      τ ∈ p.propagationVisibleTierSystem.T0 (L - 1) ->
        ∀ i, i < L - 1 ->
          ContinuousAt
            (p.propagationVisibleTierSystem.stratification.lastLayer.coeff i) τ := by
  intro τ hL hτ i hi
  cases L with
  | zero =>
      omega
  | succ m =>
      let A := p.propagationVisibleTierSystem
      have hlast : (m + 1) - 1 < m + 1 := by omega
      have hτT : τ ∈ A.T ((m + 1) - 1) :=
        A.T0_subset_T ((m + 1) - 1) hτ
      have hregularLast : τ ∉ partialUnion A.stratification.S ((m + 1) - 1) := by
        have hω : τ ∈ A.stratification.omega ((m + 1) - 1) :=
          A.mem_omega hlast hτT
        simpa [ConcreteStratification.omega] using hω
      have hregular_i :
          τ ∉ partialUnion A.stratification.S (i + 1) := by
        intro hmem
      ...
      have hcont :=
        constantProbeObservableExpansion_coeff_continuousAt_of_regular
          (paramStream θ') (Real.log (r : ℝ)) p.probe.1 p.probe.2
          (m + 1) i p.iota τ (by omega) (by
            simpa [A, FixedOStarProbe.propagationVisibleTierSystem,
              TierSystem.ofPolynomialNestedTailData,
              FixedOStarProbe.concretePrimedTierSystem,
              FixedOStarProbe.concretePrimedStratification,
              FixedOStarProbe.concretePrimedData,
              ConstantProbeConcreteData.toConcreteStratification,
              ConcreteStratificationData.toConcreteStratification,
...
/-- The final visible coefficient is continuous at points of the corrected combined
zero-free last tier. -/
theorem fixedOStar_propagationVisibleTierSystem_visibleCoeff_continuousAt_of_T0
    {L d r : Nat} {O : Set (ProbePair d)}
    {θ θ' : Params L d}
    (p : FixedOStarProbe r L d O θ θ') :
    ∀ {τ : ℂ}, 0 < L ->
      τ ∈ p.propagationVisibleTierSystem.T0 (L - 1) ->
        ContinuousAt
          (p.propagationVisibleTierSystem.stratification.lastLayer.coeff (L - 1)) τ := by
  intro τ hL hτ
  cases L with
  | zero =>
      omega
...
/-- Version of the combined-tier visible-coefficient nonvanishing theorem valid in all
positive depths.  The depth-one branch is the already-formalized constant-polynomial
case; the genuine `1 < L` branch uses the combined propagation-visible tower. -/
theorem fixedOStar_lastLayer_visibleCoeff_ne_zero_of_mem_propagationVisibleTierSystem_T0_pos
    {L d r : Nat} {O : Set (ProbePair d)}
    {θ θ' : Params L d}
    (p : FixedOStarProbe r L d O θ θ')
    (hL : 0 < L) {τ : ℂ}
    (hτ : τ ∈ p.propagationVisibleTierSystem.T0 (L - 1)) :
    p.propagationVisibleTierSystem.stratification.lastLayer.coeff (L - 1) τ ≠ 0 := by
  by_cases hDepth : 1 < L
  · simpa [FixedOStarProbe.propagationVisibleTierSystem,
      TierSystem.ofPolynomialNestedTailData] using
      fixedOStar_lastLayer_visibleCoeff_ne_zero_of_mem_propagationVisibleTierSystem_T0
        p hDepth hτ
  · have hL_eq : L = 1 := by omega
    subst L
    have hpoly :
        Step1PolynomialFamilyAssumptionsForDepth 1 d (paramStream θ') p.probe.1 p.iota :=
      p.step1PolynomialFamilyAssumptionsForDepth (by omega)
    have hvisible :
        MvPolynomial.eval
          (gatePrefix p.concretePrimedTierSystem.stratification (1 - 1) τ)
          (step1VisiblePoly (K := 1 - 1) (paramStream θ') p.probe.1 p.iota) ≠ 0 := by
      simpa [show (1 - 1 : Nat) = 0 by omega] using
        eval_ne_zero_of_ne_zero_fin_zero
          hpoly.hasDominantTopCoeff_visible.ne_zero
          (gatePrefix p.concretePrimedTierSystem.stratification 0 τ)
    have hcoeff :
        p.concretePrimedTierSystem.stratification.lastLayer.coeff (1 - 1) τ ≠ 0 := by
      rw [fixedOStar_lastLayer_visibleCoeff_eq_step1VisiblePoly p (by omega) τ]
      exact hvisible
    simpa [FixedOStarProbe.propagationVisibleTierSystem,
      TierSystem.ofPolynomialNestedTailData] using hcoeff

/-- Corrected zero-free Claim C data for the combined propagation-visible tier system.

This is the first Claim-C consumer of the combined tower: lower boundedness and
coefficient convergence come from concrete regularity, while coefficient nonvanishing
comes from the combined zero-free region. -/
noncomputable def fixedOStar_propagationVisibleTierSystem_zeroFreeLastTierBlowupData
    {L d r : Nat} {O : Set (ProbePair d)}
    {θ θ' : Params L d}
    (p : FixedOStarProbe r L d O θ θ') :
    ZeroFreeLastTierBlowupData p.propagationVisibleTierSystem where
  lower_puncturedBounded := by
    intro τ hL hτ0
    exact
      lastTierLower_puncturedBounded_of_continuousAt
        (A := p.propagationVisibleTierSystem) hL
        (fun i hi =>
          lastTierLowerGate_continuousAt_of_mem
            p.propagationVisibleTierSystem hL
            (p.propagationVisibleTierSystem.T0_subset_T (L - 1) hτ0) hi)
        (fun i hi =>
          fixedOStar_propagationVisibleTierSystem_lowerCoeff_continuousAt_of_T0
            p hL hτ0 i hi)
  visibleCoeffLimit :=
    fun τ => p.propagationVisibleTierSystem.stratification.lastLayer.coeff (L - 1) τ
  visibleCoeff_tendsto := by
    intro τ hL hτ0
    exact
      (fixedOStar_propagationVisibleTierSystem_visibleCoeff_continuousAt_of_T0
        p hL hτ0).tendsto.mono_left nhdsWithin_le_nhds
  visibleCoeff_ne_zero := by
    intro τ hL hτ0
    exact
      fixedOStar_lastLayer_visibleCoeff_ne_zero_of_mem_propagationVisibleTierSystem_T0_pos
        p hL hτ0

/-- Corrected zero-free Claim C blow-up package for the combined propagation-visible tier
system. -/
noncomputable def fixedOStar_propagationVisibleTierSystem_zeroFreeLastTierBlowup
    {L d r : Nat} {O : Set (ProbePair d)}
    {θ θ' : Params L d}
    (p : FixedOStarProbe r L d O θ θ') (hL : 0 < L) :
    ZeroFreeLastTierBlowup p.propagationVisibleTierSystem :=
  ZeroFreeLastTierBlowupData.toZeroFreeLastTierBlowup
    (fixedOStar_propagationVisibleTierSystem_zeroFreeLastTierBlowupData p) hL

/-- Pointwise corrected Claim C: the selected observable blows up at every point of the
combined propagation-visible zero-free last tier. -/
theorem fixedOStar_propagationVisibleTierSystem_observable_blowsUpAt
    {L d r : Nat} {O : Set (ProbePair d)}
    {θ θ' : Params L d}
    (p : FixedOStarProbe r L d O θ θ') (hL : 0 < L)
    {τ : ℂ} (hτ : τ ∈ p.propagationVisibleTierSystem.T0 (L - 1)) :
    BlowsUpAt p.propagationVisibleTierSystem.stratification.observable τ :=
  ZeroFreeLastTierBlowupData.observable_blowsUpAt
    (fixedOStar_propagationVisibleTierSystem_zeroFreeLastTierBlowupData p) hL hτ

/-- Corrected mixed terminal tier family for the Step 1 endpoint route.

Interior levels use the canonical formal-phi zero-free tiers.  The terminal level uses
the combined propagation-visible zero-free tier, where Claim C is already proved without
the false boundary formal-phi lead. -/
noncomputable def FixedOStarProbe.mixedTerminalTier
    {L d r : Nat} {O : Set (ProbePair d)}
    {θ θ' : Params L d}
    (p : FixedOStarProbe r L d O θ θ') : Nat -> Set ℂ :=
  fun j =>
    if j + 1 < L then
      p.concretePrimedTierSystem.T0 j
    else
      p.propagationVisibleTierSystem.T0 j

theorem fixedOStar_mixedTerminalTier_eq_concretePrimed_of_lt
    {L d r : Nat} {O : Set (ProbePair d)}
    {θ θ' : Params L d}
    (p : FixedOStarProbe r L d O θ θ')
    {j : Nat} (hj : j + 1 < L) :
    p.mixedTerminalTier j = p.concretePrimedTierSystem.T0 j := by
  simp [FixedOStarProbe.mixedTerminalTier, hj]

theorem fixedOStar_mixedTerminalTier_zero_eq_concretePrimed_of_depth_gt_one
    {L d r : Nat} {O : Set (ProbePair d)}
    {θ θ' : Params L d}
    (p : FixedOStarProbe r L d O θ θ') (hDepth : 1 < L) :
    p.mixedTerminalTier 0 = p.concretePrimedTierSystem.T0 0 := by
  exact fixedOStar_mixedTerminalTier_eq_concretePrimed_of_lt p (by omega)

theorem fixedOStar_mixedTerminalTier_eq_propagationVisible_of_not_lt
    {L d r : Nat} {O : Set (ProbePair d)}
    {θ θ' : Params L d}
    (p : FixedOStarProbe r L d O θ θ')
    {j : Nat} (hj : ¬ j + 1 < L) :
    p.mixedTerminalTier j = p.propagationVisibleTierSystem.T0 j := by
  simp [FixedOStarProbe.mixedTerminalTier, hj]

theorem fixedOStar_mixedTerminalTier_last_eq_propagationVisible
    {L d r : Nat} {O : Set (ProbePair d)}
    {θ θ' : Params L d}
    (p : FixedOStarProbe r L d O θ θ') (hL : 0 < L) :
    p.mixedTerminalTier (L - 1) =
      p.propagationVisibleTierSystem.T0 (L - 1) := by
  exact fixedOStar_mixedTerminalTier_eq_propagationVisible_of_not_lt p (by omega)

/-- Corrected Claim-D transfer for the combined propagation-visible terminal tier. -/
theorem fixedOStar_propagationVisibleLastTier_subset_partialUnion
    {L d r : Nat} {O : Set (ProbePair d)}
    {θ θ' : Params L d}
    (p : FixedOStarProbe r L d O θ θ') (hr : 0 < r) (hL : 0 < L) :
    p.propagationVisibleTierSystem.T0 (L - 1) ⊆
      partialUnion p.concreteUnprimedStratification.S L := by
  exact
    transferred_zeroFreeLastTier_subset_partialUnion_ofConcreteWithBlowupOnRealTail
      (unprimed := p.concreteUnprimedStratification)
      (A := p.propagationVisibleTierSystem)
      hL (max p.T0 0)
      (by
        intro t ht
        simpa [FixedOStarProbe.propagationVisibleTierSystem,
      ...
      (fixedOStar_propagationVisibleTierSystem_zeroFreeLastTierBlowup p hL)

theorem fixedOStar_mixedTerminalTier_last_subset_partialUnion
    {L d r : Nat} {O : Set (ProbePair d)}
    {θ θ' : Params L d}
    (p : FixedOStarProbe r L d O θ θ') (hr : 0 < r) (hL : 0 < L) :
    p.mixedTerminalTier (L - 1) ⊆
      partialUnion p.concreteUnprimedStratification.S L := by
  rw [fixedOStar_mixedTerminalTier_last_eq_propagationVisible p hL]
  exact fixedOStar_propagationVisibleLastTier_subset_partialUnion p hr hL
...
/-- Compatibility alias for the depth-`> 1` fixed-`O_star` bridge. -/
noncomputable def fixedOStar_zeroFreeAnalyticBridgeData_of_tex_of_depth_gt_one
    {L d r : Nat} {O : Set (ProbePair d)}
    {θ θ' : Params L d}
    (hr : 2 <= r)
    (hStanding : Step1StandingAssumptions r L d O θ θ')
    ...
    (p : FixedOStarProbe r L d O θ θ')
    (hDepth : 1 < L) :
    FixedOStarZeroFreeAnalyticBridgeData p :=
...
/-- Depth-`> 1` fixed-probe zero-free concrete descent.  This version no longer uses
the first-leading-coefficient placeholder; it proves that field from the `O_star` polynomial
family assumptions and still depends on the remaining Claim B/C interfaces. -/
theorem fixedOStar_zeroFreeConcreteDescent_exists_of_tex_of_depth_gt_one
    {L d r : Nat} {O : Set (ProbePair d)}
    {θ θ' : Params L d}
    (hr : 2 <= r)
    (hStanding : Step1StandingAssumptions r L d O θ θ')
    ...
    ∃ m : Nat, ∃ D : ZeroFreeConcreteDescentData m,
      D.unprimed.lambda1 = firstSlope θ p.probe.1 p.probe.2 ∧
        D.primed.stratification.lambda1 =
          firstSlope θ' p.probe.1 p.probe.2 := by
  exact fixedOStar_zeroFreeConcreteDescent_exists_of_tex hr hStanding claimB claimC p hDepth
...
/-- Depth-drop transfer for the corrected propagation-visible terminal tier.

This is the Claim-D tail-transfer analogue of
`fixedOStar_propagationVisibleLastTier_subset_partialUnion`, used by the corrected
endpoint route when the first unprimed value matrix is assumed to vanish. -/
theorem fixedOStar_propagationVisibleLastTier_subset_tail_partialUnion_of_headValue_zero
    {m d r : Nat} {O : Set (ProbePair d)}
    {θ θ' : Params (m + 1) d}
    (hr : 0 < r)
    (p : FixedOStarProbe r (m + 1) d O θ θ')
    (firstValue_zero : (θ 0).1 = 0) :
    p.propagationVisibleTierSystem.T0 m ⊆
      partialUnion
        (ConstantProbeConcreteData.toConcreteStratification
          (ConstantProbeConcreteData.ofConcrete
            (paramStream (Params.tail θ)) (Real.log (r : ℝ))
  ...
  have hEqOnTail :
    ...
    calc
      ...
      _ = A.stratification.observable (t : ℂ) := by
          simp [A, FixedOStarProbe.propagationVisibleTierSystem,
            TierSystem.ofPolynomialNestedTailData]
  have hblow : ∀ τ, τ ∈ A.T0 ((m + 1) - 1) ->
      BlowsUpAt A.stratification.observable τ := by
    intro τ hτ
    exact
      (fixedOStar_propagationVisibleTierSystem_zeroFreeLastTierBlowup p
        (Nat.succ_pos m)).observable_blowsUpAt hτ
  have hfreq :
      ∃ᶠ z in nhdsWithin ((max p.T0 0 + 1 : ℝ) : ℂ)
        ({((max p.T0 0 + 1 : ℝ) : ℂ)}ᶜ : Set ℂ),
        tailStrat.observable z = A.stratification.observable z :=
...
/-- Step 1 endpoint from the combined fixed-`O_star` provider package. -/
theorem firstLayerEndpoint_of_step1ProviderData
    {L d r : Nat} {O : Set (ProbePair d)}
    {θ θ' : Params (L + 1) d}
    (hr : 2 <= r)
    (hStanding : Step1StandingAssumptions r (L + 1) d O θ θ')
    ...
    FirstLayerEndpointData
      (Params.headAttention θ) (Params.headAttention θ')
      (Params.headValue θ) :=
  firstLayerEndpoint_of_step1Standing hr hStanding
    (fixedOStarZeroFreeLevelSetLeadData_of_step1ProviderData D)
    (fixedOStarLastTierVisibleCoeffNonzeroData_of_step1ProviderData D)
    hDepth
...
/-- Step 1 endpoint specialized to global probe agreement at the top-level wrapper. -/
theorem firstLayerEndpoint_of_texGenericStep
    {L d r : Nat} (hr : 2 <= r)
    {θ θ' : Params (L + 1) d}
    (hstep :
      TexGenericStepClauses L d (TexGeneric L d)
    ...
    (claimB :
      ∀ hStanding : Step1StandingAssumptions r (L + 1) d Set.univ θ θ',
        FixedOStarZeroFreeLevelSetLeadData hr hStanding)
    (claimC :
      ∀ hStanding : Step1StandingAssumptions r (L + 1) d Set.univ θ θ',
        FixedOStarLastTierVisibleCoeffNonzeroData hr hStanding) :
  ...
  | zero =>
      ...
      have hvalue : Params.headValue θ = Params.headValue θ' :=
        IDL_depth_one_headValue_eq_of_probeAgreement hr_pos hagree
      have hvalue_ne : Params.headValue θ ≠ 0 := by
        rw [hvalue]
        have hV : (paramStream θ' 0).1 ≠ 0 := by
          intro hzero
          have hzero' : (θ' 0).1 = 0 := by
            simpa [paramStream] using hzero
          exact hstep.g2_visible (by
            simpa [realVprod, hzero'])
        simpa [Params.headValue, Params.headLayer, paramStream_apply_of_lt] using hV
      have hattention : Params.headAttention θ = Params.headAttention θ' :=
        IDL_depth_one_headAttention_eq_of_probeAgreement
          hr_pos hagree hvalue hvalue_ne
      exact FirstLayerEndpointData.of_attention_eq hattention hvalue_ne
  | succ L =>
      let O : Set (ProbePair d) := Set.univ
      have hStanding : Step1StandingAssumptions r (L + 1 + 1) d O θ θ' :=
        { depth_pos := Nat.succ_pos (L + 1)
          generic := hstep.toOStarGenericAssumptions isOpen_univ Set.univ_nonempty
          agreement := (constantProbeTailAgreement_of_probeObservableAgreement hagree).toLocal }
      exact firstLayerEndpoint_of_step1Standing hr hStanding
        (claimB hStanding) (claimC hStanding) (Nat.succ_pos L)

set_option linter.style.longLine false in
/-- Step 1 endpoint from the local path/open-set IDL invariant using the active
projection/graph-reduced Step 1 frontier.

`TexGenericStepClauses` contributes only the matrix-level `O_star` genericity used in
`hStanding`.  The remaining Step 1 polynomial/no-pole inputs are exactly the three
fields of `Step1OStarProjectionGraphGenericPolynomialWitnessData`: positive-tail
propagation nonvanishing on the `O_star` input projection, visible-polynomial
nonvanishing on the projected last propagation region, and last-boundary no-pole on
the raw probe/coordinate graph. -/
theorem firstLayerEndpoint_of_texGenericStep_of_IDLData_projectionGraphGenericPolynomials
    {L d r : Nat} (hr : 2 <= r)
    {θ θ' : Params (L + 1) d}
    (hstep :
      TexGenericStepClauses L d (TexGeneric L d)
    ...
    (regularPoleLeadAtTier :
      ∀ hStanding : Step1StandingAssumptions r (L + 1) d D.O θ θ',
        FixedOStarRegularPoleLeadAtTierData hr hStanding)
    (genericPolynomials :
      Step1OStarProjectionGraphGenericPolynomialWitnessData
        (L + 1) d r θ' D.O) :
    FirstLayerEndpointData
      (Params.headAttention θ) (Params.headAttention θ')
      (Params.headValue θ) := by
  ...
  exact firstLayerEndpoint_of_texGenericStep_of_IDLData hr hstep D
    (fun hStanding =>
      fixedOStarZeroFreeLevelSetLeadData_of_step1ProviderData
        (step1Provider hStanding))
    (fun hStanding =>
      fixedOStarLastTierVisibleCoeffNonzeroData_of_step1ProviderData
        (step1Provider hStanding))

set_option linter.style.longLine false in
/-- Global-probe Step 1 endpoint using the active projection/graph-reduced Step 1
frontier.

This is the public `Set.univ` counterpart of
`firstLayerEndpoint_of_texGenericStep_of_IDLData_projectionGraphGenericPolynomials`.
It deliberately does not derive the projection/graph polynomial witness from
`TexGenericStepClauses`; those three zero-free/no-pole fields remain explicit. -/
theorem firstLayerEndpoint_of_texGenericStep_projectionGraphGenericPolynomials
    {L d r : Nat} (hr : 2 <= r)
    {θ θ' : Params (L + 1) d}
    (hstep :
      TexGenericStepClauses L d (TexGeneric L d)
  ...
  let step1Provider :
    ...
    fun hStanding =>
      fixedOStarStep1ProviderData_of_graphBoundaryBaseTailLeadAtTierProjectionGraphReducedStep1ProviderData
        (fixedOStarProjectionGraphReducedProviderData_of_regularPole_and_genericPolynomials
          (regularPoleLeadAtTier hStanding) genericPolynomials)
  exact firstLayerEndpoint_of_texGenericStep hr hstep hagree
    (fun hStanding =>
      fixedOStarZeroFreeLevelSetLeadData_of_step1ProviderData
        (step1Provider hStanding))
    (fun hStanding =>
...
set_option linter.style.longLine false in
/-- Step 1 endpoint from local path/open-set IDL data using the exact current reduced
frontier.

This version accepts the raw probe/coordinate pole-preimage statement and the concrete
projection/graph polynomial obligations directly.  It compiles them to the older
regular-pole-at-tier plus generic-polynomial surface internally. -/
theorem firstLayerEndpoint_of_texGenericStep_of_IDLData_probeCoordPreimage_concretePolynomials
    {L d r : Nat} (hr : 2 <= r)
    {θ θ' : Params (L + 1) d}
    (hstep :
      TexGenericStepClauses L d (TexGeneric L d)
    ...
    (D : IDLData (L + 1) d r θ θ')
    (regularPolePreimage :
      Step1OStarProbeCoordRegularPolePreimageData (L + 1) d r θ' D.O)
    (concretePolynomials :
      Step1OStarProjectionGraphConcretePolynomialObligations
        (L + 1) d r θ' D.O) :
    FirstLayerEndpointData
      (Params.headAttention θ) (Params.headAttention θ')
      (Params.headValue θ) := by
  ...
  exact
    firstLayerEndpoint_of_texGenericStep_of_IDLData_projectionGraphGenericPolynomials
      hr hstep D
      (fixedOStarRegularPoleLeadAtTierData_family_of_probeCoordPreimage_and_genericPolynomials
        (θ := θ) (hr := hr) regularPolePreimage genericPolynomials)
      genericPolynomials

set_option linter.style.longLine false in
/-- Global-probe Step 1 endpoint using the exact current reduced frontier.

This is the `Set.univ` counterpart of
`firstLayerEndpoint_of_texGenericStep_of_IDLData_probeCoordPreimage_concretePolynomials`. -/
theorem firstLayerEndpoint_of_texGenericStep_probeCoordPreimage_concretePolynomials
    {L d r : Nat} (hr : 2 <= r)
    {θ θ' : Params (L + 1) d}
    (hstep :
      TexGenericStepClauses L d (TexGeneric L d)
    ...
    (hagree : ProbeObservableAgreement r θ θ')
    (regularPolePreimage :
      Step1OStarProbeCoordRegularPolePreimageData (L + 1) d r θ' Set.univ)
    (concretePolynomials :
      Step1OStarProjectionGraphConcretePolynomialObligations
        (L + 1) d r θ' Set.univ) :
    FirstLayerEndpointData
  ...
  let genericPolynomials :
    ...
    step1OStarProjectionGraphGenericPolynomialWitnessData_of_concreteObligations
      concretePolynomials
  exact
    firstLayerEndpoint_of_texGenericStep_projectionGraphGenericPolynomials
      hr hstep hagree
      (fixedOStarRegularPoleLeadAtTierData_family_of_probeCoordPreimage_and_genericPolynomials
        (θ := θ) (hr := hr) regularPolePreimage genericPolynomials)
      genericPolynomials
...
set_option linter.style.longLine false in
/-- Step 1 endpoint from local path/open-set IDL data using the split depth-sharpened
concrete polynomial frontier.

This variant separates pointwise region nonvanishing from the formal-phi no-pole
witness on the raw graph and asks for the positive propagation slice obligations only
under `2 < L + 1`. -/
theorem firstLayerEndpoint_of_texGenericStep_of_IDLData_probeCoordPreimage_byDepthConcretePolynomials
    {L d r : Nat} (hr : 2 <= r)
    {θ θ' : Params (L + 1) d}
    (hstep :
      TexGenericStepClauses L d (TexGeneric L d)
    ...
    (regularPolePreimage :
      Step1OStarProbeCoordRegularPolePreimageData (L + 1) d r θ' D.O)
    (concretePolynomials :
      Step1OStarProjectionGraphConcreteByDepthObligations
        (L + 1) d r θ' D.O) :
    FirstLayerEndpointData
      (Params.headAttention θ) (Params.headAttention θ')
      (Params.headValue θ) :=
  firstLayerEndpoint_of_texGenericStep_of_IDLData_probeCoordPreimage_concretePolynomials
    hr hstep D regularPolePreimage
    (step1OStarProjectionGraphConcretePolynomialObligations_of_byDepth
      (L := L + 1) (d := d) (r := r) (θ' := θ') (O := D.O)
      concretePolynomials)

set_option linter.style.longLine false in
/-- Global-probe Step 1 endpoint using the split depth-sharpened concrete polynomial
frontier. -/
theorem firstLayerEndpoint_of_texGenericStep_probeCoordPreimage_byDepthConcretePolynomials
    {L d r : Nat} (hr : 2 <= r)
    {θ θ' : Params (L + 1) d}
    (hstep :
      TexGenericStepClauses L d (TexGeneric L d)
    ...
    FirstLayerEndpointData
      (Params.headAttention θ) (Params.headAttention θ')
      (Params.headValue θ) :=
  firstLayerEndpoint_of_texGenericStep_probeCoordPreimage_concretePolynomials
    hr hstep hagree regularPolePreimage
    (step1OStarProjectionGraphConcretePolynomialObligations_of_byDepth
      (L := L + 1) (d := d) (r := r) (θ' := θ') (O := Set.univ)
      concretePolynomials)

set_option linter.style.longLine false in
/-- Step 1 endpoint from local path/open-set IDL data using the bundled base-tail
zero-free graph frontier.

The two pointwise region-nonvanishing fields of the older concrete-polynomial frontier
are supplied through one projection-level base-tail zero-free witness. -/
theorem firstLayerEndpoint_of_texGenericStep_of_IDLData_probeCoordPreimage_baseTailZeroFreeConcrete
    {L d r : Nat} (hr : 2 <= r)
    {θ θ' : Params (L + 1) d}
    (hstep :
      TexGenericStepClauses L d (TexGeneric L d)
    ...
    (regularPolePreimage :
      Step1OStarProbeCoordRegularPolePreimageData (L + 1) d r θ' D.O)
    (concretePolynomials :
      Step1OStarProjectionGraphBaseTailZeroFreeConcreteObligations
        (L + 1) d r θ' D.O) :
    FirstLayerEndpointData
      (Params.headAttention θ) (Params.headAttention θ')
      (Params.headValue θ) :=
  firstLayerEndpoint_of_texGenericStep_of_IDLData_probeCoordPreimage_concretePolynomials
    hr hstep D regularPolePreimage
    (step1OStarProjectionGraphConcretePolynomialObligations_of_baseTailZeroFreeConcrete
      concretePolynomials)

set_option linter.style.longLine false in
/-- Global-probe Step 1 endpoint using the bundled base-tail zero-free graph frontier. -/
theorem firstLayerEndpoint_of_texGenericStep_probeCoordPreimage_baseTailZeroFreeConcrete
    {L d r : Nat} (hr : 2 <= r)
    {θ θ' : Params (L + 1) d}
    (hstep :
      TexGenericStepClauses L d (TexGeneric L d)
...
set_option linter.style.longLine false in
/-- Step 1 endpoint from local path/open-set IDL data using the split base-tail
zero-free graph frontier. -/
theorem firstLayerEndpoint_of_texGenericStep_of_IDLData_probeCoordPreimage_splitBaseTailZeroFreeConcrete
    {L d r : Nat} (hr : 2 <= r)
    {θ θ' : Params (L + 1) d}
    (hstep :
      TexGenericStepClauses L d (TexGeneric L d)
    ...
    FirstLayerEndpointData
      (Params.headAttention θ) (Params.headAttention θ')
      (Params.headValue θ) :=
  firstLayerEndpoint_of_texGenericStep_of_IDLData_probeCoordPreimage_baseTailZeroFreeConcrete
    hr hstep D regularPolePreimage
    (step1OStarProjectionGraphBaseTailZeroFreeConcreteObligations_of_splitBaseTail
      concretePolynomials)

set_option linter.style.longLine false in
/-- Global-probe Step 1 endpoint using the split base-tail zero-free graph frontier. -/
theorem firstLayerEndpoint_of_texGenericStep_probeCoordPreimage_splitBaseTailZeroFreeConcrete
    {L d r : Nat} (hr : 2 <= r)
    {θ θ' : Params (L + 1) d}
    (hstep :
      TexGenericStepClauses L d (TexGeneric L d)
    ...
    FirstLayerEndpointData
      (Params.headAttention θ) (Params.headAttention θ')
      (Params.headValue θ) :=
  firstLayerEndpoint_of_texGenericStep_probeCoordPreimage_baseTailZeroFreeConcrete
    hr hstep hagree regularPolePreimage
    (step1OStarProjectionGraphBaseTailZeroFreeConcreteObligations_of_splitBaseTail
      concretePolynomials)

set_option linter.style.longLine false in
/-- Current-constructor spelling of the local IDL endpoint at depth `L + 2` using the
bundled base-tail zero-free graph frontier. -/
theorem firstLayerEndpoint_of_texGenericStep_of_IDLData_current_probeCoordPreimage_baseTailZeroFreeConcrete
    {L d r : Nat} (hr : 2 <= r)
    {θ θ' : Params (L + 2) d}
    (hstep :
    ...
    (regularPolePreimage :
      Step1OStarProbeCoordRegularPolePreimageData (L + 2) d r θ' D.O)
    (concretePolynomials :
      Step1OStarProjectionGraphBaseTailZeroFreeConcreteObligations
        (L + 2) d r θ' D.O) :
    FirstLayerEndpointData
      ...
      (Params.headValue θ) :=
  firstLayerEndpoint_of_texGenericStep_of_IDLData_probeCoordPreimage_baseTailZeroFreeConcrete
    (L := L + 1) hr hstep D regularPolePreimage concretePolynomials

set_option linter.style.longLine false in
/-- Current-constructor spelling of the local IDL endpoint at depth `L + 2` using the
split base-tail zero-free graph frontier. -/
theorem firstLayerEndpoint_of_texGenericStep_of_IDLData_current_probeCoordPreimage_splitBaseTailZeroFreeConcrete
    {L d r : Nat} (hr : 2 <= r)
    {θ θ' : Params (L + 2) d}
    (hstep :
      TexGenericStepClauses (L + 1) d (TexGeneric (L + 1) d)
    ...
    FirstLayerEndpointData
      (Params.headAttention θ) (Params.headAttention θ')
      (Params.headValue θ) :=
  firstLayerEndpoint_of_texGenericStep_of_IDLData_probeCoordPreimage_splitBaseTailZeroFreeConcrete
    (L := L + 1) hr hstep D regularPolePreimage concretePolynomials

set_option linter.style.longLine false in
/-- Current local IDL endpoint with the boundary no-pole source stated directly on the
raw probe/coordinate graph. -/
theorem firstLayerEndpoint_of_IDLData_current_probeCoordPreimage_splitBaseTail_graphNoPole
    {L d r : Nat} (hr : 2 <= r)
    {θ θ' : Params (L + 2) d}
    (hstep :
      TexGenericStepClauses (L + 1) d (TexGeneric (L + 1) d)
    ...
    FirstLayerEndpointData
      (Params.headAttention θ) (Params.headAttention θ')
      (Params.headValue θ) :=
  firstLayerEndpoint_of_texGenericStep_of_IDLData_current_probeCoordPreimage_splitBaseTailZeroFreeConcrete
    hr hstep D regularPolePreimage
    (step1SplitBaseTailGraphObligations_of_graphNoPole
      baseTailZeroFree lastBoundaryNoPole)

set_option linter.style.longLine false in
/-- Local IDL endpoint from family-level regular-pole preimage, split base-tail
zero-freeness, and raw graph no-pole. -/
theorem firstLayerEndpoint_of_texGenericStep_of_IDLData_regularPolePreimage_splitBaseTail_graphNoPole
    {L d r : Nat} (hr : 2 <= r)
    {θ θ' : Params (L + 1) d}
    (hstep :
      TexGenericStepClauses L d (TexGeneric L d)
    ...
    (regularPolePreimage :
      Step1OStarRegularPolePreimageFamilyData
        ...
        (θ := θ) (θ' := θ') hr)
    (baseTailZeroFree :
      Step1OStarProjectionSplitBaseTailZeroFreeWitnessData (L + 1) d θ' D.O)
    (lastBoundaryNoPole :
      Step1LastBoundaryFormalPhiNoPoleOnOStarProbeCoordGraphData
        (L := L + 1) (d := d) (O := D.O) (r := r) θ') :
    FirstLayerEndpointData
      (Params.headAttention θ) (Params.headAttention θ')
      (Params.headValue θ) := by
  let hStanding : Step1StandingAssumptions r (L + 1) d D.O θ θ' :=
    { depth_pos := Nat.succ_pos L
      generic := hstep.toOStarGenericAssumptions D.O_open D.O_nonempty
      agreement := localProbeTailAgreement_of_IDLData D }
  exact
    firstLayerEndpoint_of_texGenericStep_of_IDLData_probeCoordPreimage_splitBaseTailZeroFreeConcrete
      hr hstep D
      (step1OStarProbeCoordRegularPolePreimageData_of_regularPolePreimageFamilyData
        regularPolePreimage hStanding)
      (step1SplitBaseTailGraphObligations_of_graphNoPole
        baseTailZeroFree lastBoundaryNoPole)

set_option linter.style.longLine false in
/-- Local IDL endpoint from family-level regular-pole preimage, split base-tail
zero-freeness, and the existing fixed-probe formal-phi no-pole package.

This is the formal-phi source variant of
`firstLayerEndpoint_of_texGenericStep_of_IDLData_regularPolePreimage_splitBaseTail_graphNoPole`;
the boundary package is transported to the active raw probe/coordinate graph before
provider assembly. -/
theorem firstLayerEndpoint_of_texGenericStep_of_IDLData_regularPolePreimage_splitBaseTail_formalPhiNoPole
    {L d r : Nat} (hr : 2 <= r)
    {θ θ' : Params (L + 1) d}
    (hstep :
      TexGenericStepClauses L d (TexGeneric L d)
  ...
  exact
    firstLayerEndpoint_of_texGenericStep_of_IDLData_regularPolePreimage_splitBaseTail_graphNoPole
      hr hstep D regularPolePreimage baseTailZeroFree
      (step1LastBoundaryFormalPhiNoPoleOnOStarProbeCoordGraphData_of_lastBoundaryFormalPhiNoPole
        (lastBoundaryFormalPhiNoPole hStanding))

set_option linter.style.longLine false in
/-- Current local IDL endpoint from family-level regular-pole preimage, split base-tail
zero-freeness, and raw graph no-pole. -/
theorem firstLayerEndpoint_of_IDLData_current_regularPolePreimage_splitBaseTail_graphNoPole
    {L d r : Nat} (hr : 2 <= r)
    {θ θ' : Params (L + 2) d}
    (hstep :
      TexGenericStepClauses (L + 1) d (TexGeneric (L + 1) d)
    ...
    FirstLayerEndpointData
      ...
      (Params.headValue θ) :=
  firstLayerEndpoint_of_texGenericStep_of_IDLData_regularPolePreimage_splitBaseTail_graphNoPole
    (L := L + 1) hr hstep D regularPolePreimage
    baseTailZeroFree lastBoundaryNoPole

set_option linter.style.longLine false in
/-- Current local IDL endpoint from family-level regular-pole preimage, split base-tail
zero-freeness, and the existing fixed-probe formal-phi no-pole package. -/
theorem firstLayerEndpoint_of_IDLData_current_regularPolePreimage_splitBaseTail_formalPhiNoPole
    {L d r : Nat} (hr : 2 <= r)
    {θ θ' : Params (L + 2) d}
    (hstep :
      TexGenericStepClauses (L + 1) d (TexGeneric (L + 1) d)
    ...
    FirstLayerEndpointData
      (Params.headAttention θ) (Params.headAttention θ')
      (Params.headValue θ) :=
  firstLayerEndpoint_of_texGenericStep_of_IDLData_regularPolePreimage_splitBaseTail_formalPhiNoPole
    (L := L + 1) hr hstep D regularPolePreimage
    baseTailZeroFree lastBoundaryFormalPhiNoPole

set_option linter.style.longLine false in
/-- Current local IDL endpoint with the active raw graph no-pole field compiled from
the existing fixed-probe last-boundary formal-phi no-pole package. -/
theorem firstLayerEndpoint_of_IDLData_current_probeCoordPreimage_splitBaseTail_formalPhiNoPole
    {L d r : Nat} (hr : 2 <= r)
    {θ θ' : Params (L + 2) d}
    (hstep :
      TexGenericStepClauses (L + 1) d (TexGeneric (L + 1) d)
  ...
  exact
    firstLayerEndpoint_of_texGenericStep_of_IDLData_current_probeCoordPreimage_splitBaseTailZeroFreeConcrete
      hr hstep D regularPolePreimage
      (step1SplitBaseTailGraphObligations_of_formalPhiNoPole
        (hStanding := hStanding) baseTailZeroFree
        (lastBoundaryFormalPhiNoPole hStanding))
...
/-- **Global-probe Step 1 endpoint, claimB/claimC-free.**  Global-product (`A_gp`)
analogue of `firstLayerEndpoint_of_texGenericStep`, specialized to global probe agreement
at the top-level wrapper.  No Step 1 analytic side inputs. -/
theorem firstLayerEndpoint_of_texGenericStep_globalProduct
    {L d r : Nat} (hr : 2 <= r)
    {θ θ' : Params (L + 1) d}
    (hstep :
      TexGenericStepClauses L d (TexGeneric L d)
