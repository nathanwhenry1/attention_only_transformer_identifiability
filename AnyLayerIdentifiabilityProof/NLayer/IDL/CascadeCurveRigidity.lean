...
theorem formalFactor_complexGateAssignmentOfTail_zero {d : Nat}
    (θ : LayerStream d) (tail : Nat → ℝ) (z : ℂ) :
    formalFactor θ (complexGateAssignmentOfTail z tail) 0 =
      matC (skipB (θ 0).1) - z • matC (θ 0).1 := by
...
/-- Product-patch vector separation for the zero branch.

This is the `lem:quadA`-level handoff used before calling
`cascadeCurveRigidityProvider_of_product_patch_formalXY_on_J`: if the residual
`formalX - γ(t) A` is represented by one real matrix `M`, the symmetric `formalY`
residual is represented by real matrices `R t`, and the vector residual
`M v + R(t) w` vanishes on the coefficient-separating product patch, then the two
matrix identities required by the curve-rigidity constructor hold on `patch.J`. -/
theorem product_patch_formalXY_on_J_of_real_vector_residual {L d : Nat}
    {θ : Params L d} {A : Matrix (Fin d) (Fin d) ℝ} {level : Nat} {tail : Nat → ℝ}
    {U0 : Set (ProbePair d × ℝ)}
    (patch : CoefficientSeparatingProductPatch A U0)
    (gamma0 gamma1 : ℂ)
    ...
    (∀ t : ℝ, t ∈ patch.J →
      ...
      (∀ t : ℝ, t ∈ patch.J →
        formalY (paramStream θ) level (gateAssignmentOfTail t tail)
          + (formalY (paramStream θ) level (gateAssignmentOfTail t tail))ᵀ = 0) := by
  rcases patch.coefficient_separation M R hvanish with ⟨hM_mulVec, hR⟩
  have hM : M = 0 := by
    ext i j
    have hij := congrFun (hM_mulVec (Pi.single j (1 : ℝ))) i
  ...
  · intro t ht
    ...
    have hx0 :
        formalX (paramStream θ) level (gateAssignmentOfTail t tail)
            - (gamma0 + gamma1 * (t : ℂ)) • matC A = 0 := by
      simpa [hM, matC] using hx
    exact sub_eq_zero.mp hx0
  · intro t ht
    have hy := hYres t ht
    simpa [hR t ht, matC] using hy

/-- Repackage affine curve-rigidity data as provider data once the `formalX`/`formalY`
matrix identities have been extracted on an infinite real slice. -/
noncomputable def cascadeCurveRigidityProvider_of_data_formalXY_on_infinite {L d : Nat}
    {θ : Params L d} {A : Matrix (Fin d) (Fin d) ℝ} {level : Nat} {tail : Nat → ℝ}
      ...
      ∀ t : ℝ, t ∈ slice →
        formalX (paramStream θ) level (gateAssignmentOfTail t tail) =
          ...
          ∧ formalY (paramStream θ) level (gateAssignmentOfTail t tail)
              + (formalY (paramStream θ) level (gateAssignmentOfTail t tail))ᵀ = 0) :
    CascadeCurveRigidityProvider θ A level tail := by
  let gamma0 : ℂ := Classical.choose D.gamma_affine
  let gamma1 : ℂ := Classical.choose (Classical.choose_spec D.gamma_affine)
  have hgamma : ∀ z : ℂ, D.gamma z = gamma0 + gamma1 * z :=
    Classical.choose_spec (Classical.choose_spec D.gamma_affine)
  exact cascadeCurveRigidityProvider_of_formalXY_on_infinite gamma0 gamma1 slice hslice
    (fun t ht => by
      have hX := (hXY t ht).1
      rwa [hgamma (t : ℂ)] at hX)
    (fun t ht => (hXY t ht).2)

/-- Direct constructor from the post-`lem:quadA` slice identities on an infinite real set.

This is the intended handoff point for the product-patch/topology argument: once per-slice
rigidity and the trace-formula affine scalar have supplied the two displayed identities
on `slice`, this constructor performs the polynomial upgrade and packages the resulting
curve identity. -/
noncomputable def cascadeCurveRigidityData_of_formalXY_on_infinite {L d : Nat}
    {θ : Params L d} {A : Matrix (Fin d) (Fin d) ℝ} {level : Nat} {tail : Nat → ℝ}
    (gamma0 gamma1 : ℂ) (slice : Set ℝ) (hslice : slice.Infinite)
    (hX :
      ∀ t : ℝ, t ∈ slice →
    ...
    (hY :
      ∀ t : ℝ, t ∈ slice →
        formalY (paramStream θ) level (gateAssignmentOfTail t tail)
          + (formalY (paramStream θ) level (gateAssignmentOfTail t tail))ᵀ = 0) :
    CascadeCurveRigidityData θ A level tail :=
  cascadeCurveRigidityData_of_provider
    (cascadeCurveRigidityProvider_of_formalXY_on_infinite gamma0 gamma1 slice hslice hX hY)

/-- Product-patch constructor for the zero branch, in `CascadeCurveRigidityData` form.

This is the product-neighborhood handoff: once `lem:quadA` supplies the two per-slice
matrix identities on the patch interval `J`, this theorem upgrades them to the global
curve-rigidity data. -/
noncomputable def cascadeCurveRigidityData_of_product_patch_formalXY_on_J {L d : Nat}
    {θ : Params L d} {A : Matrix (Fin d) (Fin d) ℝ} {level : Nat} {tail : Nat → ℝ}
    {U0 : Set (ProbePair d × ℝ)}
    (patch : CoefficientSeparatingProductPatch A U0)
    (gamma0 gamma1 : ℂ)
    ...
    CascadeCurveRigidityData θ A level tail :=
  cascadeCurveRigidityData_of_provider
    (cascadeCurveRigidityProvider_of_product_patch_formalXY_on_J patch gamma0 gamma1 hX hY)
