...
/-- Localize a point with nonzero specialized slope to a connected sign component and
package it as a canonical nonzero branch source once `Ustar` is selected inside that
component. -/
noncomputable def
  ...
  · refine ⟨⟨1, Or.inr rfl⟩, ?_⟩
    exact
      cascadeLevelBranchSource_nonzero_of_canonicalFrecGateAlongSignRegion
        signRegion hAA A Ustar U0 hU0 level tail hlevel_pos hlevel_lt
        prior_saturates provider U1 hU1_subset_U0 hUstar_subset_U1 hU1_nonempty
        hU1_rel_open hU1_preconnected slope_nonzero 1 (Or.inr rfl)
        (by
          intro x hxU1
          have hpos :
...
/-- Assemble provider-facing cascade induction data from canonical branch sources on
a selected sign-region component. -/
noncomputable def cascadeTrichotomyInductionProviderData_of_canonicalBranchSources
    {L d r : Nat} {θ θ' : Params (L + 1) d} {O : Set (ProbePair d)}
    (signRegion :
      SignRegionData (L := L + 1) (d := d) θ' O
        (Params.headAttention θ') (Real.log (r : ℝ)))
    ...
    (hUstar : Ustar ⊆ signRegion.U)
    (hne : Ustar.Nonempty)
    (hrel : ∃ W : Set (ProbePair d × ℝ), IsOpen W ∧ Ustar = W ∩ signRegion.U)
    (hconn : IsPreconnected Ustar)
    (varsigma : Nat → ℝ)
    (branches :
      ∀ n : Nat, 1 ≤ n → n < L + 1 →
        CascadeLevelBranchSource (Real.log (r : ℝ)) θ A Ustar
          (canonicalFrecGateAlongSignRegion r signRegion θ) n (varsigma n)) :
    CascadeTrichotomyInductionProviderData
      (L := L + 1) (d := d) (Real.log (r : ℝ)) θ A signRegion.U
      (canonicalFrecGateAlongSignRegion r signRegion θ)
      (canonicalFrecGateAlongSignRegion r signRegion θ') where
  Ustar := Ustar
  subset_sign_region := hUstar
  nonempty := hne
  relatively_open_in_sign_region := hrel
  connected := hconn
  varsigma := varsigma
  unprimed_branch := branches
  primed_saturates_one := by
    intro x hx n hn_pos hn_lt
    exact
      canonicalFrecGateAlongSignRegion_primed_saturates_one signRegion x
        (hUstar hx) n hn_pos hn_lt
...
/-- Builder-backed closed-recursion/R4 obligation from all-dial actual gate data,
restricted internally to the canonical regular-quadric matching dials. -/
theorem texMatchingRegularQuadricClosedRecursionLimitObligation_of_cascadeBuilder_actualGateData
    {L d r : Nat}
    {θ θ' θB : Params (L + 1) d}
    {O : Set (ProbePair d)}
    (signRegion :
    ...
    (hAA : Params.headAttention θ = Params.headAttention θ')
    (actual :
      TexTrichotomyMatchingActualGateData
        (θ := θ) (θ' := θ')
        (texTrichotomyConstructionData_of_cascadeBuilder B)) :
    TexMatchingRegularQuadricClosedRecursionLimitObligation
      signRegion (texTrichotomyConstructionData_of_cascadeBuilder B) N S :=
  texMatchingRegularQuadricClosedRecursionLimitObligation_of_cascadeBuilder
    signRegion B N S hAA
    (texTrichotomyMatchingCanonicalActualGateData_of_actualGateData
      signRegion (texTrichotomyConstructionData_of_cascadeBuilder B) N actual)

/-- Provider-facing closed-recursion/R4 obligation from all-dial actual gate data,
restricted internally to the canonical regular-quadric matching dials. -/
theorem texMatchingRegularQuadricClosedRecursionLimitObligation_of_inductionProvider_actualGateData
    {L d r : Nat}
    {θ θ' θB : Params (L + 1) d}
    {O : Set (ProbePair d)}
    (signRegion :
    ...
    (actual :
      TexTrichotomyMatchingActualGateData
        (θ := θ) (θ' := θ')
        (texTrichotomyConstructionData_of_inductionProvider P)) :
    TexMatchingRegularQuadricClosedRecursionLimitObligation
      signRegion (texTrichotomyConstructionData_of_inductionProvider P) N S :=
  texMatchingRegularQuadricClosedRecursionLimitObligation_of_inductionProvider
    signRegion P N S hAA
    (texTrichotomyMatchingCanonicalActualGateData_of_actualGateData
      signRegion (texTrichotomyConstructionData_of_inductionProvider P) N actual)
