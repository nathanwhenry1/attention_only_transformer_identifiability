...
/-- The concrete anchor point has an open safe neighborhood under the same continuity
hypotheses. -/
theorem texRegionSafeSet_mem_nhds_anchorPoint_of_continuous
    {L d : Nat} {θ' : Params (L + 1) d} {O : Set (ProbePair d)}
    (hO_open : IsOpen O)
    (anchor : TexRegionAnchorPointData θ' O)
    (hcont :
      ∀ n : Nat, 1 ≤ n -> n < L + 1 ->
        Continuous fun x : ProbePair d × ℝ =>
          (specializedPhi θ' n (gateAssignmentOneTail x.2) x.1).re) :
    texRegionSafeSet θ' O ∈ nhds (anchor.point, anchor.t) :=
  (isOpen_texRegionSafeSet_of_continuous hO_open hcont).mem_nhds
...
theorem quadricProbeSliceQuadraticCoefficientSeparation_firstLayerQuadricProbeSet
    {d : Nat} {A : Matrix (Fin d) (Fin d) ℝ}
    (hd : 2 ≤ d) (hdet : A.det ≠ 0) :
    QuadricProbeSliceQuadraticCoefficientSeparation A
      (firstLayerQuadricProbeSet A) := by
  simpa [firstLayerQuadricProbeSet] using
    quadricProbeSliceQuadraticCoefficientSeparation_firstLayerQuadric
      (A := A) hd hdet
...
/-- A deliberately strong scalar richness condition on a quadric probe slice.

It is stronger than the local regular-quadric geometry should ultimately need, but it
is a small correct replacement for the false inference from mere nonempty relative
openness: testing all probes on the two coordinate axes separates the `v` and `w`
linear coefficients. -/
def QuadricProbeSliceAxisRich {d : Nat} (Uq : Set (ProbePair d)) : Prop :=
  (∀ w : Fin d -> ℝ, ((w, 0) : ProbePair d) ∈ Uq) ∧
    (∀ v : Fin d -> ℝ, (((0 : Fin d -> ℝ), v) : ProbePair d) ∈ Uq)

/-- Axis-rich probe slices satisfy the scalar row coefficient-separation theorem. -/
theorem quadricProbeSliceLinearCoefficientSeparation_of_axisRich
    {d : Nat} {A : Matrix (Fin d) (Fin d) ℝ} {Uq : Set (ProbePair d)}
    (hrich : QuadricProbeSliceAxisRich Uq) :
    QuadricProbeSliceLinearCoefficientSeparation A Uq := by
  intro m r hvanish
  constructor
  · ext i
    have h :=
      hvanish (((0 : Fin d -> ℝ), Pi.single i (1 : ℝ)) : ProbePair d)
        (hrich.2 (Pi.single i (1 : ℝ)))
    have hmi : dotProduct m (Pi.single i (1 : ℝ)) = 0 := by
      simpa using h
    rw [dotProduct_comm, single_dotProduct, one_mul] at hmi
    exact hmi
  · ext i
    have h :=
      hvanish ((Pi.single i (1 : ℝ), (0 : Fin d -> ℝ)) : ProbePair d)
        (hrich.1 (Pi.single i (1 : ℝ)))
    have hri : dotProduct r (Pi.single i (1 : ℝ)) = 0 := by
      simpa using h
    rw [dotProduct_comm, single_dotProduct, one_mul] at hri
    exact hri
...
/-- The whole first-layer quadric contains both coordinate axes in probe space. -/
theorem quadricProbeSliceAxisRich_firstLayerQuadricProbeSet
    {d : Nat} (A : Matrix (Fin d) (Fin d) ℝ) :
    QuadricProbeSliceAxisRich (firstLayerQuadricProbeSet A) := by
  constructor
  · intro w
    simp [firstLayerQuadricProbeSet, firstLayerQuadric, matrixBilin]
  · intro v
    simp [firstLayerQuadricProbeSet, firstLayerQuadric, matrixBilin]

/-- On the full first-layer quadric, scalar linear coefficient separation is immediate
from the two coordinate axes. -/
theorem quadricProbeSliceNoLinearComponent_firstLayerQuadricProbeSet
    {d : Nat} (A : Matrix (Fin d) (Fin d) ℝ) :
    QuadricProbeSliceNoLinearComponent A (firstLayerQuadricProbeSet A) :=
  quadricProbeSliceLinearCoefficientSeparation_of_axisRich
    (A := A) (quadricProbeSliceAxisRich_firstLayerQuadricProbeSet A)
...
/-- For a nonsingular matrix, every nonzero right vector has a nonzero `A * v`
coordinate. -/
theorem exists_mulVec_ne_zero_coord_of_ne_zero
    {d : Nat} (A : Matrix (Fin d) (Fin d) ℝ) (hdet : A.det ≠ 0)
    {v : Fin d -> ℝ} (hv : v ≠ 0) :
    ∃ i : Fin d, (Matrix.mulVec A v) i ≠ 0 := by
  classical
  by_contra h
  apply hv
  apply matrix_mulVec_eq_zero_of_det_ne_zero A hdet
  ext i
...
/-- Correct reduced scalar identity with the missing slice-richness hypothesis made
explicit. -/
theorem regularQuadricProbeSliceScalarIdentity_of_axisRich
    {d : Nat} {A : Matrix (Fin d) (Fin d) ℝ}
    {P : Set (ProbePair d)} {m r : Fin d -> ℝ}
    (hrich : QuadricProbeSliceAxisRich (P ∩ firstLayerQuadricProbeSet A))
    (hvanish :
      ∀ p : ProbePair d, p ∈ P -> firstLayerQuadric A p ->
        dotProduct m p.2 + dotProduct r p.1 = 0) :
    m = 0 ∧ r = 0 := by
  exact
    (quadricProbeSliceLinearCoefficientSeparation_of_axisRich
...
/-- The per-slice no-linear-component hypothesis is exactly scalar linear coefficient
separation on that slice. -/
theorem quadricProbeSliceLinearCoefficientSeparation_of_noLinearComponent
    {d : Nat} {A : Matrix (Fin d) (Fin d) ℝ} {Uq : Set (ProbePair d)}
    (hnoLinear : QuadricProbeSliceNoLinearComponent A Uq) :
    QuadricProbeSliceLinearCoefficientSeparation A Uq :=
  hnoLinear
...
/-- A locally connected quadric subtype supplies the point-local subtype basis. -/
theorem of_locallyConnectedSpace
    (hlocal : LocallyConnectedSpace {p : ProbePair d // firstLayerQuadric A p}) :
    RegularQuadricLocalConnectedQuadricSubtypeBasisData A point := by
  intro hquad hregular S hS_open hpointS
  have _ := hregular
  let qpoint : {p : ProbePair d // firstLayerQuadric A p} := ⟨point, hquad⟩
  rcases (locallyConnectedSpace_iff_subsets_isOpen_isConnected.mp hlocal)
      qpoint S (hS_open.mem_nhds hpointS) with
    ⟨S0, hS0_subset, hS0_open, hqpointS0, hS0_connected⟩
  exact ⟨S0, hS0_open, hqpointS0, hS0_subset, hS0_connected.isPreconnected⟩
...
/-- Global local connectedness of the quadric subtype is a sufficient topology
primitive for the ambient probe-slice basis. -/
def of_locallyConnectedQuadricSubtype
    (hlocal : LocallyConnectedSpace {p : ProbePair d // firstLayerQuadric A p}) :
    RegularQuadricLocalConnectedProbeSliceBasisData A point :=
  of_quadricSubtypeBasis
    (RegularQuadricLocalConnectedQuadricSubtypeBasisData.of_locallyConnectedSpace
      (A := A) (point := point) hlocal)
...
source constructor. -/
noncomputable def
    exists_cascadeLevelBranchSource_nonzero_of_canonicalFrecGateAlongSignRegion_point_from_basis
    ...
    Σ U1 : Set (ProbePair d × ℝ),
      Ustar ⊆ U1 ->
        Σ selected : {selected : ℝ // selected = 0 ∨ selected = 1},
          CascadeLevelBranchSource (Real.log (r : ℝ)) θ A Ustar
            (canonicalFrecGateAlongSignRegion r signRegion θ) level selected.1 := by
  ...
  exact
    exists_cascadeLevelBranchSource_nonzero_of_canonicalFrecGateAlongSignRegion_point
      signRegion hAA A Ustar U0 hU0 x0 level tail hlevel_pos hlevel_lt
...
/-- Compile the provider package to the full coefficient-separating open-neighborhood
chart wrapper. -/
noncomputable def toOpenNeighborhoodChartData
    {t : ℝ} (G : RegularQuadricReducedChartCoefficientData A point)
    (hd : 2 ≤ d) (hdet : A.det ≠ 0) :
    RegularQuadricLocalConnectedCoefficientSeparatingOpenNeighborhoodChartData
      A point t :=
  RegularQuadricLocalConnectedCoefficientSeparatingOpenNeighborhoodChartData.of_reduced
    G.quadricSubtypeBasisData G.noLinearComponentData hd hdet
...
/-- IDL-region local obligation from the reduced pure inputs: a connected chart core
plus coefficient separation on relatively open quadric probe slices. -/
noncomputable def texRegionConstructionDataOfIDLDataObligation_of_connectedChartCore
    {L d r : Nat} {θ θ' : Params (L + 1) d}
    (hL : 0 < L) (hd : 2 ≤ d)
    ...
    (hcont :
      ∀ n : Nat, 1 ≤ n -> n < L + 1 ->
        Continuous fun x : ProbePair d × ℝ =>
          (specializedPhi θ' n (gateAssignmentOneTail x.2) x.1).re)
    (core :
      RegularQuadricLocalConnectedOpenNeighborhoodChartCoreData
        (Params.headAttention θ')
        (texRegionAnchorPointData_of_IDLData hL D).point
        (texRegionAnchorPointData_of_IDLData hL D).t)
    (sep :
  ...
  texRegionConstructionDataOfIDLDataObligation_of_openNeighborhoodChart
    hL D hcont
    (RegularQuadricLocalConnectedCoefficientSeparatingOpenNeighborhoodChartData.of_core
      core sep hd hdet)

/-- IDL-region local obligation from the minimized pure inputs: a relative-open
connected quadric-slice basis plus scalar linear separation on relatively open quadric
probe slices.  The wrappers above discharge the product-patch topology and the rowwise
matrix bookkeeping. -/
noncomputable def texRegionConstructionDataOfIDLDataObligation_of_relativeOpenSliceBasis
    {L d r : Nat} {θ θ' : Params (L + 1) d}
    (hL : 0 < L) (hd : 2 ≤ d)
    (hdet : (Params.headAttention θ').det ≠ 0)
    (D : IDLData (L + 1) d r θ θ')
    (hcont :
      ∀ n : Nat, 1 ≤ n -> n < L + 1 ->
        Continuous fun x : ProbePair d × ℝ =>
          (specializedPhi θ' n (gateAssignmentOneTail x.2) x.1).re)
    (chartBasis :
      RegularQuadricLocalConnectedRelativeOpenSliceBasisData
        (Params.headAttention θ')
        (texRegionAnchorPointData_of_IDLData hL D).point
        (texRegionAnchorPointData_of_IDLData hL D).t)
    (linearSep :
      RegularQuadricProbeSliceLinearSeparationData
        (Params.headAttention θ')) :
    TexRegionConstructionDataOfIDLDataObligation hL D :=
  texRegionConstructionDataOfIDLDataObligation_of_connectedChartCore
    hL hd hdet D hcont
    chartBasis.toOpenNeighborhoodChartCoreData
    (RegularQuadricProbeSliceCoefficientSeparationData.of_linearSeparation linearSep)
...
/-- Package the affine coefficient identities as the `MatchingLimitData` consumed by the
formal `K = I` extraction. -/
noncomputable def toMatchingLimitData (C : MatchingCoefficientComparisonData θ θ') :
    MatchingLimitData θ θ' where
  skipProduct_eq := C.skipProduct_eq
  K := realSkipBprod (paramStream (Params.tail θ)) L - C.D
  K_maps_headValue := C.K_maps_headValue
  K_maps_headSkip := C.K_maps_headSkip
...
/-- Top-level specialization of the dial-path bridge when `D.Paths = Set.univ`.
The caller supplies the actual dial paths and the analytic limit theorem for
observable agreement along those paths; membership in the path class is automatic. -/
noncomputable def texMatchingLimitVectorAgreementData_of_univDialPathLimit
    {L d r : Nat}
    {θ θ' : Params (L + 1) d}
    (D : IDLData (L + 1) d r θ θ')
    (hPaths : D.Paths = Set.univ)
    ...
    (dial :
      ∀ p : ProbePair d, p ∈ N.Uq -> ∀ t : ℝ, t ∈ N.J ->
        DialPathData (Params.headAttention θ') (Real.log (r : ℝ)))
    (dial_base : ∀ p hp t ht, (dial p hp t ht).base = p)
    (dial_target : ∀ p hp t ht, (dial p hp t ht).t = t)
    (hlimit :
      ∀ p hp t ht, ∀ T0 : ℝ,
        ObservableAgreementOnPath r θ θ'
          (DialPathData.probe (dial p hp t ht)) T0 ->
    ...
    TexMatchingLimitVectorAgreementData θ θ' N S := by
  exact
    texMatchingLimitVectorAgreementData_of_dialPathLimitBridge D N S
      { dial := dial
        dial_base := dial_base
        dial_target := dial_target
        path_mem := by
          intro p hp t ht
          exact texMatching_dialPath_mem_of_paths_univ D hPaths (dial p hp t ht)
        limit_of_observable := hlimit }

/-- Canonical bridge from product-neighborhood points to IDL dial paths when the path
class is universal.  This discharges the exact dial construction and path membership;
the remaining input is only the analytic limit theorem along these canonical dials. -/
noncomputable def texMatchingDialPathLimitBridgeData_of_univPaths
    {L d r : Nat}
    {θ θ' : Params (L + 1) d}
    (D : IDLData (L + 1) d r θ θ')
    (hPaths : D.Paths = Set.univ)
    ...
    (hlimit :
      ∀ p hp t ht, ∀ T0 : ℝ,
        ObservableAgreementOnPath r θ θ'
          (DialPathData.probe
            (texMatchingDialPathData_of_product
  ...
  exact
    { dial := fun p hp t ht =>
      ...
      dial_base := by
        intro p hp t ht
        simp [texMatchingDialPathData_of_product]
      dial_target := by
        intro p hp t ht
        simp [texMatchingDialPathData_of_product]
      path_mem := by
        intro p hp t ht
        exact texMatching_dialPath_mem_of_paths_univ D hPaths
          (texMatchingDialPathData_of_product
            (L := L) (d := d) (r := r) signRegion T N p hp t ht)
      limit_of_observable := hlimit }

/-- Product-neighborhood bases are in the IDL open probe region. -/
theorem texMatching_base_mem_openRegion_of_product
    {L d : Nat}
    {θ' : Params (L + 1) d} {O : Set (ProbePair d)}
    {b : ℝ}
    (signRegion :
    ...
    p ∈ O :=
  signRegion.point_mem_base (p, t)
    (texMatching_product_mem_signRegion signRegion T N hp ht)

/-- The currently exposed IDL interface gives observable agreement for the constant
path through every product-neighborhood base point. -/
theorem texMatching_constantPathAgreement_of_product
    {L d r : Nat} {θ θ' : Params (L + 1) d}
    (D : IDLData (L + 1) d r θ θ')
    (signRegion :
      SignRegionData (L := L + 1) (d := d) θ' D.O
    ...
    (N :
      TexMatchingProductNeighborhoodData (d := d)
        (Params.headAttention θ') T.Ustar)
    {p : ProbePair d} (hp : p ∈ N.Uq) {t : ℝ} (ht : t ∈ N.J) :
    ∃ T0 : ℝ, ObservableAgreementOnPath r θ θ' (constantProbePath p) T0 := by
  exact D.observableAgreementOn_constantProbePath
    (texMatching_base_mem_openRegion_of_product signRegion T N hp ht)
...
/-- The direct algebra available from the current Step 4 hypotheses: on every selected
quadric probe, the `M` term is determined by any one `R t` term. -/
theorem texMatching_M_mulVec_eq_neg_R_mulVec_on_Uq
    {d : Nat}
    {M : Matrix (Fin d) (Fin d) ℝ}
    {R : ℝ -> Matrix (Fin d) (Fin d) ℝ}
    {Uq : Set (ProbePair d)} {J : Set ℝ}
    (hlimit :
      ∀ p : ProbePair d, p ∈ Uq -> ∀ t : ℝ, t ∈ J ->
        Matrix.mulVec M p.2 + Matrix.mulVec (R t) p.1 = 0) :
    ∀ p : ProbePair d, p ∈ Uq -> ∀ t : ℝ, t ∈ J ->
      Matrix.mulVec M p.2 = -Matrix.mulVec (R t) p.1 := by
  intro p hp t ht
  exact eq_neg_of_add_eq_zero_left (hlimit p hp t ht)

/-- The other direct algebra available from the current Step 4 hypotheses: all `R t`
have the same action on first coordinates of the selected quadric probes. -/
theorem texMatching_R_sub_mulVec_eq_zero_on_Uq
    {d : Nat}
    {M : Matrix (Fin d) (Fin d) ℝ}
    {R : ℝ -> Matrix (Fin d) (Fin d) ℝ}
    {Uq : Set (ProbePair d)} {J : Set ℝ}
    (hlimit :
      ∀ p : ProbePair d, p ∈ Uq -> ∀ t : ℝ, t ∈ J ->
        Matrix.mulVec M p.2 + Matrix.mulVec (R t) p.1 = 0) :
    ∀ p : ProbePair d, p ∈ Uq -> ∀ t0 : ℝ, t0 ∈ J ->
      ∀ t1 : ℝ, t1 ∈ J ->
        Matrix.mulVec (R t1 - R t0) p.1 = 0 := by
  intro p hp t0 ht0 t1 ht1
  have h0 :
      Matrix.mulVec M p.2 = -Matrix.mulVec (R t0) p.1 :=
    texMatching_M_mulVec_eq_neg_R_mulVec_on_Uq hlimit p hp t0 ht0
  have h1 :
      Matrix.mulVec M p.2 = -Matrix.mulVec (R t1) p.1 :=
    texMatching_M_mulVec_eq_neg_R_mulVec_on_Uq hlimit p hp t1 ht1
  have hR :
      Matrix.mulVec (R t1) p.1 = Matrix.mulVec (R t0) p.1 := by
    exact neg_inj.mp (h1.symm.trans h0)
  rw [Matrix.sub_mulVec]
  exact sub_eq_zero.mpr hR

/-- A deliberately strong richness condition on the selected quadric probe patch.

It contains both coordinate axes in probe space, which is enough to separate the two
matrix coefficients in the Step 4 identity.  Actual region construction should replace
this with the geometric rich-quadric patch supplied by the regular quadric chart. -/
def texMatchingAxisRichPatch {d : Nat} (Uq : Set (ProbePair d)) : Prop :=
  (∀ w : Fin d -> ℝ, ((w, 0) : ProbePair d) ∈ Uq) ∧
    (∀ v : Fin d -> ℝ, (((0 : Fin d -> ℝ), v) : ProbePair d) ∈ Uq)

/-- A rich probe patch rules out the singleton counterexample: testing on `(0, v)`
kills `M`, and testing on `(w, 0)` kills each `R t`. -/
theorem texMatching_coefficient_separation_of_axisRichPatch
    {d : Nat} {Uq : Set (ProbePair d)} {J : Set ℝ}
    (hJ_nonempty : J.Nonempty)
    (hrich : texMatchingAxisRichPatch Uq) :
    ∀ (M : Matrix (Fin d) (Fin d) ℝ)
      (R : ℝ -> Matrix (Fin d) (Fin d) ℝ),
      (∀ p : ProbePair d, p ∈ Uq -> ∀ t : ℝ, t ∈ J ->
        Matrix.mulVec M p.2 + Matrix.mulVec (R t) p.1 = 0) ->
  ...
  · rcases hJ_nonempty with ⟨t0, ht0⟩
    intro v
    have h := hlimit (((0 : Fin d -> ℝ), v) : ProbePair d) (hrich.2 v) t0 ht0
    simpa using h
  · intro t ht
    apply matrix_eq_zero_of_forall_mulVec_eq_zero
    intro w
    have h := hlimit ((w, 0) : ProbePair d) (hrich.1 w) t ht
    simpa using h

/-- A concrete singleton counterexample to the currently stated Step 4 conclusion.

Even in dimension two, the hypotheses allow `Uq` to contain only the probe
`(w, v) = (e0, 0)`.  The identity then imposes no condition on `M` away from `v = 0`,
so `M` need not vanish globally. -/
theorem texMatching_limit_difference_hypotheses_counterexample :
    ∃ (A M : Matrix (Fin 2) (Fin 2) ℝ)
      (R : ℝ -> Matrix (Fin 2) (Fin 2) ℝ)
      (Uq : Set (ProbePair 2)) (J : Set ℝ),
        Uq.Nonempty ∧ J.Nonempty ∧ IsOpen J ∧
          (∀ p ∈ Uq, firstLayerQuadric A p) ∧
          (∀ p : ProbePair 2, p ∈ Uq -> ∀ t : ℝ, t ∈ J ->
            Matrix.mulVec M p.2 + Matrix.mulVec (R t) p.1 = 0) ∧
          ¬ ((∀ v : Fin 2 -> ℝ, Matrix.mulVec M v = 0) ∧
            (∀ t : ℝ, t ∈ J -> R t = 0)) := by
  let e0 : Fin 2 := 0
  let oneVec : Fin 2 -> ℝ := Pi.single e0 1
  let zeroVec : Fin 2 -> ℝ := 0
  let p0 : ProbePair 2 := (oneVec, zeroVec)
  let A : Matrix (Fin 2) (Fin 2) ℝ := 1
  let M : Matrix (Fin 2) (Fin 2) ℝ := 1
  let R : ℝ -> Matrix (Fin 2) (Fin 2) ℝ := fun _ => 0
  let Uq : Set (ProbePair 2) := {p0}
  let J : Set ℝ := Set.univ
  refine ⟨A, M, R, Uq, J, ?_, ?_, ?_, ?_, ?_, ?_⟩
  · exact ⟨p0, by simp [Uq]⟩
  · exact ⟨0, by simp [J]⟩
  · simp [J]
  · intro p hp
    have hp0 : p = p0 := by simpa [Uq] using hp
    subst p
    simp [firstLayerQuadric, matrixBilin, Matrix.mulVec, dotProduct,
      A, p0, oneVec, zeroVec]
  · intro p hp t ht
    have hp0 : p = p0 := by simpa [Uq] using hp
    subst p
    ext i
    simp [M, R, p0, oneVec, zeroVec]
  · intro h
    have hM := h.1 oneVec
    have hcoord := congrFun hM e0
    simp [M, oneVec, e0] at hcoord
...
/-- Local Step 5 algebraic obligation for the affine constant coefficient.

This isolates the constant-term comparison in the affine-in-`t` identity obtained from
the dial-limit agreement on `Uq × J`. -/
theorem texMatching_affineConstantCoefficient_bilinear_eq_of_dialLimitAgreement
    {L d : Nat}
    {θ θ' : Params (L + 1) d}
    {Ustar : Set (ProbePair d × ℝ)}
    (N :
    ...
    ∀ p : ProbePair d, p ∈ N.Uq -> ∀ t : ℝ, t ∈ N.J ->
      matrixBilin
          ((realSkipBprod (paramStream (Params.tail θ)) L - S.D) *
            skipB (θ 0).1)
          p.1 p.2 =
        matrixBilin (skipB (θ' 0).1) p.1 p.2 := by
  have hskip :
      realSkipBprod (paramStream θ) (L + 1) =
        realSkipBprod (paramStream θ') (L + 1) :=
    texMatching_skipProduct_eq_of_skipProductDifferenceMatrix_eq_zero
...
/-- Step 5 affine `t`-coefficient comparison on the product neighborhood chosen in
Step 0.  This is an immediate bilinear consequence of the matrix coefficient identity
extracted from `E_t = 0` on `J`. -/
theorem texMatching_affineTCoefficient_bilinear_eq_on_productNeighborhood
    {L d r : Nat} (hr : 2 <= r)
    {θ θ' : Params (L + 1) d}
    (hstep :
      TexGenericStepClauses L d (TexGeneric L d)
...
/-- Step 5 affine `t`-coefficient comparison after Step 4 separation.  The matrix
coefficient identity gives the bilinear equality for all probes by rewriting. -/
theorem texMatching_affineTCoefficient_bilinear_eq
    {L d r : Nat} (hr : 2 <= r)
    {θ θ' : Params (L + 1) d}
    (hstep :
      TexGenericStepClauses L d (TexGeneric L d)
...
/-- Step 5 affine constant-coefficient comparison on the product neighborhood chosen in
Step 0, before the quadric/separation upgrade.  This is the local limit identity
obtained by comparing the constant terms of the TeX affine-in-`t` formula. -/
theorem texMatching_affineConstantCoefficient_bilinear_eq_on_productNeighborhood
    {L d r : Nat} (hr : 2 <= r)
    {θ θ' : Params (L + 1) d}
    (hstep :
      TexGenericStepClauses L d (TexGeneric L d)
  ...
  have limits : TexMatchingDialLimitAgreementData θ θ' N S :=
    texMatching_dialLimitAgreement_of_saturatedContribution
      ...
      limitAgreement
  exact
    texMatching_affineConstantCoefficient_bilinear_eq_of_dialLimitAgreement
      (L := L) (d := d) N S limits

/-- Step 5 affine constant-coefficient comparison after Step 4 separation.  The matrix
coefficient identity gives the bilinear equality for all probes by rewriting. -/
theorem texMatching_affineConstantCoefficient_bilinear_eq
    {L d r : Nat} (hr : 2 <= r)
    {θ θ' : Params (L + 1) d}
    (hstep :
      TexGenericStepClauses L d (TexGeneric L d)
    ...
    ∀ w v : Fin d -> ℝ,
      matrixBilin
          ((realSkipBprod (paramStream (Params.tail θ)) L - S.D) *
            skipB (θ 0).1)
          w v =
        matrixBilin (skipB (θ' 0).1) w v := by
  have limits : TexMatchingDialLimitAgreementData θ θ' N S :=
    texMatching_dialLimitAgreement_of_saturatedContribution
      (L := L) (d := d) (r := r) hr hstep endpoint D signRegion T N S
      limitAgreement
...
/-- Top-level matching-limit endpoint when all full-depth probe paths are available.
The remaining hypotheses are the geometric/trichotomy/product choices and the analytic
dial-path limit theorem. -/
noncomputable def matchingLimitData_of_IDLData_of_univDialPathLimit
    {L d r : Nat} (hr : 2 <= r)
    {θ θ' : Params (L + 1) d}
    (hstep :
      TexGenericStepClauses L d (TexGeneric L d)
  ...
  let limitAgreement :
      TexMatchingLimitVectorAgreementData θ θ' N S :=
    texMatchingLimitVectorAgreementData_of_univDialPathLimit D hPaths N S
      dial dial_base dial_target hlimit
  exact
    matchingLimitData_of_IDLData_of_saturatedLimitAgreement
      (L := L) (d := d) (r := r) hr hstep endpoint D signRegion T N S
      limitAgreement
...
/-- Exact remaining analytic limit obligation after the local-region patch is supplied
and `D.Paths = Set.univ`.  The product neighborhood and dial paths are the canonical
ones selected by the local-patch region/trichotomy constructors. -/
abbrev TexMatchingLocalPatchUnivLimitObligation
    {L d r : Nat} (hL : 0 < L) (hr : 2 <= r)
    {θ θ' : Params (L + 1) d}
    (hstep :
      TexGenericStepClauses L d (TexGeneric L d)
...
/-- Structure wrapper for the peeled unprimed effective-tail asymptotic. -/
structure TexMatchingUnprimedSaturatedEffectiveTailFrecAsymptoticObligation
    {L d r : Nat}
    {θ θ' : Params (L + 1) d}
    {O : Set (ProbePair d)}
    (signRegion :
...
/-- Peeling the first unprimed recursion layer reduces the full dial asymptotic to the
effective-tail asymptotic. -/
theorem texMatchingUnprimedSaturatedDialFrecTendsto_of_effectiveTail
    {L d r : Nat}
    {θ θ' : Params (L + 1) d}
    {signU : Set (ProbePair d × ℝ)}
    {T :
    ...
    (hunprimed :
      TexMatchingUnprimedSaturatedEffectiveTailFrecAsymptotic
        (θ := θ) (θ' := θ') T S) :
    TexMatchingUnprimedSaturatedDialFrecTendsto
      (θ := θ) (θ' := θ') T S := by
  intro δ hδ
  have hfun :
      (fun τ : ℝ =>
    ...
    exact Frec_succ_paramsFirstLayerEffectivePoint r θ
      (DialPathData.probe δ τ).1 (DialPathData.probe δ τ).2 τ
  rw [hfun]
  exact hunprimed δ hδ

/-- The full unprimed dial asymptotic also supplies the peeled effective-tail form. -/
theorem texMatchingUnprimedSaturatedEffectiveTailFrecAsymptotic_of_dialFrecTendsto
    {L d r : Nat}
    {θ θ' : Params (L + 1) d}
    {signU : Set (ProbePair d × ℝ)}
    {T :
...
/-- The exposed unprimed dial gap is exactly the peeled effective-tail gap. -/
theorem texMatchingUnprimedSaturatedDialFrecTendsto_iff_effectiveTail
    {L d r : Nat}
    {θ θ' : Params (L + 1) d}
    {signU : Set (ProbePair d × ℝ)}
    (T :
    ...
    TexMatchingUnprimedSaturatedDialFrecTendsto
      ...
      TexMatchingUnprimedSaturatedEffectiveTailFrecAsymptotic
        (θ := θ) (θ' := θ') T S := by
  exact
    ⟨texMatchingUnprimedSaturatedEffectiveTailFrecAsymptotic_of_dialFrecTendsto,
      texMatchingUnprimedSaturatedDialFrecTendsto_of_effectiveTail⟩

/-- Packaged version of the peeled unprimed effective-tail reduction. -/
theorem texMatchingUnprimedSaturatedFrecAsymptoticObligation_of_effectiveTail
    {L d r : Nat}
    {θ θ' : Params (L + 1) d}
    {O : Set (ProbePair d)}
    {signRegion :
    ...
    {S : TexMatchingSaturatedContributionData θ varsigma}
    (hunprimed :
      TexMatchingUnprimedSaturatedEffectiveTailFrecAsymptoticObligation
        signRegion T S) :
    TexMatchingUnprimedSaturatedFrecAsymptoticObligation signRegion T S :=
  ⟨texMatchingUnprimedSaturatedDialFrecTendsto_of_effectiveTail hunprimed.tendsto⟩

/-- The unprimed saturated `Frec` obligation has no additional bookkeeping beyond the
pointwise dial tendsto statement. -/
theorem texMatchingUnprimedSaturatedFrecAsymptoticObligation_iff_dialFrecTendsto
    {L d r : Nat}
    {θ θ' : Params (L + 1) d}
    {O : Set (ProbePair d)}
    (signRegion :
    ...
    TexMatchingUnprimedSaturatedFrecAsymptoticObligation signRegion T S ↔
      TexMatchingUnprimedSaturatedDialFrecTendsto
        (θ := θ) (θ' := θ') T S := by
  exact ⟨fun h => h.tendsto, fun h =>
    texMatchingUnprimedSaturatedFrecAsymptoticObligation_of_dialFrecTendsto
      signRegion T S h⟩
...
/-- Once the first layers are matched, the unprimed and primed effective tail points
along the same dial are identical. -/
theorem texMatchingUnprimedDialEffectiveTailPoint_eq_primed_of_matching
    {L d r : Nat}
    {θ θ' : Params (L + 1) d}
    (matching : FirstLayerMatchedData θ θ')
    (δ : DialPathData (Params.headAttention θ') (Real.log (r : ℝ)))
    ...
    texMatchingUnprimedDialEffectiveTailPoint
        (L := L) (d := d) (r := r) (θ := θ) (θ' := θ') δ τ =
      texMatchingPrimedDialEffectiveTailPoint
        (L := L) (d := d) (r := r) (θ' := θ') δ τ := by
  exact matching.paramsFirstLayerEffectivePoint_eq r
    (DialPathData.probe δ τ).1 (DialPathData.probe δ τ).2 τ

/-- Path-level form of
`texMatchingUnprimedDialEffectiveTailPoint_eq_primed_of_matching`. -/
theorem texMatchingUnprimedDialEffectiveTailPath_eq_primed_of_matching
    {L d r : Nat}
    {θ θ' : Params (L + 1) d}
    (matching : FirstLayerMatchedData θ θ')
    (δ : DialPathData (Params.headAttention θ') (Real.log (r : ℝ))) :
    texMatchingUnprimedDialEffectiveTailPath
        (L := L) (d := d) (r := r) (θ := θ) (θ' := θ') δ =
      texMatchingPrimedDialEffectiveTailPath
        (L := L) (d := d) (r := r) (θ' := θ') δ := by
  funext τ
  exact texMatchingUnprimedDialEffectiveTailPoint_eq_primed_of_matching
    (L := L) (d := d) (r := r) matching δ τ

/-- Observable agreement along the original dial transports to observable agreement of
the lower-depth tails along the peeled effective-tail path. -/
theorem texMatching_tailObservableAgreementOn_unprimedEffectiveTailPath_of_dial
    {L d r : Nat} (hr_pos : 0 < r)
    {θ θ' : Params (L + 1) d}
    (matching : FirstLayerMatchedData θ θ')
    {δ : DialPathData (Params.headAttention θ') (Real.log (r : ℝ))}
    {T0 : ℝ}
    (hobs : ObservableAgreementOnPath r θ θ' (DialPathData.probe δ) T0) :
    ObservableAgreementOnPath r (Params.tail θ) (Params.tail θ')
      (texMatchingUnprimedDialEffectiveTailPath
        (L := L) (d := d) (r := r) (θ := θ) (θ' := θ') δ) T0 := by
  refine ⟨hobs.1, ?_⟩
  intro τ hτ
  have hτ_nonneg : 0 ≤ τ := le_of_lt (lt_of_le_of_lt hobs.1 hτ)
  have hmatch := matching.paramsFirstLayerEffectivePoint_eq r
    (DialPathData.probe δ τ).1 (DialPathData.probe δ τ).2 τ
  calc
    Fobs r (Params.tail θ)
        ...
        =
      Fobs r θ (DialPathData.probe δ τ).1 (DialPathData.probe δ τ).2 τ := by
          exact (peeling_paramsFirstLayerEffectivePoint r hr_pos θ
            (DialPathData.probe δ τ).1 (DialPathData.probe δ τ).2 hτ_nonneg).symm
    _ = Fobs r θ' (DialPathData.probe δ τ).1 (DialPathData.probe δ τ).2 τ :=
          hobs.2 τ hτ
    _ =
      Fobs r (Params.tail θ')
        (texMatchingUnprimedDialEffectiveTailPath
          (L := L) (d := d) (r := r) (θ := θ) (θ' := θ') δ τ).1
        (texMatchingUnprimedDialEffectiveTailPath
          (L := L) (d := d) (r := r) (θ := θ) (θ' := θ') δ τ).2 τ := by
          have htarget :
              paramsFirstLayerEffectivePoint r θ'
                  (DialPathData.probe δ τ).1 (DialPathData.probe δ τ).2 τ =
                texMatchingUnprimedDialEffectiveTailPath
                  (L := L) (d := d) (r := r) (θ := θ) (θ' := θ') δ τ := by
            rw [← hmatch]
            rfl
          rw [peeling_paramsFirstLayerEffectivePoint r hr_pos θ'
            (DialPathData.probe δ τ).1 (DialPathData.probe δ τ).2 hτ_nonneg]
          rw [htarget]

/-- Frec version of the matched first-layer tail transport: eventual observable
agreement on a full dial gives eventual equality of the two tail recursions on the
peeled effective-tail path. -/
theorem texMatching_tailFrec_eq_on_unprimedEffectiveTailPath_of_dialObservableAgreement
    {L d r : Nat} (hr_pos : 0 < r)
    {θ θ' : Params (L + 1) d}
    (matching : FirstLayerMatchedData θ θ')
    {δ : DialPathData (Params.headAttention θ') (Real.log (r : ℝ))}
  ...
  exact texMatching_frec_eq_on_path_of_observableAgreement
    (L := L) (d := d) (r := r) hr_pos
    (P := texMatchingUnprimedDialEffectiveTailPath
      (L := L) (d := d) (r := r) (θ := θ) (θ' := θ') δ)
    (texMatching_tailObservableAgreementOn_unprimedEffectiveTailPath_of_dial
      (L := L) (d := d) (r := r) hr_pos matching hobs)

/-- Lower-depth tail-path convergence transport across matched first layers.

Once dial observable agreement has been peeled to eventual equality of the two tail
`Frec` recursions, a primed lower-depth convergence theorem along the matched unprimed
effective-tail path gives the corresponding unprimed convergence theorem with the same
primed target limit. -/
theorem texMatching_unprimedTailFrecTendsto_primedLimit_of_primedTailTendsto_of_dialObs
    {L d r : Nat} (hr_pos : 0 < r)
    {θ θ' : Params (L + 1) d}
    (matching : FirstLayerMatchedData θ θ')
    {δ : DialPathData (Params.headAttention θ') (Real.log (r : ℝ))}
    ...
    (hprimed :
      Filter.Tendsto
        (fun τ : ℝ =>
          ...
          Frec r (Params.tail θ') q.1 q.2 τ)
        Filter.atTop
        (nhds (texMatchingPrimedTelescopedLimitVector θ' δ.t δ.base))) :
    Filter.Tendsto
      (fun τ : ℝ =>
        let q :=
          texMatchingUnprimedDialEffectiveTailPath
        ...
        Frec r (Params.tail θ) q.1 q.2 τ)
      Filter.atTop
      (nhds (texMatchingPrimedTelescopedLimitVector θ' δ.t δ.base)) := by
  have htail_eq :
      (fun τ : ℝ =>
        let q :=
          texMatchingUnprimedDialEffectiveTailPath
    ...
    exact
      texMatching_tailFrec_eq_on_unprimedEffectiveTailPath_of_dialObservableAgreement
        (L := L) (d := d) (r := r) hr_pos matching hobs τ hτ
  intro s hs
  have hprimed_mem := hprimed hs
  rw [Filter.mem_map] at hprimed_mem ⊢
  filter_upwards [htail_eq, hprimed_mem] with τ hfg hmem
  change
    (let q :=
      texMatchingUnprimedDialEffectiveTailPath
        (L := L) (d := d) (r := r) (θ := θ) (θ' := θ') δ τ
    Frec r (Params.tail θ) q.1 q.2 τ) ∈ s
  rw [hfg]
  exact hmem

/-- Version of
`texMatching_unprimedTailFrecTendsto_primedLimit_of_primedTailTendsto_of_dialObs`
with the target rewritten to the saturated unprimed limit vector. -/
theorem texMatching_unprimedTailFrecTendsto_of_primedTailTendsto_of_dialObs
    {L d r : Nat} (hr_pos : 0 < r)
    {θ θ' : Params (L + 1) d}
    {varsigma : Nat -> ℝ}
    {S : TexMatchingSaturatedContributionData θ varsigma}
    ...
    (hobs : ObservableAgreementOnPath r θ θ' (DialPathData.probe δ) T0)
    (hlimit :
      texMatchingUnprimedSaturatedLimitVector θ S.D δ.t δ.base =
        texMatchingPrimedTelescopedLimitVector θ' δ.t δ.base)
    (hprimed :
      Filter.Tendsto
        (fun τ : ℝ =>
          let q :=
    ...
    Filter.Tendsto
      (fun τ : ℝ =>
        ...
        Frec r (Params.tail θ) q.1 q.2 τ)
      Filter.atTop
      (nhds (texMatchingUnprimedSaturatedLimitVector θ S.D δ.t δ.base)) := by
  simpa [hlimit] using
    texMatching_unprimedTailFrecTendsto_primedLimit_of_primedTailTendsto_of_dialObs
      (L := L) (d := d) (r := r) hr_pos matching hobs hprimed

/-- Peeled primed Frec asymptotic over the refined trichotomy component.

This is the remaining Step 2 analytic theorem after the first recursion layer has been
removed by the exact `Frec_succ_paramsFirstLayerEffectivePoint` identity: the tail
recursion, started at the actual first-layer effective point of the primed dial, tends
to the telescoped primed limit vector. -/
abbrev TexMatchingPrimedTelescopedEffectiveTailFrecAsymptotic
    {L d r : Nat}
    {θ' : Params (L + 1) d}
    {O : Set (ProbePair d)}
    (signRegion :
    ...
    (T :
      TexTrichotomyConstructionData (L := L + 1) (d := d)
        (Real.log (r : ℝ)) signRegion.U) : Prop :=
  ∀ δ : DialPathData (Params.headAttention θ') (Real.log (r : ℝ)),
    (δ.base, δ.t) ∈ T.Ustar ->
    Filter.Tendsto
      (fun τ : ℝ =>
...
/-- Primed lower-depth Frec asymptotic along the matched unprimed effective-tail path.

After first-layer matching this is equivalent to the peeled primed effective-tail
asymptotic, but it is the form naturally produced by the realized-tail path handoff. -/
abbrev TexMatchingPrimedTelescopedMatchedTailPathFrecAsymptotic
    {L d r : Nat}
    {θ θ' : Params (L + 1) d}
    {O : Set (ProbePair d)}
    (signRegion :
...
/-- The unprimed peeled statement is exactly lower-depth Frec convergence along the
unprimed effective-tail path. -/
theorem texMatchingUnprimedSaturatedEffectiveTailFrecAsymptotic_of_tailPath
    {L d r : Nat}
    {θ θ' : Params (L + 1) d}
    {signU : Set (ProbePair d × ℝ)}
    {T :
    ...
    TexMatchingUnprimedSaturatedEffectiveTailFrecAsymptotic
      (θ := θ) (θ' := θ') T S := by
  intro δ hδ
  simpa [texMatchingUnprimedDialEffectiveTailPath] using htail δ hδ

/-- The primed peeled statement is exactly lower-depth Frec convergence along the
primed effective-tail path. -/
theorem texMatchingPrimedTelescopedEffectiveTailFrecAsymptotic_of_tailPath
    {L d r : Nat}
    {θ' : Params (L + 1) d}
    {O : Set (ProbePair d)}
    {signRegion :
    ...
    (htail :
      ∀ δ : DialPathData (Params.headAttention θ') (Real.log (r : ℝ)),
        ...
        Filter.Tendsto
          ...
          Filter.atTop
          (nhds (texMatchingPrimedTelescopedLimitVector θ' δ.t δ.base))) :
    TexMatchingPrimedTelescopedEffectiveTailFrecAsymptotic signRegion T := by
  intro δ hδ
  simpa [texMatchingPrimedDialEffectiveTailPath] using htail δ hδ

/-- First-layer matching converts the lower-depth primed asymptotic on the matched
unprimed effective-tail path into the peeled primed effective-tail statement. -/
theorem texMatchingPrimedTelescopedEffectiveTailFrecAsymptotic_of_matchedTailPath
    {L d r : Nat}
    {θ θ' : Params (L + 1) d}
    {O : Set (ProbePair d)}
    {signRegion :
    ...
    (matching : FirstLayerMatchedData θ θ')
    (htail :
      TexMatchingPrimedTelescopedMatchedTailPathFrecAsymptotic
        (θ := θ) (θ' := θ') signRegion T) :
    TexMatchingPrimedTelescopedEffectiveTailFrecAsymptotic signRegion T := by
  intro δ hδ
  have hfun :
      (fun τ : ℝ =>
      ...
      (fun τ : ℝ =>
        ...
        Frec r (Params.tail θ') q.1 q.2 τ) := by
    funext τ
    rw [← texMatchingUnprimedDialEffectiveTailPoint_eq_primed_of_matching
      (L := L) (d := d) (r := r) matching δ τ]
    rfl
  rw [hfun]
  exact htail δ hδ

/-- Regular-quadric selected-dial version of the primed matched-tail Frec asymptotic.

This is the pointwise form that a recursive reduced provider naturally obtains after
showing that the matched unprimed effective-tail path belongs to the lower-depth path
class. -/
abbrev TexMatchingRegularQuadricPrimedMatchedTailPathFrecAsymptotic
    {L d r : Nat}
    {θ θ' : Params (L + 1) d}
    {O : Set (ProbePair d)}
    (signRegion :
...
/-- Selected-dial unprimed saturated full `Frec` asymptotic for a regular-quadric
product patch.

This is the product-patch restriction of
`TexMatchingUnprimedSaturatedDialFrecTendsto`: it only asks for the canonical dials
selected by `N`, and it does not mention local-patch construction parameters. -/
abbrev TexMatchingRegularQuadricUnprimedSaturatedFrecAsymptotic
    {L d r : Nat}
    {θ θ' : Params (L + 1) d}
    {O : Set (ProbePair d)}
    (signRegion :
...
/-- Restrict a global unprimed saturated dial asymptotic to the selected
regular-quadric dials. -/
theorem texMatchingRegularQuadricUnprimedSaturatedFrecAsymptotic_of_dialAsymptotic
    {L d r : Nat}
    {θ θ' : Params (L + 1) d}
    {O : Set (ProbePair d)}
    {signRegion :
    ...
    {varsigma : Nat -> ℝ}
    {S : TexMatchingSaturatedContributionData θ varsigma}
    (hunprimed : TexMatchingUnprimedSaturatedDialFrecTendsto (θ := θ) (θ' := θ') T S) :
    TexMatchingRegularQuadricUnprimedSaturatedFrecAsymptotic signRegion T N S := by
  intro p hp t ht
  let δ :=
    texMatchingRegularQuadricDialPathData
      (L := L) (d := d) (r := r) signRegion T N p hp t ht
  have hmem : (δ.base, δ.t) ∈ T.Ustar := by
    simpa [δ] using
      texMatchingRegularQuadricDialPathData_mem_Ustar
        (L := L) (d := d) (r := r) signRegion T N p hp t ht
  have hbase : δ.base = p := by
    simp [δ]
  have htarget : δ.t = t := by
    simp [δ]
  simpa [δ, hbase, htarget] using hunprimed δ hmem

/-- Convert the selected unprimed tail-path form to the selected effective-tail form. -/
theorem texMatchingRegularQuadricUnprimedSaturatedEffectiveTailFrecAsymptotic_of_tailPath
    {L d r : Nat}
    {θ θ' : Params (L + 1) d}
    {O : Set (ProbePair d)}
    {signRegion :
    ...
    TexMatchingRegularQuadricUnprimedSaturatedEffectiveTailFrecAsymptotic
      signRegion T N S := by
  intro p hp t ht
  simpa [texMatchingUnprimedDialEffectiveTailPath] using htail p hp t ht

/-- Peeling the first unprimed recursion layer reduces the selected full
regular-quadric asymptotic to the selected effective-tail theorem. -/
theorem texMatchingRegularQuadricUnprimedSaturatedFrecAsymptotic_of_effectiveTail
    {L d r : Nat}
    {θ θ' : Params (L + 1) d}
    {O : Set (ProbePair d)}
    {signRegion :
...
/-- Selected full regular-quadric unprimed asymptotic from the selected lower-depth
tail-path theorem. -/
theorem texMatchingRegularQuadricUnprimedSaturatedFrecAsymptotic_of_tailPath
    {L d r : Nat}
    {θ θ' : Params (L + 1) d}
    {O : Set (ProbePair d)}
    {signRegion :
    ...
    (htail :
      TexMatchingRegularQuadricUnprimedSaturatedTailPathFrecAsymptotic
        signRegion T N S) :
    TexMatchingRegularQuadricUnprimedSaturatedFrecAsymptotic signRegion T N S :=
  texMatchingRegularQuadricUnprimedSaturatedFrecAsymptotic_of_effectiveTail
    (texMatchingRegularQuadricUnprimedSaturatedEffectiveTailFrecAsymptotic_of_tailPath
      htail)

/-- Selected-dial primed telescoped full `Frec` asymptotic for a regular-quadric
product patch.

This is the product-patch restriction of
`TexMatchingPrimedTelescopedFrecDialAsymptotic`: it only asks for the canonical dials
selected by `N`, and it does not mention local-patch construction parameters. -/
abbrev TexMatchingRegularQuadricPrimedTelescopedFrecAsymptotic
    {L d r : Nat}
    {θ' : Params (L + 1) d}
    {O : Set (ProbePair d)}
    (signRegion :
...
/-- Direct equality of the two selected effective-tail limit functions.

This is the smallest comparison surface before identifying either selected limit with
the saturated/telescoped closed-form vectors. -/
structure TexMatchingRegularQuadricSelectedEffectiveTailLimitAgreementData
    {L d r : Nat}
    {θ θ' : Params (L + 1) d}
    {O : Set (ProbePair d)}
    (signRegion :
...
/-- Restrict a bundled lower-depth provider to selected effective-tail analytic data.

The provider may also carry limit identification fields; this constructor uses only its
lower-depth convergence and selected path-membership fields. -/
noncomputable def texMatchingRegularQuadricSelectedEffectiveTailFrecAnalyticData_of_lowerDepthProvider
    {L d r : Nat}
    {θ θ' : Params (L + 1) d}
    {O : Set (ProbePair d)}
    {signRegion :
  ...
  texMatchingRegularQuadricSelectedEffectiveTailFrecAnalyticData_of_lowerDepthTailPath
    (texMatchingRegularQuadricLowerDepthTailPathFrecAnalyticData_of_tailData
      provider.unprimed_tail provider.primed_tail)
    { unprimed_path_mem := provider.unprimed_path_mem
      primed_path_mem := provider.primed_path_mem }

/-- Selected effective-tail convergence data directly from closed-recursion limits.

The full-depth canonical dial limits are peeled by the exact first-layer identity, so
the selected effective-tail limits are the concrete saturated/telescoped vectors. -/
noncomputable def texMatchingRegularQuadricSelectedEffectiveTailFrecAnalyticData_of_closedRecursionLimits
    {L d r : Nat}
    {θ θ' : Params (L + 1) d}
    {O : Set (ProbePair d)}
    {signRegion :
    ...
    TexMatchingRegularQuadricSelectedEffectiveTailFrecAnalyticData
      (θ := θ) (θ' := θ') signRegion T N where
  unprimedLimit := fun p _hp t _ht =>
    texMatchingUnprimedSaturatedLimitVector θ S.D t p
  primedLimit := fun p _hp t _ht =>
    texMatchingPrimedTelescopedLimitVector θ' t p
  unprimed_tendsto := by
    intro p hp t ht
    let δ :=
...
/-- The selected effective-tail convergence/identification package implies the
selected unprimed peeled asymptotic. -/
theorem texMatchingRegularQuadricUnprimedSaturatedEffectiveTailFrecAsymptotic_of_selectedEffectiveTailData
    {L d r : Nat}
    {θ θ' : Params (L + 1) d}
    {O : Set (ProbePair d)}
    {signRegion :
    ...
    TexMatchingRegularQuadricUnprimedSaturatedEffectiveTailFrecAsymptotic
      signRegion T N S := by
  refine texMatchingRegularQuadricUnprimedSaturatedEffectiveTailFrecAsymptotic_of_tailPath ?_
  intro p hp t ht
  have hlimit := limits.unprimed_limit_eq p hp t ht
  simpa [hlimit] using analytic.unprimed_tendsto p hp t ht

/-- The selected effective-tail convergence/identification package implies the
selected primed peeled asymptotic. -/
theorem texMatchingRegularQuadricPrimedTelescopedEffectiveTailFrecAsymptotic_of_selectedEffectiveTailData
    {L d r : Nat}
    {θ θ' : Params (L + 1) d}
    {O : Set (ProbePair d)}
    {signRegion :
  ...
  simpa [hlimit, texMatchingPrimedDialEffectiveTailPath] using
    analytic.primed_tendsto p hp t ht

/-- Extract the selected unprimed peeled asymptotic from a lower-depth provider. -/
theorem texMatchingRegularQuadricUnprimedSaturatedEffectiveTailFrecAsymptotic_of_lowerDepthProvider
    {L d r : Nat}
    {θ θ' : Params (L + 1) d}
    {O : Set (ProbePair d)}
    {signRegion :
...
/-- Extract the selected primed peeled asymptotic from a lower-depth provider. -/
theorem texMatchingRegularQuadricPrimedTelescopedEffectiveTailFrecAsymptotic_of_lowerDepthProvider
    {L d r : Nat}
    {θ θ' : Params (L + 1) d}
    {O : Set (ProbePair d)}
    {signRegion :
...
/-- Selected regular-quadric tail-path asymptotics discharge the selected
closed-recursion limits after the exact first-layer peel. -/
theorem texMatchingRegularQuadricClosedRecursionLimitObligation_of_selectedTailPath
    {L d r : Nat}
    {θ θ' : Params (L + 1) d}
    {O : Set (ProbePair d)}
    {signRegion :
...
/-- A lower-depth provider supplies the selected closed-recursion limit obligation.

This keeps the remaining assumptions at the recursive path-class level: convergence,
selected effective-tail path membership, and selected limit-vector identification. -/
theorem texMatchingRegularQuadricClosedRecursionLimitObligation_of_lowerDepthProvider
    {L d r : Nat}
    {θ θ' : Params (L + 1) d}
    {O : Set (ProbePair d)}
    {signRegion :
    ...
    (provider :
      TexMatchingRegularQuadricLowerDepthTailPathFrecProviderData
        signRegion T N S UnprimedTailPaths PrimedTailPaths) :
    TexMatchingRegularQuadricClosedRecursionLimitObligation signRegion T N S :=
  texMatchingRegularQuadricClosedRecursionLimitObligation_of_selectedTailPath
    (texMatchingRegularQuadricUnprimedSaturatedTailPathFrecAsymptotic_of_lowerDepthProvider
      provider)
    (texMatchingRegularQuadricPrimedTelescopedTailPathFrecAsymptotic_of_lowerDepthProvider
      provider)

/-- Restrict a global primed telescoped dial asymptotic to the selected regular-quadric
dials. -/
theorem texMatchingRegularQuadricPrimedTelescopedFrecAsymptotic_of_dialAsymptotic
    {L d r : Nat}
    {θ' : Params (L + 1) d}
    {O : Set (ProbePair d)}
    {signRegion :
    ...
    {N :
      TexMatchingProductNeighborhoodData (d := d)
        (Params.headAttention θ') T.Ustar}
    (hprimed : TexMatchingPrimedTelescopedFrecDialAsymptotic signRegion T) :
    TexMatchingRegularQuadricPrimedTelescopedFrecAsymptotic signRegion T N := by
  intro p hp t ht
  let δ :=
    texMatchingRegularQuadricDialPathData
      (L := L) (d := d) (r := r) signRegion T N p hp t ht
  ...
  have htarget : δ.t = t := by
    simp [δ]
  simpa [δ, hbase, htarget] using hprimed δ hmem

/-- Convert the selected tail-path form to the selected effective-tail form. -/
theorem texMatchingRegularQuadricPrimedTelescopedEffectiveTailFrecAsymptotic_of_tailPath
    {L d r : Nat}
    {θ' : Params (L + 1) d}
    {O : Set (ProbePair d)}
    {signRegion :
    ...
    TexMatchingRegularQuadricPrimedTelescopedEffectiveTailFrecAsymptotic
      signRegion T N := by
  intro p hp t ht
  simpa [texMatchingPrimedDialEffectiveTailPath] using htail p hp t ht

/-- Peeling the first primed recursion layer reduces the selected full regular-quadric
asymptotic to the selected effective-tail theorem. -/
theorem texMatchingRegularQuadricPrimedTelescopedFrecAsymptotic_of_effectiveTail
    {L d r : Nat}
    {θ' : Params (L + 1) d}
    {O : Set (ProbePair d)}
    {signRegion :
...
/-- Selected full regular-quadric primed asymptotic from the selected lower-depth
tail-path theorem. -/
theorem texMatchingRegularQuadricPrimedTelescopedFrecAsymptotic_of_tailPath
    {L d r : Nat}
    {θ' : Params (L + 1) d}
    {O : Set (ProbePair d)}
    {signRegion :
    ...
    (htail :
      TexMatchingRegularQuadricPrimedTelescopedTailPathFrecAsymptotic
        signRegion T N) :
    TexMatchingRegularQuadricPrimedTelescopedFrecAsymptotic signRegion T N :=
  texMatchingRegularQuadricPrimedTelescopedFrecAsymptotic_of_effectiveTail
    (texMatchingRegularQuadricPrimedTelescopedEffectiveTailFrecAsymptotic_of_tailPath
      htail)

/-- Provider-facing Frec package for selected regular-quadric dials.

The recursive side is expected to supply the primed lower-depth asymptotic along the
matched unprimed effective-tail path.  To transport that theorem to the saturated
unprimed target, the provider also records the non-Frec endpoint limit-vector equality;
path membership connects the selected full dial to `IDLData.path_agreement`. -/
structure TexMatchingRegularQuadricMatchedTailPathFrecProviderData
    {L d r : Nat}
    {θ θ' : Params (L + 1) d}
    (D : IDLData (L + 1) d r θ θ')
    {O : Set (ProbePair d)}
  ...
  primed_matched_tail :
    TexMatchingRegularQuadricPrimedMatchedTailPathFrecAsymptotic
      (θ := θ) (θ' := θ') signRegion T N
  limit_vector_eq :
    ∀ (p : ProbePair d) (_hp : p ∈ N.Uq) (t : ℝ) (_ht : t ∈ N.J),
      texMatchingUnprimedSaturatedLimitVector θ S.D t p =
        texMatchingPrimedTelescopedLimitVector θ' t p

/-- Restrict a global matched-tail primed Frec asymptotic to the selected
regular-quadric dials. -/
theorem texMatchingRegularQuadricPrimedMatchedTailPathFrecAsymptotic_of_global
    {L d r : Nat}
    {θ θ' : Params (L + 1) d}
    {O : Set (ProbePair d)}
    {signRegion :
...
/-- Assemble the selected-dial Frec provider from the global matched-tail asymptotic
and the two selected-dial bookkeeping fields. -/
theorem texMatchingRegularQuadricMatchedTailPathFrecProviderData_of_global
    {L d r : Nat}
    {θ θ' : Params (L + 1) d}
    {D : IDLData (L + 1) d r θ θ'}
    {O : Set (ProbePair d)}
    ...
    (hlimit :
      ∀ (p : ProbePair d) (_hp : p ∈ N.Uq) (t : ℝ) (_ht : t ∈ N.J),
        texMatchingUnprimedSaturatedLimitVector θ S.D t p =
          texMatchingPrimedTelescopedLimitVector θ' t p) :
    TexMatchingRegularQuadricMatchedTailPathFrecProviderData
      D signRegion T N S :=
  { dial_mem := hdial
    primed_matched_tail :=
      texMatchingRegularQuadricPrimedMatchedTailPathFrecAsymptotic_of_global
        (N := N) hprimed
    limit_vector_eq := hlimit }

/-- The selected-dial matched-tail Frec provider supplies the closed-recursion limits
needed by the regular-quadric matching bridge. -/
theorem texMatchingRegularQuadricClosedRecursionLimitObligation_of_matchedTailPathProvider
    {L d r : Nat} (hr_pos : 0 < r)
    {θ θ' : Params (L + 1) d}
    (D : IDLData (L + 1) d r θ θ')
    {O : Set (ProbePair d)}
  ...
  · intro p hp t ht
    let δ :=
      texMatchingRegularQuadricDialPathData
        (L := L) (d := d) (r := r) signRegion T N p hp t ht
    rcases D.path_agreement (DialPathData.probe δ)
        (by simpa [δ] using provider.dial_mem p hp t ht) with
      ⟨T0, hobs⟩
    have hbase : δ.base = p := by
      simp [δ]
    have htarget : δ.t = t := by
    ...
    have hprimed :
        Filter.Tendsto
          ...
          Filter.atTop
          (nhds (texMatchingPrimedTelescopedLimitVector θ' δ.t δ.base)) := by
      simpa [δ, hbase, htarget] using provider.primed_matched_tail p hp t ht
    have hlimit :
        texMatchingUnprimedSaturatedLimitVector θ S.D δ.t δ.base =
          texMatchingPrimedTelescopedLimitVector θ' δ.t δ.base := by
      simpa [δ, hbase, htarget] using provider.limit_vector_eq p hp t ht
    have htail :=
      texMatching_unprimedTailFrecTendsto_of_primedTailTendsto_of_dialObs
        (L := L) (d := d) (r := r) (θ := θ) (θ' := θ') (S := S)
        hr_pos matching hobs hlimit hprimed
    have htail_point :
        Filter.Tendsto
          (fun τ : ℝ =>
            let q :=
          ...
          Filter.atTop
          (nhds (texMatchingUnprimedSaturatedLimitVector θ S.D t p)) := by
      simpa [texMatchingUnprimedDialEffectiveTailPath, δ, hbase, htarget] using htail
    have hfun :
        (fun τ : ℝ =>
          Frec r θ (DialPathData.probe δ τ).1 (DialPathData.probe δ τ).2 τ) =
        (fun τ : ℝ =>
      ...
      exact Frec_succ_paramsFirstLayerEffectivePoint r θ
        (DialPathData.probe δ τ).1 (DialPathData.probe δ τ).2 τ
    simpa [δ, hfun] using htail_point
  · intro p hp t ht
    let δ :=
      texMatchingRegularQuadricDialPathData
        (L := L) (d := d) (r := r) signRegion T N p hp t ht
    ...
    have hprimed_tail :
        Filter.Tendsto
          ...
          Filter.atTop
          (nhds (texMatchingPrimedTelescopedLimitVector θ' t p)) := by
      simpa [δ] using provider.primed_matched_tail p hp t ht
    have hprimed_point :
        Filter.Tendsto
          (fun τ : ℝ =>
            let q :=
    ...
    have hfun :
      ...
      exact Frec_succ_paramsFirstLayerEffectivePoint r θ'
        (DialPathData.probe δ τ).1 (DialPathData.probe δ τ).2 τ
    simpa [δ, hfun, hbase, htarget] using hprimed_point

/-- Structure wrapper for the peeled primed effective-tail asymptotic. -/
structure TexMatchingPrimedTelescopedEffectiveTailFrecAsymptoticObligation
    {L d r : Nat}
    {θ' : Params (L + 1) d}
    {O : Set (ProbePair d)}
    (signRegion :
    ...
    (T :
      TexTrichotomyConstructionData (L := L + 1) (d := d)
        (Real.log (r : ℝ)) signRegion.U) : Prop where
  tendsto :
    TexMatchingPrimedTelescopedEffectiveTailFrecAsymptotic signRegion T

/-- Peeling the first primed recursion layer reduces the full dial asymptotic to the
effective-tail asymptotic. -/
theorem texMatchingPrimedTelescopedFrecDialAsymptotic_of_effectiveTail
    {L d r : Nat}
    {θ' : Params (L + 1) d}
    {O : Set (ProbePair d)}
    {signRegion :
...
/-- The full primed dial asymptotic also supplies the peeled effective-tail form. -/
theorem texMatchingPrimedTelescopedEffectiveTailFrecAsymptotic_of_dialAsymptotic
    {L d r : Nat}
    {θ' : Params (L + 1) d}
    {O : Set (ProbePair d)}
    {signRegion :
...
/-- The exposed primed dial gap is exactly the peeled effective-tail gap. -/
theorem texMatchingPrimedTelescopedFrecDialAsymptotic_iff_effectiveTail
    {L d r : Nat}
    {θ' : Params (L + 1) d}
    {O : Set (ProbePair d)}
    (signRegion :
    ...
    (T :
      TexTrichotomyConstructionData (L := L + 1) (d := d)
        (Real.log (r : ℝ)) signRegion.U) :
    TexMatchingPrimedTelescopedFrecDialAsymptotic signRegion T ↔
      TexMatchingPrimedTelescopedEffectiveTailFrecAsymptotic signRegion T := by
  exact
    ⟨texMatchingPrimedTelescopedEffectiveTailFrecAsymptotic_of_dialAsymptotic,
      texMatchingPrimedTelescopedFrecDialAsymptotic_of_effectiveTail⟩

/-- Packaged version of the peeled effective-tail reduction. -/
theorem texMatchingPrimedTelescopedFrecAsymptoticObligation_of_effectiveTail
    {L d r : Nat}
    {θ' : Params (L + 1) d}
    {O : Set (ProbePair d)}
    {signRegion :
    ...
    {T :
      TexTrichotomyConstructionData (L := L + 1) (d := d)
        (Real.log (r : ℝ)) signRegion.U}
    (hprimed :
      TexMatchingPrimedTelescopedEffectiveTailFrecAsymptoticObligation
        signRegion T) :
    TexMatchingPrimedTelescopedFrecAsymptoticObligation signRegion T :=
  ⟨texMatchingPrimedTelescopedFrecDialAsymptotic_of_effectiveTail hprimed.tendsto⟩

/-- Extract the pointwise primed dial asymptotic from the packaged obligation. -/
theorem texMatchingPrimedTelescopedFrecDialAsymptotic_of_obligation
    {L d r : Nat}
    {θ' : Params (L + 1) d}
    {O : Set (ProbePair d)}
    {signRegion :
    ...
    {T :
      TexTrichotomyConstructionData (L := L + 1) (d := d)
        (Real.log (r : ℝ)) signRegion.U}
    (hprimed : TexMatchingPrimedTelescopedFrecAsymptoticObligation signRegion T) :
    TexMatchingPrimedTelescopedFrecDialAsymptotic signRegion T :=
  hprimed.tendsto

/-- The pointwise dial asymptotics directly discharge the closed-recursion package for
the canonical regular-quadric product-patch dials. -/
theorem texMatchingRegularQuadricClosedRecursionLimitObligation_of_dialAsymptotics
    {L d r : Nat}
    {θ θ' : Params (L + 1) d}
    {O : Set (ProbePair d)}
    (signRegion :
    ...
    (hprimed : TexMatchingPrimedTelescopedFrecDialAsymptotic signRegion T) :
    TexMatchingRegularQuadricClosedRecursionLimitObligation signRegion T N S :=
  texMatchingRegularQuadricClosedRecursionLimitObligation_of_frecAsymptotics
    (L := L) (d := d) (r := r) signRegion T N S
    (texMatchingUnprimedSaturatedFrecAsymptoticObligation_of_dialFrecTendsto
      signRegion T S hunprimed)
    (texMatchingPrimedTelescopedFrecAsymptoticObligation_of_dialAsymptotic
      hprimed)

/-- The peeled effective-tail asymptotics directly discharge the closed-recursion
package after the exact first-layer recursion peel. -/
theorem texMatchingRegularQuadricClosedRecursionLimitObligation_of_effectiveTail
    {L d r : Nat}
    {θ θ' : Params (L + 1) d}
    {O : Set (ProbePair d)}
    (signRegion :
    ...
    (hunprimed :
      TexMatchingUnprimedSaturatedEffectiveTailFrecAsymptotic
        (θ := θ) (θ' := θ') T S)
    (hprimed : TexMatchingPrimedTelescopedEffectiveTailFrecAsymptotic signRegion T) :
    TexMatchingRegularQuadricClosedRecursionLimitObligation signRegion T N S :=
  texMatchingRegularQuadricClosedRecursionLimitObligation_of_dialAsymptotics
    (L := L) (d := d) (r := r) signRegion T N S
    (texMatchingUnprimedSaturatedDialFrecTendsto_of_effectiveTail hunprimed)
    (texMatchingPrimedTelescopedFrecDialAsymptotic_of_effectiveTail hprimed)
...
/-- Direct regular-quadric limit-vector theorem from the selected-dial matched-tail
Frec provider. -/
theorem texMatchingRegularQuadricLimitVectorObligation_of_matchedTailPathProvider
    {L d r : Nat} (hr_pos : 0 < r)
    {θ θ' : Params (L + 1) d}
    (D : IDLData (L + 1) d r θ θ')
    {O : Set (ProbePair d)}
  ...
  texMatchingRegularQuadricLimitVectorObligation_of_closedRecursionLimits
    (L := L) (d := d) (r := r) hr_pos signRegion T N S
    (texMatchingRegularQuadricClosedRecursionLimitObligation_of_matchedTailPathProvider
      (L := L) (d := d) (r := r) hr_pos D signRegion T N S matching provider)

/-- The selected-dial matched-tail Frec provider also supplies the dial-path bridge
consumed by the matching-limit endpoint: the dial is the canonical regular-quadric
dial, path membership is a provider field, and the observable-limit implication comes
from the limit-vector theorem above. -/
noncomputable def texMatchingDialPathLimitBridgeData_of_matchedTailPathProvider
    {L d r : Nat} (hr_pos : 0 < r)
    {θ θ' : Params (L + 1) d}
    (D : IDLData (L + 1) d r θ θ')
    {O : Set (ProbePair d)}
  ...
  let hlimit :
      TexMatchingRegularQuadricLimitVectorObligation signRegion T N S :=
    texMatchingRegularQuadricLimitVectorObligation_of_matchedTailPathProvider
      (L := L) (d := d) (r := r) hr_pos D signRegion T N S matching provider
  exact
    { dial := fun p hp t ht =>
        texMatchingRegularQuadricDialPathData
      ...
      path_mem := by
        intro p hp t ht
        exact provider.dial_mem p hp t ht
      limit_of_observable := hlimit }

/-- Product-patch spelling of the pointwise regular-quadric dial limit theorem. -/
abbrev TexMatchingRegularQuadricProductLimitObligation
    {L d r : Nat}
    {θ θ' : Params (L + 1) d}
    {O : Set (ProbePair d)}
    (signRegion :
...
/-- Local-patch spelling of the unprimed saturated `Frec` asymptotic over the canonical
`region/signRegion/T/N/S` generated by the local quadric patch. -/
abbrev TexMatchingLocalPatchUnprimedSaturatedFrecAsymptoticObligation
    {L d r : Nat} (hL : 0 < L) (hr : 2 <= r)
    {θ θ' : Params (L + 1) d}
    (hstep :
      TexGenericStepClauses L d (TexGeneric L d)
...
/-- Local-patch spelling of the primed telescoped `Frec` asymptotic over the canonical
`region/signRegion/T/N/S` generated by the local quadric patch. -/
abbrev TexMatchingLocalPatchPrimedTelescopedFrecAsymptoticObligation
    {L d r : Nat} (hL : 0 < L) (hr : 2 <= r)
    {θ θ' : Params (L + 1) d}
    (hstep :
      TexGenericStepClauses L d (TexGeneric L d)
...
/-- Local-patch spelling of the primed telescoped dial-level `Frec` asymptotic over the
canonical `region/signRegion/T` generated by the local quadric patch. -/
abbrev TexMatchingLocalPatchPrimedTelescopedFrecDialAsymptotic
    {L d r : Nat} (hL : 0 < L) (hr : 2 <= r)
    {θ θ' : Params (L + 1) d}
    (hstep :
      TexGenericStepClauses L d (TexGeneric L d)
...
/-- Local-patch spelling of the peeled primed telescoped `Frec` asymptotic. -/
abbrev TexMatchingLocalPatchPrimedTelescopedEffectiveTailFrecAsymptotic
    {L d r : Nat} (hL : 0 < L) (hr : 2 <= r)
    {θ θ' : Params (L + 1) d}
    (hstep :
      TexGenericStepClauses L d (TexGeneric L d)
...
/-- Local-patch spelling of the lower-depth primed tail-path `Frec` limit.

This is the explicit path form of
`TexMatchingLocalPatchPrimedTelescopedEffectiveTailFrecAsymptotic`: callers provide
the tail recursion along the peeled primed effective-tail path. -/
abbrev TexMatchingLocalPatchPrimedTelescopedTailPathFrecAsymptotic
    {L d r : Nat} (hL : 0 < L) (hr : 2 <= r)
    {θ θ' : Params (L + 1) d}
    (hstep :
      TexGenericStepClauses L d (TexGeneric L d)
...
/-- Local-patch spelling of the primed lower-depth Frec limit along the matched
unprimed effective-tail path. -/
abbrev TexMatchingLocalPatchPrimedTelescopedMatchedTailPathFrecAsymptotic
    {L d r : Nat} (hL : 0 < L) (hr : 2 <= r)
    {θ θ' : Params (L + 1) d}
    (hstep :
      TexGenericStepClauses L d (TexGeneric L d)
...
/-- Convert the local lower-depth unprimed tail-path limit to the peeled effective-tail
asymptotic field. -/
theorem texMatchingLocalPatchUnprimedSaturatedEffectiveTailFrecAsymptotic_of_tailPath
    {L d r : Nat} (hL : 0 < L) (hr : 2 <= r)
    {θ θ' : Params (L + 1) d}
    (hstep :
      TexGenericStepClauses L d (TexGeneric L d)
    ...
    (localRegion : TexRegionConstructionDataOfIDLDataObligation hL D)
    (htail :
      TexMatchingLocalPatchUnprimedSaturatedTailPathFrecAsymptotic
        hL hr hstep endpoint D localRegion) :
    TexMatchingLocalPatchUnprimedSaturatedEffectiveTailFrecAsymptotic
      hL hr hstep endpoint D localRegion := by
  dsimp [TexMatchingLocalPatchUnprimedSaturatedTailPathFrecAsymptotic,
    TexMatchingLocalPatchUnprimedSaturatedEffectiveTailFrecAsymptotic] at htail ⊢
  exact
    texMatchingUnprimedSaturatedEffectiveTailFrecAsymptotic_of_tailPath
      (θ := θ) (θ' := θ') htail

/-- Convert the local lower-depth primed tail-path limit to the peeled effective-tail
asymptotic field. -/
theorem texMatchingLocalPatchPrimedTelescopedEffectiveTailFrecAsymptotic_of_tailPath
    {L d r : Nat} (hL : 0 < L) (hr : 2 <= r)
    {θ θ' : Params (L + 1) d}
    (hstep :
      TexGenericStepClauses L d (TexGeneric L d)
    ...
    (localRegion : TexRegionConstructionDataOfIDLDataObligation hL D)
    (htail :
      TexMatchingLocalPatchPrimedTelescopedTailPathFrecAsymptotic
        hL hr hstep endpoint D localRegion) :
    TexMatchingLocalPatchPrimedTelescopedEffectiveTailFrecAsymptotic
      hL hr hstep endpoint D localRegion := by
  dsimp [TexMatchingLocalPatchPrimedTelescopedTailPathFrecAsymptotic,
    TexMatchingLocalPatchPrimedTelescopedEffectiveTailFrecAsymptotic] at htail ⊢
  exact texMatchingPrimedTelescopedEffectiveTailFrecAsymptotic_of_tailPath htail

/-- First-layer matching converts the local matched-tail path asymptotic into the
local primed peeled effective-tail field. -/
theorem texMatchingLocalPatchPrimedTelescopedEffectiveTailFrecAsymptotic_of_matchedTailPath
    {L d r : Nat} (hL : 0 < L) (hr : 2 <= r)
    {θ θ' : Params (L + 1) d}
    (hstep :
      TexGenericStepClauses L d (TexGeneric L d)
    ...
    (matching : FirstLayerMatchedData θ θ')
    (htail :
      TexMatchingLocalPatchPrimedTelescopedMatchedTailPathFrecAsymptotic
        hL hr hstep endpoint D localRegion) :
    TexMatchingLocalPatchPrimedTelescopedEffectiveTailFrecAsymptotic
      hL hr hstep endpoint D localRegion := by
  dsimp [TexMatchingLocalPatchPrimedTelescopedMatchedTailPathFrecAsymptotic,
    TexMatchingLocalPatchPrimedTelescopedEffectiveTailFrecAsymptotic] at htail ⊢
  exact
    texMatchingPrimedTelescopedEffectiveTailFrecAsymptotic_of_matchedTailPath
      matching htail

/-- Peeling the first unprimed recursion layer discharges the local-patch unprimed
dial-level `Frec` tendsto statement. -/
theorem texMatchingLocalPatchUnprimedSaturatedDialFrecTendsto_of_effectiveTail
    {L d r : Nat} (hL : 0 < L) (hr : 2 <= r)
    {θ θ' : Params (L + 1) d}
    (hstep :
      TexGenericStepClauses L d (TexGeneric L d)
    ...
    (hunprimed :
      TexMatchingLocalPatchUnprimedSaturatedEffectiveTailFrecAsymptotic
        hL hr hstep endpoint D localRegion) :
    TexMatchingLocalPatchUnprimedSaturatedDialFrecTendsto
      hL hr hstep endpoint D localRegion := by
  dsimp [TexMatchingLocalPatchUnprimedSaturatedEffectiveTailFrecAsymptotic,
    TexMatchingLocalPatchUnprimedSaturatedDialFrecTendsto] at hunprimed ⊢
  exact texMatchingUnprimedSaturatedDialFrecTendsto_of_effectiveTail hunprimed

/-- Peeling the first primed recursion layer discharges the local-patch primed
dial-level `Frec` asymptotic. -/
theorem texMatchingLocalPatchPrimedTelescopedFrecDialAsymptotic_of_effectiveTail
    {L d r : Nat} (hL : 0 < L) (hr : 2 <= r)
    {θ θ' : Params (L + 1) d}
    (hstep :
      TexGenericStepClauses L d (TexGeneric L d)
    ...
    (hprimed :
      TexMatchingLocalPatchPrimedTelescopedEffectiveTailFrecAsymptotic
        hL hr hstep endpoint D localRegion) :
    TexMatchingLocalPatchPrimedTelescopedFrecDialAsymptotic
      hL hr hstep endpoint D localRegion := by
  dsimp [TexMatchingLocalPatchPrimedTelescopedEffectiveTailFrecAsymptotic,
    TexMatchingLocalPatchPrimedTelescopedFrecDialAsymptotic] at hprimed ⊢
  exact texMatchingPrimedTelescopedFrecDialAsymptotic_of_effectiveTail hprimed

/-- The local-patch unprimed dial-level tendsto theorem is exactly the unprimed `Frec`
asymptotic obligation for the canonical local patch. -/
theorem texMatchingLocalPatchUnprimedSaturatedFrecAsymptoticObligation_of_dialFrecTendsto
    {L d r : Nat} (hL : 0 < L) (hr : 2 <= r)
    {θ θ' : Params (L + 1) d}
    (hstep :
      TexGenericStepClauses L d (TexGeneric L d)
    ...
    (h :
      TexMatchingLocalPatchUnprimedSaturatedDialFrecTendsto
        hL hr hstep endpoint D localRegion) :
    TexMatchingLocalPatchUnprimedSaturatedFrecAsymptoticObligation
      hL hr hstep endpoint D localRegion := by
  dsimp [TexMatchingLocalPatchUnprimedSaturatedDialFrecTendsto,
    TexMatchingLocalPatchUnprimedSaturatedFrecAsymptoticObligation] at h ⊢
  exact
    texMatchingUnprimedSaturatedFrecAsymptoticObligation_of_dialFrecTendsto
      (texMatchingSignRegionData_of_region
        (L := L) (d := d) (r := r) hr hstep endpoint D
...
/-- The local-patch primed dial-level asymptotic is exactly the primed `Frec`
asymptotic obligation for the canonical local patch. -/
theorem texMatchingLocalPatchPrimedTelescopedFrecAsymptoticObligation_of_dialAsymptotic
    {L d r : Nat} (hL : 0 < L) (hr : 2 <= r)
    {θ θ' : Params (L + 1) d}
    (hstep :
      TexGenericStepClauses L d (TexGeneric L d)
    ...
    (h :
      TexMatchingLocalPatchPrimedTelescopedFrecDialAsymptotic
        hL hr hstep endpoint D localRegion) :
    TexMatchingLocalPatchPrimedTelescopedFrecAsymptoticObligation
      hL hr hstep endpoint D localRegion := by
  dsimp [TexMatchingLocalPatchPrimedTelescopedFrecDialAsymptotic,
    TexMatchingLocalPatchPrimedTelescopedFrecAsymptoticObligation] at h ⊢
  exact
    texMatchingPrimedTelescopedFrecAsymptoticObligation_of_dialAsymptotic
      (signRegion :=
        texMatchingSignRegionData_of_region
...
/-- Local-patch regular-quadric limit theorem from the direct unprimed and primed
`Frec` asymptotics.  This route does not use matched-tail provider data or an already
constructed `FirstLayerMatchedData θ θ'`. -/
theorem texMatchingLocalPatchRegularQuadricLimitObligation_of_frecAsymptotics
    {L d r : Nat} (hL : 0 < L) (hr : 2 <= r)
    {θ θ' : Params (L + 1) d}
    (hstep :
      TexGenericStepClauses L d (TexGeneric L d)
    ...
    TexMatchingLocalPatchRegularQuadricLimitObligation
      hL hr hstep endpoint D localRegion := by
  dsimp [TexMatchingLocalPatchUnprimedSaturatedFrecAsymptoticObligation,
    TexMatchingLocalPatchPrimedTelescopedFrecAsymptoticObligation,
    TexMatchingLocalPatchRegularQuadricLimitObligation,
    TexMatchingLocalPatchRegularQuadricLimitVectorObligation] at hunprimed hprimed ⊢
  exact
    texMatchingRegularQuadricLimitVectorObligation_of_closedRecursionLimits
...
/-- Local-patch spelling of the selected-dial matched-tail Frec provider over the
canonical `region/signRegion/T/N/S` generated by the local quadric patch. -/
abbrev TexMatchingLocalPatchMatchedTailPathFrecProviderData
    {L d r : Nat} (hL : 0 < L) (hr : 2 <= r)
    {θ θ' : Params (L + 1) d}
    (hstep :
      TexGenericStepClauses L d (TexGeneric L d)
  ...
  let S :=
    texMatchingSaturatedContributionData_of_region
      (L := L) (d := d) (r := r) hr hstep endpoint D region
  TexMatchingRegularQuadricMatchedTailPathFrecProviderData D signRegion T N S

/-- Local-patch regular-quadric limit theorem from the selected-dial matched-tail
Frec provider specialized to the canonical region/trichotomy/product patch. -/
theorem texMatchingLocalPatchRegularQuadricLimitObligation_of_matchedTailPathProvider
    {L d r : Nat} (hL : 0 < L) (hr : 2 <= r)
    {θ θ' : Params (L + 1) d}
    (hstep :
      TexGenericStepClauses L d (TexGeneric L d)
    ...
    (matching : FirstLayerMatchedData θ θ')
    (provider :
      TexMatchingLocalPatchMatchedTailPathFrecProviderData
        hL hr hstep endpoint D localRegion) :
    TexMatchingLocalPatchRegularQuadricLimitObligation
      hL hr hstep endpoint D localRegion := by
  dsimp [TexMatchingLocalPatchMatchedTailPathFrecProviderData,
    TexMatchingLocalPatchRegularQuadricLimitObligation,
    TexMatchingLocalPatchRegularQuadricLimitVectorObligation] at provider ⊢
  exact
    texMatchingRegularQuadricLimitVectorObligation_of_matchedTailPathProvider
      (L := L) (d := d) (r := r) (by omega) D
      (texMatchingSignRegionData_of_region
        (L := L) (d := d) (r := r) hr hstep endpoint D
        (texRegionConstructionData_of_IDLData_of_localPatch
          (L := L) (d := d) (r := r) hL hr hstep endpoint D localRegion))
...
/-- Local-patch region provider from the selected-dial matched-tail Frec provider.
This fills the canonical dial-path bridge directly, without a universal-path
assumption. -/
noncomputable def texMatchingRegionProviderFromLocalPatchData_of_matchedTailPathProvider
    {L d r : Nat} (hL : 0 < L) (hr : 2 <= r)
    {θ θ' : Params (L + 1) d}
    (hstep :
      TexGenericStepClauses L d (TexGeneric L d)
    ...
    TexMatchingRegionProviderFromLocalPatchData
      hL hr hstep endpoint D localRegion := by
  dsimp [TexMatchingLocalPatchMatchedTailPathFrecProviderData] at provider
  exact
    { bridge :=
        texMatchingDialPathLimitBridgeData_of_matchedTailPathProvider
          (L := L) (d := d) (r := r) (by omega) D
          (texMatchingSignRegionData_of_region
            (L := L) (d := d) (r := r) hr hstep endpoint D
            (texRegionConstructionData_of_IDLData_of_localPatch
...
/-- Selected dial membership and `D.path_agreement` identify the two selected
effective-tail limit functions directly, without first naming the saturated or
telescoped closed-form vectors. -/
theorem texMatchingRegularQuadricSelectedEffectiveTailLimitAgreementData_of_dialMem
    {L d r : Nat} (hr_pos : 0 < r)
    {θ θ' : Params (L + 1) d}
    (D : IDLData (L + 1) d r θ θ')
    {O : Set (ProbePair d)}
    ...
    TexMatchingRegularQuadricSelectedEffectiveTailLimitAgreementData
      signRegion T N analytic where
  selected_limit_eq := by
    intro p hp t ht
    let δ :=
      texMatchingRegularQuadricDialPathData
        (L := L) (d := d) (r := r) signRegion T N p hp t ht
    rcases D.path_agreement (DialPathData.probe δ)
        (by simpa [δ] using hdial_mem p hp t ht) with
      ⟨T0, hobs⟩
    let f : ℝ -> Fin d -> ℝ := fun τ =>
      Frec r (Params.tail θ)
        (texMatchingUnprimedDialEffectiveTailPath
          (L := L) (d := d) (r := r) (θ := θ) (θ' := θ') δ τ).1
        (texMatchingUnprimedDialEffectiveTailPath
          (L := L) (d := d) (r := r) (θ := θ) (θ' := θ') δ τ).2 τ
    let g : ℝ -> Fin d -> ℝ := fun τ =>
      Frec r (Params.tail θ')
        (texMatchingPrimedDialEffectiveTailPath
          (L := L) (d := d) (r := r) (θ' := θ') δ τ).1
        (texMatchingPrimedDialEffectiveTailPath
          (L := L) (d := d) (r := r) (θ' := θ') δ τ).2 τ
    have htail_eq : f =ᶠ[Filter.atTop] g := by
      filter_upwards [Filter.Ioi_mem_atTop T0] with τ hτ
      have hunprimed_full :
          Frec r θ (DialPathData.probe δ τ).1 (DialPathData.probe δ τ).2 τ =
            f τ := by
        simpa [f, texMatchingUnprimedDialEffectiveTailPath,
          texMatchingUnprimedDialEffectiveTailPoint] using
          Frec_succ_paramsFirstLayerEffectivePoint r θ
            (DialPathData.probe δ τ).1 (DialPathData.probe δ τ).2 τ
      have hprimed_full :
          Frec r θ' (DialPathData.probe δ τ).1 (DialPathData.probe δ τ).2 τ =
            g τ := by
        simpa [g, texMatchingPrimedDialEffectiveTailPath,
          texMatchingPrimedDialEffectiveTailPoint] using
          Frec_succ_paramsFirstLayerEffectivePoint r θ'
            (DialPathData.probe δ τ).1 (DialPathData.probe δ τ).2 τ
      have hfull_eq :
          Frec r θ (DialPathData.probe δ τ).1 (DialPathData.probe δ τ).2 τ =
            Frec r θ' (DialPathData.probe δ τ).1 (DialPathData.probe δ τ).2 τ :=
        texMatching_frec_eq_on_path_of_observableAgreement
          (L := L + 1) (d := d) (r := r) hr_pos hobs τ hτ
      exact hunprimed_full.symm.trans (hfull_eq.trans hprimed_full)
    have hunprimed_tendsto :
        Filter.Tendsto f Filter.atTop
          (nhds (analytic.unprimedLimit p hp t ht)) := by
      simpa [f] using analytic.unprimed_tendsto p hp t ht
    have hprimed_tendsto :
        Filter.Tendsto g Filter.atTop
          (nhds (analytic.primedLimit p hp t ht)) := by
      simpa [g] using analytic.primed_tendsto p hp t ht
    have hprimed_as_unprimed :
        Filter.Tendsto f Filter.atTop
          (nhds (analytic.primedLimit p hp t ht)) := by
      intro s hs
      have hprimed_mem := hprimed_tendsto hs
      rw [Filter.mem_map] at hprimed_mem ⊢
      filter_upwards [htail_eq, hprimed_mem] with τ heq hmem
      simpa [heq] using hmem
    exact tendsto_nhds_unique hunprimed_tendsto hprimed_as_unprimed
...
/-- Source-path realization data for the selected regular-quadric dials when the
current path class is a swept `realizedTailPathSet`.

This is the local-patch-independent version of
`TexMatchingLocalPatchRegularQuadricDialRealizedTailMemData`: the selected
`signRegion/T/N` dials are fixed explicitly, and each dial is supplied by an available
source path plus a realization witness. -/
structure TexMatchingRegularQuadricDialRealizedTailMemData
    {L d r : Nat}
    {θ θ' : Params (L + 1) d}
    (_D : IDLData (L + 1) d r θ θ')
    {O : Set (ProbePair d)}
  ...
  source :
    ∀ (p : ProbePair d), p ∈ N.Uq -> ∀ (t : ℝ), t ∈ N.J -> ProbePath d
  source_mem :
...
/-- The selected source/realization package implies direct selected membership in the
swept realized-tail path class. -/
theorem texMatchingRegularQuadricDialRealizedTailMemObligation_of_data
    {L d r : Nat}
    {θ θ' : Params (L + 1) d}
    (D : IDLData (L + 1) d r θ θ')
    {O : Set (ProbePair d)}
...
/-- Direct selected realized-tail membership from selected membership in `D.Paths`
when the current path class is definitionally the swept realized-tail set. -/
theorem texMatchingRegularQuadricDialRealizedTailMemObligation_of_dialMem
    {L d r : Nat}
    {θ θ' : Params (L + 1) d}
    (D : IDLData (L + 1) d r θ θ')
    {O : Set (ProbePair d)}
    ...
    TexMatchingRegularQuadricDialRealizedTailMemObligation
      D signRegion T N θfull FullPaths := by
  intro p hp t ht
  simpa [hPaths] using hdial_mem p hp t ht
...
/-- Generic selected regular-quadric membership from source/realization data for a
swept realized-tail path class. -/
theorem texMatchingRegularQuadricDialMemObligation_of_realizedTailPathSet
    {L d r : Nat}
    {θ θ' : Params (L + 1) d}
    (D : IDLData (L + 1) d r θ θ')
    {O : Set (ProbePair d)}
  ...
  texMatchingRegularQuadricDialMemObligation_of_realizedTailMem
    D signRegion T N θfull FullPaths hPaths
    (texMatchingRegularQuadricDialRealizedTailMemObligation_of_data
      D signRegion T N θfull FullPaths realized)
...
/-- Selected effective-tail membership in realized-tail path classes from direct
selected-dial membership in the swept path class.  This removes the intermediate
`TexMatchingRegularQuadricDialMemObligation` from recursive matching callers. -/
theorem texMatchingRegularQuadricLowerDepthEffectiveTailPathMemData_of_realizedTailMem
    {L d r : Nat}
    {θ θ' : Params (L + 1) d}
    (D : IDLData (L + 1) d r θ θ')
    {O : Set (ProbePair d)}
...
/-- Realized-tail lower-depth provider from direct selected-dial membership in the
swept path class.  This compiles direct realized-tail dial membership into both the
current dial membership and the selected effective-tail lower-depth path membership. -/
def texMatchingRegularQuadricLowerDepthTailPathFrecProviderData_of_realizedTailMem
    {L d r : Nat}
    {θ θ' : Params (L + 1) d}
    (D : IDLData (L + 1) d r θ θ')
    {O : Set (ProbePair d)}
  ...
  texMatchingRegularQuadricLowerDepthTailPathFrecProviderData_of_parts
    analytic
    (texMatchingRegularQuadricLowerDepthEffectiveTailPathMemData_of_realizedTailMem
      D signRegion T N θfull FullPaths hPaths realized)
    limits
...
/-- Local-patch realized-tail membership from the smaller generic selected-dial
membership surface over the already constructed `signRegion/T/N`. -/
theorem texMatchingLocalPatchRegularQuadricDialRealizedTailMemObligation_of_selectedDial
    {L d r : Nat} (hL : 0 < L) (hr : 2 <= r)
    {θ θ' : Params (L + 1) d}
    (hstep :
      TexGenericStepClauses L d (TexGeneric L d)
    ...
    TexMatchingLocalPatchRegularQuadricDialRealizedTailMemObligation
      hL hr hstep endpoint D localRegion θfull FullPaths := by
  dsimp [TexMatchingLocalPatchRegularQuadricDialRealizedTailMemObligation,
    TexMatchingRegularQuadricDialRealizedTailMemObligation] at realized ⊢
  exact realized

/-- Local-patch realized-tail membership from selected membership in `D.Paths` over
the already constructed `signRegion/T/N`, using
`D.Paths = realizedTailPathSet r θfull FullPaths`. -/
theorem texMatchingLocalPatchRegularQuadricDialRealizedTailMemObligation_of_selectedDialMem
    {L d r : Nat} (hL : 0 < L) (hr : 2 <= r)
    {θ θ' : Params (L + 1) d}
    (hstep :
      TexGenericStepClauses L d (TexGeneric L d)
    ...
    TexMatchingLocalPatchRegularQuadricDialRealizedTailMemObligation
      hL hr hstep endpoint D localRegion θfull FullPaths := by
  refine
    texMatchingLocalPatchRegularQuadricDialRealizedTailMemObligation_of_selectedDial
      (L := L) (d := d) (r := r) hL hr hstep endpoint D localRegion
      θfull FullPaths ?_
  exact
    texMatchingRegularQuadricDialRealizedTailMemObligation_of_dialMem
      D
      (texMatchingSignRegionData_of_region
        (L := L) (d := d) (r := r) hr hstep endpoint D
        (texRegionConstructionData_of_IDLData_of_localPatch
      ...
      (texMatchingProductNeighborhoodData_of_region
        ...
        (texRegionConstructionData_of_IDLData_of_localPatch
          (L := L) (d := d) (r := r) hL hr hstep endpoint D localRegion))
      θfull FullPaths hPaths hdial_mem

/-- Local-patch realized-tail membership from the local selected-dial membership
obligation and the swept-path-class identity. -/
theorem texMatchingLocalPatchRegularQuadricDialRealizedTailMemObligation_of_dialMem
    {L d r : Nat} (hL : 0 < L) (hr : 2 <= r)
    {θ θ' : Params (L + 1) d}
    (hstep :
      TexGenericStepClauses L d (TexGeneric L d)
    ...
    TexMatchingLocalPatchRegularQuadricDialRealizedTailMemObligation
      hL hr hstep endpoint D localRegion θfull FullPaths := by
  exact
    texMatchingLocalPatchRegularQuadricDialRealizedTailMemObligation_of_selectedDialMem
      (L := L) (d := d) (r := r) hL hr hstep endpoint D localRegion
      θfull FullPaths hPaths
      (by
        dsimp [TexMatchingLocalPatchRegularQuadricDialMemObligation,
          TexMatchingRegularQuadricDialMemObligation] at hdial_mem ⊢
        exact hdial_mem)
...
/-- Local-patch spelling of direct equality between the selected unprimed and primed
effective-tail limit functions. -/
abbrev TexMatchingLocalPatchRegularQuadricSelectedEffectiveTailLimitAgreementData
    {L d r : Nat} (hL : 0 < L) (hr : 2 <= r)
    {θ θ' : Params (L + 1) d}
    (hstep :
      TexGenericStepClauses L d (TexGeneric L d)
  ...
  let N :=
    texMatchingProductNeighborhoodData_of_region
      (L := L) (d := d) (r := r) hr hstep endpoint D region
  TexMatchingRegularQuadricSelectedEffectiveTailLimitAgreementData
    signRegion T N analytic

/-- Local selected effective-tail limit equality from selected dial membership and
`D.path_agreement`. -/
theorem
    texMatchingLocalPatchRegularQuadricSelectedEffectiveTailLimitAgreementData_of_dialMem
    {L d r : Nat} (hL : 0 < L) (hr : 2 <= r)
    {θ θ' : Params (L + 1) d}
    (hstep :
      TexGenericStepClauses L d (TexGeneric L d)
    ...
    (hdial_mem :
      TexMatchingLocalPatchRegularQuadricDialMemObligation
        hL hr hstep endpoint D localRegion) :
    TexMatchingLocalPatchRegularQuadricSelectedEffectiveTailLimitAgreementData
  ...
  dsimp [TexMatchingLocalPatchRegularQuadricSelectedEffectiveTailFrecAnalyticData,
    TexMatchingLocalPatchRegularQuadricSelectedEffectiveTailLimitAgreementData,
    TexMatchingLocalPatchRegularQuadricDialMemObligation] at analytic hdial_mem ⊢
  exact
    texMatchingRegularQuadricSelectedEffectiveTailLimitAgreementData_of_dialMem
      (L := L) (d := d) (r := r) (by omega) D analytic hdial_mem
...
/-- Local-patch selected effective-tail convergence data directly from closed-recursion
limits. -/
noncomputable def
    texMatchingLocalPatchRegularQuadricSelectedEffectiveTailFrecAnalyticData_of_closedRecursionLimits
    {L d r : Nat} (hL : 0 < L) (hr : 2 <= r)
    {θ θ' : Params (L + 1) d}
    (hstep :
      TexGenericStepClauses L d (TexGeneric L d)
    ...
    TexMatchingLocalPatchRegularQuadricSelectedEffectiveTailFrecAnalyticData
      hL hr hstep endpoint D localRegion := by
  dsimp [TexMatchingLocalPatchRegularQuadricSelectedEffectiveTailFrecAnalyticData,
    TexMatchingLocalPatchRegularQuadricClosedRecursionLimitObligation] at hlimits ⊢
  exact
    texMatchingRegularQuadricSelectedEffectiveTailFrecAnalyticData_of_closedRecursionLimits
      hlimits
...
/-- Local-patch closed-recursion limits from the direct unprimed and primed `Frec`
asymptotics.

This is the acyclic reduction for the currently exposed closed-recursion field: it does
not use a matched-tail provider or an already constructed `FirstLayerMatchedData θ θ'`. -/
theorem texMatchingLocalPatchRegularQuadricClosedRecursionLimitObligation_of_frecAsymptotics
    {L d r : Nat} (hL : 0 < L) (hr : 2 <= r)
    {θ θ' : Params (L + 1) d}
    (hstep :
      TexGenericStepClauses L d (TexGeneric L d)
  ...
  dsimp [TexMatchingLocalPatchUnprimedSaturatedFrecAsymptoticObligation,
    TexMatchingLocalPatchPrimedTelescopedFrecAsymptoticObligation,
    TexMatchingLocalPatchRegularQuadricClosedRecursionLimitObligation] at hunprimed hprimed ⊢
  exact
    texMatchingRegularQuadricClosedRecursionLimitObligation_of_frecAsymptotics
      (L := L) (d := d) (r := r)
      (texMatchingSignRegionData_of_region
...
/-- Local-patch closed-recursion limits from the pointwise dial-level `Frec`
asymptotics. -/
theorem texMatchingLocalPatchRegularQuadricClosedRecursionLimitObligation_of_dialAsymptotics
    {L d r : Nat} (hL : 0 < L) (hr : 2 <= r)
    {θ θ' : Params (L + 1) d}
    (hstep :
      TexGenericStepClauses L d (TexGeneric L d)
    ...
    TexMatchingLocalPatchRegularQuadricClosedRecursionLimitObligation
      hL hr hstep endpoint D localRegion := by
  exact
    texMatchingLocalPatchRegularQuadricClosedRecursionLimitObligation_of_frecAsymptotics
      (L := L) (d := d) (r := r) hL hr hstep endpoint D localRegion
      (texMatchingLocalPatchUnprimedSaturatedFrecAsymptoticObligation_of_dialFrecTendsto
        (L := L) (d := d) (r := r) hL hr hstep endpoint D localRegion hunprimed)
      (texMatchingLocalPatchPrimedTelescopedFrecAsymptoticObligation_of_dialAsymptotic
        (L := L) (d := d) (r := r) hL hr hstep endpoint D localRegion hprimed)

/-- Local-patch closed-recursion limits from the peeled effective-tail `Frec`
asymptotics.  This is the closed-recursion replacement surface that keeps the exact
first-layer peel outside the remaining analytic hypotheses. -/
theorem texMatchingLocalPatchRegularQuadricClosedRecursionLimitObligation_of_effectiveTail
    {L d r : Nat} (hL : 0 < L) (hr : 2 <= r)
    {θ θ' : Params (L + 1) d}
    (hstep :
      TexGenericStepClauses L d (TexGeneric L d)
    ...
    TexMatchingLocalPatchRegularQuadricClosedRecursionLimitObligation
      hL hr hstep endpoint D localRegion :=
  texMatchingLocalPatchRegularQuadricClosedRecursionLimitObligation_of_dialAsymptotics
    (L := L) (d := d) (r := r) hL hr hstep endpoint D localRegion
    (texMatchingLocalPatchUnprimedSaturatedDialFrecTendsto_of_effectiveTail
      (L := L) (d := d) (r := r) hL hr hstep endpoint D localRegion hunprimed)
    (texMatchingLocalPatchPrimedTelescopedFrecDialAsymptotic_of_effectiveTail
      (L := L) (d := d) (r := r) hL hr hstep endpoint D localRegion hprimed)

/-- Local-patch closed-recursion limits from lower-depth effective-tail path
asymptotics. -/
theorem texMatchingLocalPatchRegularQuadricClosedRecursionLimitObligation_of_tailPathAsymptotics
    {L d r : Nat} (hL : 0 < L) (hr : 2 <= r)
    {θ θ' : Params (L + 1) d}
    (hstep :
      TexGenericStepClauses L d (TexGeneric L d)
    ...
    TexMatchingLocalPatchRegularQuadricClosedRecursionLimitObligation
      hL hr hstep endpoint D localRegion :=
  texMatchingLocalPatchRegularQuadricClosedRecursionLimitObligation_of_effectiveTail
    (L := L) (d := d) (r := r) hL hr hstep endpoint D localRegion
    (texMatchingLocalPatchUnprimedSaturatedEffectiveTailFrecAsymptotic_of_tailPath
      (L := L) (d := d) (r := r) hL hr hstep endpoint D localRegion hunprimed)
    (texMatchingLocalPatchPrimedTelescopedEffectiveTailFrecAsymptotic_of_tailPath
      (L := L) (d := d) (r := r) hL hr hstep endpoint D localRegion hprimed)

/-- The selected-dial matched-tail provider gives the local-patch closed-recursion
limits for the canonical regular-quadric dials.

This is the local-patch spelling of
`texMatchingRegularQuadricClosedRecursionLimitObligation_of_matchedTailPathProvider`:
path membership comes from the provider, observable agreement is consumed from
`D.path_agreement`, and the lower-depth matched-tail asymptotic supplies the two peeled
`Frec` limits. -/
theorem texMatchingLocalPatchRegularQuadricClosedRecursionLimitObligation_of_matchedTailPathProvider
    {L d r : Nat} (hL : 0 < L) (hr : 2 <= r)
    {θ θ' : Params (L + 1) d}
    (hstep :
      TexGenericStepClauses L d (TexGeneric L d)
    ...
    TexMatchingLocalPatchRegularQuadricClosedRecursionLimitObligation
      hL hr hstep endpoint D localRegion := by
  dsimp [TexMatchingLocalPatchMatchedTailPathFrecProviderData,
    TexMatchingLocalPatchRegularQuadricClosedRecursionLimitObligation] at provider ⊢
  exact
    texMatchingRegularQuadricClosedRecursionLimitObligation_of_matchedTailPathProvider
      (L := L) (d := d) (r := r) (by omega) D
      (texMatchingSignRegionData_of_region
...
/-- Restrict the global local-patch unprimed tail-path field to the selected
regular-quadric product-patch dials. -/
theorem texMatchingLocalPatchRegularQuadricUnprimedSaturatedTailPathFrecAsymptotic_of_tailPath
    {L d r : Nat} (hL : 0 < L) (hr : 2 <= r)
    {θ θ' : Params (L + 1) d}
    (hstep :
      TexGenericStepClauses L d (TexGeneric L d)
    ...
    TexMatchingLocalPatchRegularQuadricUnprimedSaturatedTailPathFrecAsymptotic
      hL hr hstep endpoint D localRegion := by
  dsimp [TexMatchingLocalPatchUnprimedSaturatedTailPathFrecAsymptotic,
    TexMatchingLocalPatchRegularQuadricUnprimedSaturatedTailPathFrecAsymptotic] at htail ⊢
  intro p hp t ht
  let δ :=
    texMatchingRegularQuadricDialPathData
      (L := L) (d := d) (r := r)
      ...
      (texMatchingProductNeighborhoodData_of_region
        ...
        (texRegionConstructionData_of_IDLData_of_localPatch
          (L := L) (d := d) (r := r) hL hr hstep endpoint D localRegion))
      p hp t ht
  have hmem : (δ.base, δ.t) ∈
      (texTrichotomyConstructionData_of_signRegion
        (L := L) (d := d) (r := r) hr hstep endpoint D
        (texMatchingSignRegionData_of_region
          (L := L) (d := d) (r := r) hr hstep endpoint D
          (texRegionConstructionData_of_IDLData_of_localPatch
            (L := L) (d := d) (r := r) hL hr hstep endpoint D localRegion))).Ustar := by
    simpa [δ] using
      texMatchingRegularQuadricDialPathData_mem_Ustar
        (L := L) (d := d) (r := r)
        (texMatchingSignRegionData_of_region
  ...
  have htarget : δ.t = t := by
    simp [δ]
  simpa [δ, hbase, htarget] using htail δ hmem

/-- Restrict the global local-patch primed tail-path field to the selected
regular-quadric product-patch dials. -/
theorem texMatchingLocalPatchRegularQuadricPrimedTelescopedTailPathFrecAsymptotic_of_tailPath
    {L d r : Nat} (hL : 0 < L) (hr : 2 <= r)
    {θ θ' : Params (L + 1) d}
    (hstep :
      TexGenericStepClauses L d (TexGeneric L d)
    ...
    TexMatchingLocalPatchRegularQuadricPrimedTelescopedTailPathFrecAsymptotic
      hL hr hstep endpoint D localRegion := by
  dsimp [TexMatchingLocalPatchPrimedTelescopedTailPathFrecAsymptotic,
    TexMatchingLocalPatchRegularQuadricPrimedTelescopedTailPathFrecAsymptotic] at htail ⊢
  intro p hp t ht
  let δ :=
    texMatchingRegularQuadricDialPathData
      (L := L) (d := d) (r := r)
...
/-- Restrict the global local-patch unprimed full `Frec` obligation to the selected
regular-quadric product-patch dials. -/
theorem
    texMatchingLocalPatchRegularQuadricUnprimedSaturatedFrecAsymptoticObligation_of_global
    {L d r : Nat} (hL : 0 < L) (hr : 2 <= r)
    {θ θ' : Params (L + 1) d}
    (hstep :
      TexGenericStepClauses L d (TexGeneric L d)
    ...
    TexMatchingLocalPatchRegularQuadricUnprimedSaturatedFrecAsymptoticObligation
      hL hr hstep endpoint D localRegion := by
  dsimp [TexMatchingLocalPatchUnprimedSaturatedFrecAsymptoticObligation] at hglobal
  dsimp [TexMatchingLocalPatchRegularQuadricUnprimedSaturatedFrecAsymptoticObligation]
  intro p hp t ht
  let region :=
    texRegionConstructionData_of_IDLData_of_localPatch
      (L := L) (d := d) (r := r) hL hr hstep endpoint D localRegion
  ...
  have htarget : δ.t = t := by
    simp [δ]
  simpa [region, signRegion, T, N, δ, hbase, htarget] using
    hglobal.tendsto δ hmem

/-- Restrict the global local-patch primed full `Frec` obligation to the selected
regular-quadric product-patch dials. -/
theorem
    texMatchingLocalPatchRegularQuadricPrimedTelescopedFrecAsymptoticObligation_of_global
    {L d r : Nat} (hL : 0 < L) (hr : 2 <= r)
    {θ θ' : Params (L + 1) d}
    (hstep :
      TexGenericStepClauses L d (TexGeneric L d)
    ...
    TexMatchingLocalPatchRegularQuadricPrimedTelescopedFrecAsymptoticObligation
      hL hr hstep endpoint D localRegion := by
  dsimp [TexMatchingLocalPatchPrimedTelescopedFrecAsymptoticObligation] at hglobal
  dsimp [TexMatchingLocalPatchRegularQuadricPrimedTelescopedFrecAsymptoticObligation]
  intro p hp t ht
  let region :=
    texRegionConstructionData_of_IDLData_of_localPatch
      (L := L) (d := d) (r := r) hL hr hstep endpoint D localRegion
...
/-- Local-patch selected full primed `Frec` field from the smaller generic selected-dial
regular-quadric theorem.

This removes the local-region constructor parameters from the analytic statement a
caller has to prove: the hypothesis is only the pointwise tendsto theorem over the
already constructed `signRegion/T/N`. -/
theorem
    texMatchingLocalPatchRegularQuadricPrimedTelescopedFrecAsymptoticObligation_of_selectedDial
    {L d r : Nat} (hL : 0 < L) (hr : 2 <= r)
    {θ θ' : Params (L + 1) d}
    (hstep :
      TexGenericStepClauses L d (TexGeneric L d)
    ...
    TexMatchingLocalPatchRegularQuadricPrimedTelescopedFrecAsymptoticObligation
      hL hr hstep endpoint D localRegion := by
  dsimp [TexMatchingLocalPatchRegularQuadricPrimedTelescopedFrecAsymptoticObligation,
    TexMatchingRegularQuadricPrimedTelescopedFrecAsymptotic] at hprimed ⊢
  exact hprimed
...
/-- Local-patch selected full unprimed `Frec` field from the smaller generic
selected-dial regular-quadric theorem. -/
theorem
    texMatchingLocalPatchRegularQuadricUnprimedSaturatedFrecAsymptoticObligation_of_selectedDial
    {L d r : Nat} (hL : 0 < L) (hr : 2 <= r)
    {θ θ' : Params (L + 1) d}
    (hstep :
      TexGenericStepClauses L d (TexGeneric L d)
    ...
    TexMatchingLocalPatchRegularQuadricUnprimedSaturatedFrecAsymptoticObligation
      hL hr hstep endpoint D localRegion := by
  dsimp [TexMatchingLocalPatchRegularQuadricUnprimedSaturatedFrecAsymptoticObligation,
    TexMatchingRegularQuadricUnprimedSaturatedFrecAsymptotic] at hunprimed ⊢
  exact hunprimed
...
/-- A bundled lower-depth provider supplies the local selected closed-recursion limit
obligation.  This exposes the active closed-recursion field directly from lower-depth
path-class convergence, selected path membership, and selected limit identification. -/
theorem texMatchingLocalPatchRegularQuadricClosedRecursionLimitObligation_of_lowerDepthProvider
    {L d r : Nat} (hL : 0 < L) (hr : 2 <= r)
    {θ θ' : Params (L + 1) d}
    (hstep :
      TexGenericStepClauses L d (TexGeneric L d)
  ...
  dsimp [TexMatchingLocalPatchRegularQuadricClosedRecursionLimitObligation]
    at provider ⊢
  exact
    texMatchingRegularQuadricClosedRecursionLimitObligation_of_lowerDepthProvider
      provider
...
/-- Universal-path local-patch region provider from selected closed-recursion limits. -/
noncomputable def
    texMatchingLocalPatchProvider_of_paths_univ_closedLimits
    {L d r : Nat} (hL : 0 < L) (hr : 2 <= r)
    {θ θ' : Params (L + 1) d}
    (hstep :
      TexGenericStepClauses L d (TexGeneric L d)
...
/-- Recursive swept-path local-patch region provider from direct realized-tail
membership and selected closed-recursion limits. -/
noncomputable def
    texMatchingLocalPatchProvider_of_realizedTailMem_closedLimits
    {L d r : Nat} (hL : 0 < L) (hr : 2 <= r)
    {θ θ' : Params (L + 1) d}
    (hstep :
      TexGenericStepClauses L d (TexGeneric L d)
...
/-- Universal-path local-patch region provider from selected lower-depth tail-path
limits.  This avoids the stronger global tail-path fields over all of `T.Ustar`. -/
noncomputable def texMatchingRegionProviderFromLocalPatchData_of_paths_univ_regularQuadricTailPath
    {L d r : Nat} (hL : 0 < L) (hr : 2 <= r)
    {θ θ' : Params (L + 1) d}
    (hstep :
      TexGenericStepClauses L d (TexGeneric L d)
...
/-- Recursive swept-path local-patch region provider from direct realized-tail
membership and selected lower-depth tail-path limits. -/
noncomputable def
    texMatchingRegionProviderFromLocalPatchData_of_realizedTailMem_regularQuadricTailPath
    {L d r : Nat} (hL : 0 < L) (hr : 2 <= r)
    {θ θ' : Params (L + 1) d}
    (hstep :
      TexGenericStepClauses L d (TexGeneric L d)
...
/-- Local-patch realized-tail limit identification from selected lower-depth
tail-path asymptotics.

This is the realized-tail specialization of
`texMatchingRegularQuadricLowerDepthLimitIdentificationData_of_tailPathAsymptotics`:
the selected effective-tail path membership is derived from direct canonical dial
membership in the swept `realizedTailPathSet`, so callers do not need to mention the
generic lower-depth path-membership or limit-identification records. -/
theorem
    texMatchingLocalPatchRegularQuadricLowerDepthLimitIdentificationData_of_realizedTailMem_tailPath
    {L d r : Nat} (hL : 0 < L) (hr : 2 <= r)
    {θ θ' : Params (L + 1) d}
    (hstep :
      TexGenericStepClauses L d (TexGeneric L d)
  ...
  have realized' :
      TexMatchingRegularQuadricDialRealizedTailMemObligation
        D signRegion T N θfull FullPaths := by
    dsimp [TexMatchingRegularQuadricDialRealizedTailMemObligation,
      TexMatchingLocalPatchRegularQuadricDialRealizedTailMemObligation]
        at realized ⊢
    exact realized
  have hunprimed' :
...
/-- Realized-tail provider constructor with the limit-identification record derived
from selected lower-depth tail-path asymptotics.

Compared with
`texMatchingLocalPatchRegularQuadricFrecProviderData_of_realizedTailMem_lowerDepthTailPath`,
the explicit `TexMatchingRegularQuadricLowerDepthLimitIdentificationData` input is
replaced by the two selected lower-depth tail-path convergence statements used to
identify the abstract recursive limits. -/
def texMatchingLocalPatchRegularQuadricFrecProviderData_of_realizedTailMem_lowerDepthTailPath_of_tailPath
    {L d r : Nat} (hL : 0 < L) (hr : 2 <= r)
    {θ θ' : Params (L + 1) d}
    (hstep :
      TexGenericStepClauses L d (TexGeneric L d)
  ...
  texMatchingLocalPatchRegularQuadricFrecProviderData_of_realizedTailMem_lowerDepthTailPath
    ...
    (texMatchingLocalPatchRegularQuadricLowerDepthLimitIdentificationData_of_realizedTailMem_tailPath
      (L := L) (d := d) (r := r) hL hr hstep endpoint D localRegion
      θfull FullPaths hPaths realized analytic hunprimed hprimed)

/-- Local-patch realized-tail limit identification from selected peeled effective-tail
asymptotics.

This is the same realized-tail specialization as
`texMatchingLocalPatchRegularQuadricLowerDepthLimitIdentificationData_of_realizedTailMem_tailPath`,
but its endpoint inputs are the smaller peeled effective-tail fields. -/
theorem
    texMatchingLocalPatchRegularQuadricLowerDepthLimitIdentificationData_of_realizedTailMem_effectiveTail
    {L d r : Nat} (hL : 0 < L) (hr : 2 <= r)
    {θ θ' : Params (L + 1) d}
    (hstep :
      TexGenericStepClauses L d (TexGeneric L d)
  ...
  have hunprimed' :
      TexMatchingRegularQuadricUnprimedSaturatedEffectiveTailFrecAsymptotic
        signRegion T N S := by
    dsimp [TexMatchingRegularQuadricUnprimedSaturatedEffectiveTailFrecAsymptotic,
      TexMatchingLocalPatchRegularQuadricUnprimedSaturatedEffectiveTailFrecAsymptotic]
        at hunprimed ⊢
    exact hunprimed
  have hprimed' :
      TexMatchingRegularQuadricPrimedTelescopedEffectiveTailFrecAsymptotic
        signRegion T N := by
    dsimp [TexMatchingRegularQuadricPrimedTelescopedEffectiveTailFrecAsymptotic,
      TexMatchingLocalPatchRegularQuadricPrimedTelescopedEffectiveTailFrecAsymptotic]
        at hprimed ⊢
    exact hprimed
  exact
...
/-- Realized-tail provider constructor with the limit-identification record derived
from selected peeled effective-tail asymptotics. -/
def texMatchingLocalPatchRegularQuadricFrecProviderData_of_realizedTailMem_lowerDepthTailPath_of_effectiveTail
    {L d r : Nat} (hL : 0 < L) (hr : 2 <= r)
    {θ θ' : Params (L + 1) d}
    (hstep :
      TexGenericStepClauses L d (TexGeneric L d)
...
/-- Universal-path constructor for the selected full local-patch Frec provider.  Path
membership is supplied by `D.Paths = Set.univ`; the two selected regular-quadric Frec
asymptotics remain explicit analytic hypotheses. -/
def texMatchingLocalPatchRegularQuadricFrecProviderData_of_paths_univ_of_dialAsymptotics
    {L d r : Nat} (hL : 0 < L) (hr : 2 <= r)
    {θ θ' : Params (L + 1) d}
    (hstep :
      TexGenericStepClauses L d (TexGeneric L d)
...
/-- Identify the selected effective-tail analytic limits using the stored
closed-recursion limits. -/
theorem selectedLimitIdentification
    {L d r : Nat} {hL : 0 < L} {hr : 2 <= r}
    {θ θ' : Params (L + 1) d}
    {hstep :
      TexGenericStepClauses L d (TexGeneric L d)
    ...
    (P :
      TexMatchingLocalPatchRegularQuadricSelectedTailClosedRecursionSourceData
        hL hr hstep endpoint D localRegion) :
    TexMatchingLocalPatchRegularQuadricSelectedEffectiveTailLimitIdentificationData
      hL hr hstep endpoint D localRegion P.selectedEffectiveTail :=
  texMatchingLocalPatchRegularQuadricSelectedEffectiveTailLimitIdentificationData_of_closedRecursionLimits
    (L := L) (d := d) (r := r) hL hr hstep endpoint D localRegion
    P.selectedEffectiveTail P.closedRecursionLimits

/-- Project the selected closed-recursion limits from the path-restricted source. -/
...
/-- Construct the selected-tail source from selected dial membership.

The dial membership is used only to prove membership of the selected effective-tail
paths in the two realized lower-depth path classes. -/
def
    texMatchingLocalPatchRegularQuadricSelectedTailClosedRecursionSourceData_of_dialMem_lowerDepthTailPath
    {L d r : Nat} (hL : 0 < L) (hr : 2 <= r)
    {θ θ' : Params (L + 1) d}
    (hstep :
      TexGenericStepClauses L d (TexGeneric L d)
    ...
    TexMatchingLocalPatchRegularQuadricSelectedTailClosedRecursionSourceData
      hL hr hstep endpoint D localRegion where
  lowerDepthAnalytic := analytic
  effectiveTailPathMem := by
    let region :=
      texRegionConstructionData_of_IDLData_of_localPatch
        (L := L) (d := d) (r := r) hL hr hstep endpoint D localRegion
    let signRegion :=
    ...
    have hdial_mem' :
      ...
      dsimp [TexMatchingLocalPatchRegularQuadricDialMemObligation]
        at hdial_mem
      simpa [region, signRegion, T, N] using hdial_mem
    exact
      texMatchingRegularQuadricLowerDepthEffectiveTailPathMemData_of_realizedTailPathSet
        D signRegion T N hdial_mem'
  closedRecursionLimits := hlimits

/-- Compile the selected-tail source plus selected dial membership to the exposed
local-patch `Frec` provider. -/
noncomputable def
    texMatchingLocalPatchRegularQuadricFrecProviderData_of_selectedTailClosedRecursionSourceData
    {L d r : Nat} (hL : 0 < L) (hr : 2 <= r)
    {θ θ' : Params (L + 1) d}
    (hstep :
      TexGenericStepClauses L d (TexGeneric L d)
    ...
    TexMatchingLocalPatchRegularQuadricFrecProviderData
      hL hr hstep endpoint D localRegion :=
  let selected := P.selectedEffectiveTail
  let selectedLimits := P.selectedLimitIdentification
  texMatchingLocalPatchRegularQuadricFrecProviderData_of_effectiveTailData
    (L := L) (d := d) (r := r) hL hr hstep endpoint D localRegion
    { dial_mem := hdial_mem
      unprimed :=
...
/-- Universal-path replacement constructor for R5.

The hypothesis `D.Paths = Set.univ` is used only to build selected dial membership.
The lower-depth analytic hypothesis is path-restricted to the selected effective-tail
classes, not universal `∀ ProbePath` convergence. -/
noncomputable def
    texMatchingLocalPatchRegularQuadricFrecProviderData_of_paths_univ_selectedTailClosedRecursionSourceData
    {L d r : Nat} (hL : 0 < L) (hr : 2 <= r)
    {θ θ' : Params (L + 1) d}
    (hstep :
      TexGenericStepClauses L d (TexGeneric L d)
...
/-- Universal-path replacement constructor from path-restricted lower-depth analytic
data and selected closed-recursion limits.

This is the direct top-level selected-tail route: no
`TexMatchingRegularQuadricLowerDepthUnivTailPathFrecAnalyticData` input appears. -/
noncomputable def
    texMatchingLocalPatchRegularQuadricFrecProviderData_of_paths_univ_lowerDepthSelectedTail_closedRecursionLimits
    {L d r : Nat} (hL : 0 < L) (hr : 2 <= r)
    {θ θ' : Params (L + 1) d}
    (hstep :
      TexGenericStepClauses L d (TexGeneric L d)
...
/-- Identify selected effective-tail limits using the stored closed-recursion limits. -/
theorem selectedLimitIdentification
    {L d r : Nat}
    {θ θ' : Params (L + 1) d}
    {D : IDLData (L + 1) d r θ θ'}
    ...
    (P :
      TexMatchingRegularQuadricSelectedTailClosedRecursionSourceData
        D signRegion T N S) :
    TexMatchingRegularQuadricSelectedEffectiveTailLimitIdentificationData
      signRegion T N S P.selectedEffectiveTail :=
  texMatchingRegularQuadricSelectedEffectiveTailLimitIdentificationData_of_closedRecursionLimits
    P.selectedEffectiveTail P.closedRecursionLimits
...
/-- Build the explicit selected-tail source from selected dial membership. -/
def texMatchingRegularQuadricSelectedTailClosedRecursionSourceData_of_dialMem_lowerDepthTailPath
    {L d r : Nat}
    {θ θ' : Params (L + 1) d}
    (D : IDLData (L + 1) d r θ θ')
    {O : Set (ProbePair d)}
...
/-- Universal-path source constructor with lower-depth data restricted to the selected
realized-tail classes. -/
def texMatchingRegularQuadricSelectedTailClosedRecursionSourceData_of_paths_univ_lowerDepthTailPath
    {L d r : Nat}
    {θ θ' : Params (L + 1) d}
    (D : IDLData (L + 1) d r θ θ')
    (hPaths : D.Paths = Set.univ)
    ...
    TexMatchingRegularQuadricSelectedTailClosedRecursionSourceData
      D signRegion T N S :=
  texMatchingRegularQuadricSelectedTailClosedRecursionSourceData_of_dialMem_lowerDepthTailPath
    D signRegion T N S
    (texMatchingRegularQuadricDialMemObligation_of_paths_univ
      D hPaths signRegion T N)
    analytic hlimits

/-- Direct generic provider from universal paths, selected realized-tail lower-depth
analytic data, and explicit closed-recursion limits. -/
def texMatchingRegularQuadricLowerDepthTailPathFrecProviderData_of_paths_univ_lowerDepthSelectedTail_closedRecursionLimits
    {L d r : Nat}
    {θ θ' : Params (L + 1) d}
    (D : IDLData (L + 1) d r θ θ')
    (hPaths : D.Paths = Set.univ)
    ...
    TexMatchingRegularQuadricLowerDepthTailPathFrecProviderData
      ...
      (realizedTailPathSet r θ D.Paths)
      (realizedTailPathSet r θ' D.Paths) :=
  (texMatchingRegularQuadricSelectedTailClosedRecursionSourceData_of_paths_univ_lowerDepthTailPath
    D hPaths signRegion T N S analytic hlimits).lowerDepthProvider

/-- Builder-backed selected-tail source from canonical actual-gate data. -/
noncomputable def
    texMatchingRegularQuadricSelectedTailClosedRecursionSourceData_of_cascadeBuilder_paths_univ_lowerDepthTailPath
    {L d r : Nat}
    {θ θ' θB : Params (L + 1) d}
    (D : IDLData (L + 1) d r θ θ')
    (hPaths : D.Paths = Set.univ)
    ...
    (analytic :
      TexMatchingRegularQuadricLowerDepthTailPathFrecAnalyticData
        ...
        (realizedTailPathSet r θ' D.Paths)) :
    TexMatchingRegularQuadricSelectedTailClosedRecursionSourceData
      D signRegion (texTrichotomyConstructionData_of_cascadeBuilder B) N S :=
  texMatchingRegularQuadricSelectedTailClosedRecursionSourceData_of_paths_univ_lowerDepthTailPath
    D hPaths signRegion (texTrichotomyConstructionData_of_cascadeBuilder B) N S
    analytic
    (texMatchingRegularQuadricClosedRecursionLimitObligation_of_cascadeBuilder
      signRegion B N S hAA actual)

/-- Direct builder-backed generic provider from universal paths and selected
realized-tail lower-depth analytic data. -/
noncomputable def
    texMatchingRegularQuadricLowerDepthTailPathFrecProviderData_of_cascadeBuilder_paths_univ_lowerDepthSelectedTail
    {L d r : Nat}
    {θ θ' θB : Params (L + 1) d}
    (D : IDLData (L + 1) d r θ θ')
    (hPaths : D.Paths = Set.univ)
    ...
    TexMatchingRegularQuadricLowerDepthTailPathFrecProviderData
      ...
      (realizedTailPathSet r θ' D.Paths) :=
  (texMatchingRegularQuadricSelectedTailClosedRecursionSourceData_of_cascadeBuilder_paths_univ_lowerDepthTailPath
    D hPaths signRegion B N S hAA actual analytic).lowerDepthProvider

/-- Provider-facing selected-tail source from canonical actual-gate data. -/
noncomputable def
    texMatchingRegularQuadricSelectedTailClosedRecursionSourceData_of_inductionProvider_paths_univ_lowerDepthTailPath
    {L d r : Nat}
    {θ θ' θB : Params (L + 1) d}
    (D : IDLData (L + 1) d r θ θ')
    (hPaths : D.Paths = Set.univ)
    ...
    (analytic :
      TexMatchingRegularQuadricLowerDepthTailPathFrecAnalyticData
        ...
        (realizedTailPathSet r θ' D.Paths)) :
    TexMatchingRegularQuadricSelectedTailClosedRecursionSourceData
      D signRegion (texTrichotomyConstructionData_of_inductionProvider P) N S :=
  texMatchingRegularQuadricSelectedTailClosedRecursionSourceData_of_cascadeBuilder_paths_univ_lowerDepthTailPath
    D hPaths signRegion P.toBuilderData N S hAA actual analytic

/-- Direct provider-facing generic provider from universal paths and selected
realized-tail lower-depth analytic data. -/
noncomputable def
    texMatchingRegularQuadricLowerDepthTailPathFrecProviderData_of_inductionProvider_paths_univ_lowerDepthSelectedTail
    {L d r : Nat}
    {θ θ' θB : Params (L + 1) d}
    (D : IDLData (L + 1) d r θ θ')
    (hPaths : D.Paths = Set.univ)
    ...
    TexMatchingRegularQuadricLowerDepthTailPathFrecProviderData
      ...
      (realizedTailPathSet r θ' D.Paths) :=
  (texMatchingRegularQuadricSelectedTailClosedRecursionSourceData_of_inductionProvider_paths_univ_lowerDepthTailPath
    D hPaths signRegion P N S hAA actual analytic).lowerDepthProvider

/-- Universal-path constructor from explicit lower-depth tail-path asymptotics.

This is the most direct peeled form for recursive analytic providers: the exact first
recursion layer has already been removed, and path membership is still mechanical from
`D.Paths = Set.univ`. -/
def texMatchingLocalPatchRegularQuadricFrecProviderData_of_paths_univ_of_tailPath
    {L d r : Nat} (hL : 0 < L) (hr : 2 <= r)
    {θ θ' : Params (L + 1) d}
    (hstep :
      TexGenericStepClauses L d (TexGeneric L d)
...
/-- Local-patch universal matching input with selected endpoint tail-path asymptotics
instead of selected abstract-limit equalities.

The universal lower-depth analytic record supplies convergence to arbitrary abstract
limit functions.  The two selected tail-path asymptotics supply convergence of the
same selected paths to the saturated/telescoped endpoint vectors, so the abstract-limit
equalities are bookkeeping consequences of uniqueness of limits. -/
structure TexMatchingLocalPatchRegularQuadricUnivTailPathAsymptoticData
    {L d r : Nat} (hL : 0 < L) (hr : 2 <= r)
    {θ θ' : Params (L + 1) d}
    (hstep :
      TexGenericStepClauses L d (TexGeneric L d)
...
/-- Compile the tail-path asymptotic boundary to selected lower-depth
limit-identification data.

This is the honest proof step that removes the selected unprimed and primed
abstract-limit equality fields from the tail-path boundary: both equalities are proved
by uniqueness from the universal lower-depth convergence and the selected endpoint
tail-path limits. -/
theorem
    texMatchingLocalPatchRegularQuadricLowerDepthLimits_of_univTailPathData
    {L d r : Nat} (hL : 0 < L) (hr : 2 <= r)
    {θ θ' : Params (L + 1) d}
    (hstep :
      TexGenericStepClauses L d (TexGeneric L d)
...
/-- Compile the structured lower-depth limit-identification record to the active
universal analytic-limit boundary.

This is the narrowest local-patch compiler: the source is the existing
`TexMatchingRegularQuadricLowerDepthLimitIdentificationData` record over the universal
tail path classes.  No selected tail-path asymptotic fields and no recursive
`tailReduced` data are used. -/
def texMatchingLocalPatchRegularQuadricUnivAnalyticLimitData_of_lowerDepthLimits
    {L d r : Nat} (hL : 0 < L) (hr : 2 <= r)
    {θ θ' : Params (L + 1) d}
    (hstep :
      TexGenericStepClauses L d (TexGeneric L d)
  ...
  unprimed_limit_eq := by
    ...
    have hlimits :
        TexMatchingRegularQuadricLowerDepthLimitIdentificationData
          ...
          (texMatchingRegularQuadricLowerDepthTailPathFrecAnalyticData_of_univData
            (r := r) analytic) := by
      simpa [region, signRegion, T, N, S] using limits
    dsimp
    intro p hp t ht
    simpa [texMatchingRegularQuadricLowerDepthTailPathFrecAnalyticData_of_univData,
      texMatchingRegularQuadricLowerDepthTailPathFrecAnalyticData_of_tendsto,
      texMatchingRegularQuadricLowerDepthTailPathFrecAnalyticData_of_tailData,
      texMatchingTailPathFrecAsymptoticData_of_tendsto] using
        hlimits.unprimed_limit_eq p hp t ht
  primed_limit_eq := by
    let region :=
      texRegionConstructionData_of_IDLData_of_localPatch
        (L := L) (d := d) (r := r) hL hr hstep endpoint D localRegion
    ...
    simpa [texMatchingRegularQuadricLowerDepthTailPathFrecAnalyticData_of_univData,
      ...
      texMatchingRegularQuadricLowerDepthTailPathFrecAnalyticData_of_tailData,
      texMatchingTailPathFrecAsymptoticData_of_tendsto] using
        hlimits.primed_limit_eq p hp t ht

/-- Universal local-patch lower-depth limit identification from selected
closed-recursion limits.

This removes the explicit lower-depth limit-identification record when the caller has
already proved the selected full-recursion limits.  Universal path membership is
mechanical; the endpoint equalities are then recovered by the existing
closed-recursion-to-lower-depth uniqueness argument. -/
theorem
    texMatchingLocalPatchLowerDepthLimits_of_paths_univ_closedRecursion
    {L d r : Nat} (hL : 0 < L) (hr : 2 <= r)
    {θ θ' : Params (L + 1) d}
    (hstep :
      TexGenericStepClauses L d (TexGeneric L d)
  ...
  have hlimits' :
    ...
    dsimp [TexMatchingLocalPatchRegularQuadricClosedRecursionLimitObligation]
      at hlimits ⊢
    simpa [region, signRegion, T, N, S] using hlimits
  exact
    texMatchingRegularQuadricLowerDepthLimitIdentificationData_of_closedRecursionLimits
      (analytic := analytic)
      (texMatchingRegularQuadricLowerDepthEffectiveTailPathMemData_of_paths_univ
...
/-- Compile universal lower-depth convergence plus closed-recursion limits to the active
universal analytic-limit boundary. -/
def texMatchingLocalPatchRegularQuadricUnivAnalyticLimitData_of_closedRecursionLimits
    {L d r : Nat} (hL : 0 < L) (hr : 2 <= r)
    {θ θ' : Params (L + 1) d}
    (hstep :
      TexGenericStepClauses L d (TexGeneric L d)
...
/-- Compile the non-circular closed-recursion source to the universal analytic-limit
boundary. -/
def
    texMatchingLocalPatchRegularQuadricUnivAnalyticLimitData_of_closedRecursionSourceData
    {L d r : Nat} (hL : 0 < L) (hr : 2 <= r)
    {θ θ' : Params (L + 1) d}
    (hstep :
      TexGenericStepClauses L d (TexGeneric L d)
    ...
    (localRegion : TexRegionConstructionDataOfIDLDataObligation hL D)
    (P :
      TexMatchingLocalPatchRegularQuadricUnivAnalyticClosedRecursionSourceData
        hL hr hstep endpoint D localRegion) :
    TexMatchingLocalPatchRegularQuadricUnivAnalyticLimitData
      hL hr hstep endpoint D localRegion :=
  texMatchingLocalPatchRegularQuadricUnivAnalyticLimitData_of_closedRecursionLimits
    (L := L) (d := d) (r := r) hL hr hstep endpoint D localRegion
    P.lowerDepthAnalytic P.closedRecursionLimits

/-- Compile the non-circular closed-recursion source to the active wrapper. -/
def
    texMatchingLocalPatchRegularQuadricUnivAnalyticClosedRecursionLimitData_of_sourceData
    {L d r : Nat} (hL : 0 < L) (hr : 2 <= r)
    {θ θ' : Params (L + 1) d}
    (hstep :
      TexGenericStepClauses L d (TexGeneric L d)
  ...
  univAnalyticLimit :=
    texMatchingLocalPatchRegularQuadricUnivAnalyticLimitData_of_closedRecursionSourceData
      (L := L) (d := d) (r := r) hL hr hstep endpoint D localRegion P

/-- Project the reduced closed-recursion source record to the active universal
analytic-limit boundary. -/
def
...
/-- The active universal analytic-limit boundary also reconstructs the older
tail-path asymptotic boundary.

The selected tail-path `Tendsto` fields are projected from universal lower-depth
convergence plus the selected abstract-limit identifications, so callers that already
have the active boundary do not need to re-prove those asymptotics separately. -/
def texMatchingLocalPatchRegularQuadricUnivTailPathAsymptoticData_of_univAnalyticLimitData
    {L d r : Nat} (hL : 0 < L) (hr : 2 <= r)
    {θ θ' : Params (L + 1) d}
    (hstep :
      TexGenericStepClauses L d (TexGeneric L d)
...
/-- Build the current universal analytic-limit boundary from the tail-path asymptotic
boundary.

The two selected limit-equality fields are not assumed here; they are projected from
`texMatchingLocalPatchRegularQuadricLowerDepthLimits_of_univTailPathData`, which proves
them by uniqueness of limits. -/
def texMatchingLocalPatchRegularQuadricUnivAnalyticLimitData_of_univTailPathAsymptoticData
    {L d r : Nat} (hL : 0 < L) (hr : 2 <= r)
    {θ θ' : Params (L + 1) d}
    (hstep :
      TexGenericStepClauses L d (TexGeneric L d)
...
/-- Compile the active universal analytic-limit boundary back to the non-circular
closed-recursion source wrapper.

The resulting source stores the universal lower-depth analytic data directly and
derives the selected closed-recursion limits from the analytic-limit identifications;
no recursive `tailReduced` data are used. -/
def of_univAnalyticLimitData
    {L d r : Nat} (hL : 0 < L) (hr : 2 <= r)
    {θ θ' : Params (L + 1) d}
    (hstep :
      TexGenericStepClauses L d (TexGeneric L d)
...
/-- Compile the tail-path asymptotic boundary to the non-circular closed-recursion
source wrapper.

The lower-depth analytic field is copied from the tail-path boundary, while the
selected closed-recursion limits are derived through the analytic-limit compiler. -/
def of_univTailPathAsymptoticData
    {L d r : Nat} (hL : 0 < L) (hr : 2 <= r)
    {θ θ' : Params (L + 1) d}
    (hstep :
      TexGenericStepClauses L d (TexGeneric L d)
    ...
    TexMatchingLocalPatchRegularQuadricUnivAnalyticClosedRecursionSourceData
      hL hr hstep endpoint D localRegion :=
  of_univAnalyticLimitData
    (L := L) (d := d) (r := r) hL hr hstep endpoint D localRegion
    (texMatchingLocalPatchRegularQuadricUnivAnalyticLimitData_of_univTailPathAsymptoticData
      (L := L) (d := d) (r := r) hL hr hstep endpoint D localRegion P)

/-- Direct constructor for the non-circular closed-recursion source from universal
lower-depth convergence and the two selected tail-path asymptotics. -/
def of_univAnalytic_tailPath
    {L d r : Nat} (hL : 0 < L) (hr : 2 <= r)
    {θ θ' : Params (L + 1) d}
    (hstep :
      TexGenericStepClauses L d (TexGeneric L d)
    ...
    TexMatchingLocalPatchRegularQuadricUnivAnalyticClosedRecursionSourceData
      hL hr hstep endpoint D localRegion :=
  of_univTailPathAsymptoticData
    (L := L) (d := d) (r := r) hL hr hstep endpoint D localRegion
    { lowerDepthAnalytic := analytic
      unprimed_tailPath := hunprimed
      primed_tailPath := hprimed }
...
/-- Universal-path `Frec` provider from the smaller universal analytic-limit input.

The extra `D.Paths = Set.univ` assumption is used only for selected dial membership;
the analytic and closed-recursion fields come from `P`. -/
def texMatchingLocalPatchRegularQuadricFrecProviderData_of_paths_univ_univAnalyticLimitData
    {L d r : Nat} (hL : 0 < L) (hr : 2 <= r)
    {θ θ' : Params (L + 1) d}
    (hstep :
      TexGenericStepClauses L d (TexGeneric L d)
...
/-- Universal-path `Frec` provider from the tail-path asymptotic boundary.

This compiler keeps the provider surface available while avoiding selected
abstract-limit equality hypotheses at the boundary. -/
def texMatchingLocalPatchRegularQuadricFrecProviderData_of_paths_univ_univTailPathAsymptoticData
    {L d r : Nat} (hL : 0 < L) (hr : 2 <= r)
    {θ θ' : Params (L + 1) d}
    (hstep :
      TexGenericStepClauses L d (TexGeneric L d)
    ...
    TexMatchingLocalPatchRegularQuadricFrecProviderData
      hL hr hstep endpoint D localRegion :=
  texMatchingLocalPatchRegularQuadricFrecProviderData_of_paths_univ_univAnalyticLimitData
    (L := L) (d := d) (r := r) hL hr hstep endpoint D hPaths localRegion
    (texMatchingLocalPatchRegularQuadricUnivAnalyticLimitData_of_univTailPathAsymptoticData
      (L := L) (d := d) (r := r) hL hr hstep endpoint D localRegion P)
...
/-- Compile a universal analytic-limit boundary plus swept selected-dial membership to
the realized-tail source wrapper. -/
def of_univAnalyticLimitData
    {L d r : Nat} (hL : 0 < L) (hr : 2 <= r)
    {θ θ' : Params (L + 1) d}
    (hstep :
    ...
    (P :
      TexMatchingLocalPatchRegularQuadricUnivAnalyticLimitData
        hL hr hstep endpoint D localRegion) :
    TexMatchingLocalPatchRegularQuadricRealizedTailUnivAnalyticClosedRecursionSourceData
      hL hr hstep endpoint D localRegion θfull FullPaths hPaths where
  realized := realized
  lowerDepthAnalytic := P.lowerDepthAnalytic
  closedRecursionLimits :=
    texMatchingLocalPatchRegularQuadricClosedRecursionLimitObligation_of_univAnalyticLimitData
...
/-- Compile a tail-path asymptotic boundary plus swept selected-dial membership to the
realized-tail source wrapper. -/
def of_univTailPathAsymptoticData
    {L d r : Nat} (hL : 0 < L) (hr : 2 <= r)
    {θ θ' : Params (L + 1) d}
    (hstep :
...
/-- Direct constructor from universal lower-depth convergence, swept selected-dial
membership, and the two selected tail-path asymptotics. -/
def of_univAnalytic_tailPath
    {L d r : Nat} (hL : 0 < L) (hr : 2 <= r)
    {θ θ' : Params (L + 1) d}
    (hstep :
...
/-- Top-level spelling of the realized-tail source wrapper compiler to the full local
`Frec` provider. -/
noncomputable def
    texMatchingLocalPatchRegularQuadricFrecProviderData_of_realizedTailUnivAnalyticClosedRecursionSourceData
    {L d r : Nat} (hL : 0 < L) (hr : 2 <= r)
    {θ θ' : Params (L + 1) d}
    (hstep :
      TexGenericStepClauses L d (TexGeneric L d)
...
/-- Recursive swept-path constructor for the selected full local-patch Frec provider.
The path-membership field is filled from explicit realized-tail source data; the two
selected regular-quadric asymptotics remain the exact analytic matching obligations. -/
def texMatchingLocalPatchRegularQuadricFrecProviderDialData_of_realizedTailPathSet
    {L d r : Nat} (hL : 0 < L) (hr : 2 <= r)
    {θ θ' : Params (L + 1) d}
    (hstep :
      TexGenericStepClauses L d (TexGeneric L d)
...
/-- Recursive swept-path constructor for the dial-level local-patch Frec provider from
direct selected-dial membership in the swept path class.

Compared with `texMatchingLocalPatchRegularQuadricFrecProviderDialData_of_realizedTailPathSet`,
this consumes the smaller membership theorem and leaves the exact remaining analytic
limits as the two selected regular-quadric asymptotic hypotheses. -/
def texMatchingLocalPatchRegularQuadricFrecProviderDialData_of_realizedTailMemObligation
    {L d r : Nat} (hL : 0 < L) (hr : 2 <= r)
    {θ θ' : Params (L + 1) d}
    (hstep :
      TexGenericStepClauses L d (TexGeneric L d)
...
/-- Recursive swept-path constructor from direct realized-tail membership and peeled
effective-tail asymptotics. -/
def texMatchingLocalPatchRegularQuadricFrecProviderDialData_of_realizedTailMem_effectiveTail
    {L d r : Nat} (hL : 0 < L) (hr : 2 <= r)
    {θ θ' : Params (L + 1) d}
    (hstep :
      TexGenericStepClauses L d (TexGeneric L d)
...
/-- Recursive swept-path constructor from direct realized-tail membership and peeled
effective-tail asymptotics, packaged as the direct `Frec` provider. -/
def texMatchingLocalPatchRegularQuadricFrecProviderData_of_realizedTailMem_effectiveTail
    {L d r : Nat} (hL : 0 < L) (hr : 2 <= r)
    {θ θ' : Params (L + 1) d}
    (hstep :
      TexGenericStepClauses L d (TexGeneric L d)
  ...
  texMatchingLocalPatchRegularQuadricFrecProviderData_of_dialData
    (L := L) (d := d) (r := r) hL hr hstep endpoint D localRegion
    (texMatchingLocalPatchRegularQuadricFrecProviderDialData_of_realizedTailMem_effectiveTail
      (L := L) (d := d) (r := r) hL hr hstep endpoint D localRegion
      θfull FullPaths hPaths realized hunprimed hprimed)

/-- Recursive swept-path constructor from direct realized-tail membership and selected
effective-tail limit functions over the canonical product patch. -/
def texMatchingLocalPatchRegularQuadricFrecProviderData_of_realizedTailMem_selectedEffectiveTail
    {L d r : Nat} (hL : 0 < L) (hr : 2 <= r)
    {θ θ' : Params (L + 1) d}
    (hstep :
      TexGenericStepClauses L d (TexGeneric L d)
    ...
    TexMatchingLocalPatchRegularQuadricFrecProviderData
      hL hr hstep endpoint D localRegion :=
  texMatchingLocalPatchRegularQuadricFrecProviderData_of_realizedTailMem_effectiveTail
    (L := L) (d := d) (r := r) hL hr hstep endpoint D localRegion
    θfull FullPaths hPaths realized
    (texMatchingLocalPatchRegularQuadricUnprimedSaturatedEffectiveTailFrecAsymptotic_of_selectedEffectiveTailData
      (L := L) (d := d) (r := r) hL hr hstep endpoint D localRegion
...
/-- Recursive swept-path constructor from selected dial membership and selected
effective-tail limit data.

The realized-tail membership field is compiled from the smaller selected dial
membership obligation using the swept path-class identity. -/
def
    texMatchingLocalPatchRegularQuadricFrecProviderData_of_realizedTailDialMem_selectedEffectiveTail
    {L d r : Nat} (hL : 0 < L) (hr : 2 <= r)
    {θ θ' : Params (L + 1) d}
    (hstep :
      TexGenericStepClauses L d (TexGeneric L d)
    ...
    TexMatchingLocalPatchRegularQuadricFrecProviderData
      hL hr hstep endpoint D localRegion :=
  texMatchingLocalPatchRegularQuadricFrecProviderData_of_realizedTailMem_selectedEffectiveTail
    (L := L) (d := d) (r := r) hL hr hstep endpoint D localRegion
    θfull FullPaths hPaths
    (texMatchingLocalPatchRegularQuadricDialRealizedTailMemObligation_of_dialMem
      (L := L) (d := d) (r := r) hL hr hstep endpoint D localRegion
      θfull FullPaths hPaths hdial_mem)
    analytic limits

/-- Recursive swept-path constructor from selected dial membership, selected
effective-tail analytic data, and selected closed-recursion limits.

This is the compiled reduced-provider surface for the realized-tail branch: the larger
realized-tail membership and selected limit-identification fields are both rebuilt
internally from smaller obligations. -/
def
    texMatchingLocalPatchRegularQuadricFrecProviderData_of_realizedTailDialMem_selectedEffectiveTail_closedRecursionLimits
    {L d r : Nat} (hL : 0 < L) (hr : 2 <= r)
    {θ θ' : Params (L + 1) d}
    (hstep :
      TexGenericStepClauses L d (TexGeneric L d)
  ...
  texMatchingLocalPatchRegularQuadricFrecProviderData_of_realizedTailDialMem_selectedEffectiveTail
    (L := L) (d := d) (r := r) hL hr hstep endpoint D localRegion
    θfull FullPaths hPaths hdial_mem analytic
    (texMatchingLocalPatchRegularQuadricSelectedEffectiveTailLimitIdentificationData_of_closedRecursionLimits
      (L := L) (d := d) (r := r) hL hr hstep endpoint D localRegion
      analytic hlimits)

/-- Recursive swept-path constructor from direct realized-tail membership and explicit
lower-depth tail-path asymptotics. -/
def texMatchingLocalPatchRegularQuadricFrecProviderData_of_realizedTailMem_tailPath
    {L d r : Nat} (hL : 0 < L) (hr : 2 <= r)
    {θ θ' : Params (L + 1) d}
    (hstep :
      TexGenericStepClauses L d (TexGeneric L d)
...
/-- Recursive swept-path constructor from direct realized-tail membership and
dial-level asymptotics, packaged as the direct `Frec` provider. -/
def texMatchingLocalPatchRegularQuadricFrecProviderData_of_realizedTailMem_of_dialAsymptotics
    {L d r : Nat} (hL : 0 < L) (hr : 2 <= r)
    {θ θ' : Params (L + 1) d}
    (hstep :
      TexGenericStepClauses L d (TexGeneric L d)
  ...
  texMatchingLocalPatchRegularQuadricFrecProviderData_of_dialData
    (L := L) (d := d) (r := r) hL hr hstep endpoint D localRegion
    (texMatchingLocalPatchRegularQuadricFrecProviderDialData_of_realizedTailMemObligation
      (L := L) (d := d) (r := r) hL hr hstep endpoint D localRegion
      θfull FullPaths hPaths realized hunprimed hprimed)

/-- Lift a local-patch regular-quadric `Frec` provider to the `PLift` surface used by
the recursive reduced analytic packages. -/
def texMatchingLocalPatchRegularQuadricFrecProviderPLift_of_providerData
    {L d r : Nat} (hL : 0 < L) (hr : 2 <= r)
    {θ θ' : Params (L + 1) d}
    (hstep :
      TexGenericStepClauses L d (TexGeneric L d)
...
/-- Universal-path `PLift` constructor for the dial-level local-patch Frec provider. -/
def texMatchingLocalPatchRegularQuadricFrecProviderPLift_of_paths_univ_of_dialAsymptotics
    {L d r : Nat} (hL : 0 < L) (hr : 2 <= r)
    {θ θ' : Params (L + 1) d}
    (hstep :
      TexGenericStepClauses L d (TexGeneric L d)
    ...
    PLift
      (TexMatchingLocalPatchRegularQuadricFrecProviderData
        hL hr hstep endpoint D localRegion) :=
  texMatchingLocalPatchRegularQuadricFrecProviderPLift_of_providerData
    (L := L) (d := d) (r := r) hL hr hstep endpoint D localRegion
    (texMatchingLocalPatchRegularQuadricFrecProviderData_of_paths_univ_of_dialAsymptotics
      (L := L) (d := d) (r := r) hL hr hstep endpoint D hPaths localRegion
      hunprimed hprimed)

/-- Universal-path `PLift` constructor from peeled effective-tail asymptotics. -/
def texMatchingLocalPatchRegularQuadricFrecProviderPLift_of_paths_univ_of_effectiveTail
    {L d r : Nat} (hL : 0 < L) (hr : 2 <= r)
    {θ θ' : Params (L + 1) d}
    (hstep :
      TexGenericStepClauses L d (TexGeneric L d)
...
/-- Universal-path `PLift` constructor from selected effective-tail limit/tendsto data. -/
def texMatchingLocalPatchRegularQuadricFrecProviderPLift_of_paths_univ_selectedEffectiveTail
    {L d r : Nat} (hL : 0 < L) (hr : 2 <= r)
    {θ θ' : Params (L + 1) d}
    (hstep :
      TexGenericStepClauses L d (TexGeneric L d)
...
/-- Universal-path `PLift` constructor from selected effective-tail analytic data and
selected closed-recursion limits. -/
def
    texMatchingLocalPatchRegularQuadricFrecProviderPLift_of_paths_univ_selectedEffectiveTail_closedRecursionLimits
    {L d r : Nat} (hL : 0 < L) (hr : 2 <= r)
    {θ θ' : Params (L + 1) d}
    (hstep :
      TexGenericStepClauses L d (TexGeneric L d)
...
/-- Universal-path `PLift` constructor from explicit lower-depth tail-path asymptotics. -/
def texMatchingLocalPatchRegularQuadricFrecProviderPLift_of_paths_univ_of_tailPath
    {L d r : Nat} (hL : 0 < L) (hr : 2 <= r)
    {θ θ' : Params (L + 1) d}
    (hstep :
      TexGenericStepClauses L d (TexGeneric L d)
  ...
  texMatchingLocalPatchRegularQuadricFrecProviderPLift_of_providerData
    (L := L) (d := d) (r := r) hL hr hstep endpoint D localRegion
    (texMatchingLocalPatchRegularQuadricFrecProviderData_of_paths_univ_of_tailPath
      (L := L) (d := d) (r := r) hL hr hstep endpoint D hPaths localRegion
      hunprimed hprimed)

/-- Universal-path `PLift` constructor from split lower-depth analytic and limit data.
The selected path-membership fields are packaged internally from `D.Paths = Set.univ`. -/
def texMatchingLocalPatchRegularQuadricFrecProviderPLift_of_paths_univ_lowerDepthTailPath
    {L d r : Nat} (hL : 0 < L) (hr : 2 <= r)
    {θ θ' : Params (L + 1) d}
    (hstep :
      TexGenericStepClauses L d (TexGeneric L d)
...
/-- Universal-path `PLift` constructor from lower-depth analytic data and selected
tail-path asymptotics. -/
def texMatchingLocalPatchRegularQuadricFrecProviderPLift_of_paths_univ_lowerDepthTailPath_of_tailPath
    {L d r : Nat} (hL : 0 < L) (hr : 2 <= r)
    {θ θ' : Params (L + 1) d}
    (hstep :
      TexGenericStepClauses L d (TexGeneric L d)
...
/-- Universal-path `PLift` constructor from explicit universal lower-depth analytic
data and selected tail-path asymptotics. -/
def texMatchingLocalPatchRegularQuadricFrecProviderPLift_of_paths_univ_univAnalytic_tailPath
    {L d r : Nat} (hL : 0 < L) (hr : 2 <= r)
    {θ θ' : Params (L + 1) d}
    (hstep :
      TexGenericStepClauses L d (TexGeneric L d)
...
/-- Recursive swept-path `PLift` constructor from direct realized-tail membership and
dial-level asymptotics. -/
def texMatchingLocalPatchRegularQuadricFrecProviderPLift_of_realizedTailMem_of_dialAsymptotics
    {L d r : Nat} (hL : 0 < L) (hr : 2 <= r)
    {θ θ' : Params (L + 1) d}
    (hstep :
      TexGenericStepClauses L d (TexGeneric L d)
  ...
  texMatchingLocalPatchRegularQuadricFrecProviderPLift_of_providerData
    (L := L) (d := d) (r := r) hL hr hstep endpoint D localRegion
    (texMatchingLocalPatchRegularQuadricFrecProviderData_of_realizedTailMem_of_dialAsymptotics
      (L := L) (d := d) (r := r) hL hr hstep endpoint D localRegion
      θfull FullPaths hPaths realized hunprimed hprimed)

/-- Recursive swept-path `PLift` constructor from direct realized-tail membership and
peeled effective-tail asymptotics. -/
def texMatchingLocalPatchRegularQuadricFrecProviderPLift_of_realizedTailMem_effectiveTail
    {L d r : Nat} (hL : 0 < L) (hr : 2 <= r)
    {θ θ' : Params (L + 1) d}
    (hstep :
      TexGenericStepClauses L d (TexGeneric L d)
...
/-- Recursive swept-path `PLift` constructor from direct realized-tail membership and
selected effective-tail limit/tendsto data. -/
def texMatchingLocalPatchRegularQuadricFrecProviderPLift_of_realizedTailMem_selectedEffectiveTail
    {L d r : Nat} (hL : 0 < L) (hr : 2 <= r)
    {θ θ' : Params (L + 1) d}
    (hstep :
      TexGenericStepClauses L d (TexGeneric L d)
  ...
  texMatchingLocalPatchRegularQuadricFrecProviderPLift_of_providerData
    ...
    (texMatchingLocalPatchRegularQuadricFrecProviderData_of_realizedTailMem_selectedEffectiveTail
      (L := L) (d := d) (r := r) hL hr hstep endpoint D localRegion
      θfull FullPaths hPaths realized analytic limits)

/-- Recursive swept-path `PLift` constructor from selected dial membership and
selected effective-tail limit/tendsto data. -/
def
    texMatchingLocalPatchRegularQuadricFrecProviderPLift_of_realizedTailDialMem_selectedEffectiveTail
    {L d r : Nat} (hL : 0 < L) (hr : 2 <= r)
    {θ θ' : Params (L + 1) d}
    (hstep :
      TexGenericStepClauses L d (TexGeneric L d)
  ...
  texMatchingLocalPatchRegularQuadricFrecProviderPLift_of_providerData
    ...
    (texMatchingLocalPatchRegularQuadricFrecProviderData_of_realizedTailDialMem_selectedEffectiveTail
      (L := L) (d := d) (r := r) hL hr hstep endpoint D localRegion
      θfull FullPaths hPaths hdial_mem analytic limits)

/-- Recursive swept-path `PLift` constructor from selected dial membership, selected
effective-tail analytic data, and selected closed-recursion limits. -/
def
    texMatchingLocalPatchRegularQuadricFrecProviderPLift_of_realizedTailDialMem_selectedEffectiveTail_closedRecursionLimits
    {L d r : Nat} (hL : 0 < L) (hr : 2 <= r)
    {θ θ' : Params (L + 1) d}
    (hstep :
      TexGenericStepClauses L d (TexGeneric L d)
  ...
  texMatchingLocalPatchRegularQuadricFrecProviderPLift_of_providerData
    ...
    (texMatchingLocalPatchRegularQuadricFrecProviderData_of_realizedTailDialMem_selectedEffectiveTail_closedRecursionLimits
      (L := L) (d := d) (r := r) hL hr hstep endpoint D localRegion
      θfull FullPaths hPaths hdial_mem analytic hlimits)

/-- Recursive swept-path `PLift` constructor from direct realized-tail membership and
explicit lower-depth tail-path asymptotics. -/
def texMatchingLocalPatchRegularQuadricFrecProviderPLift_of_realizedTailMem_tailPath
    {L d r : Nat} (hL : 0 < L) (hr : 2 <= r)
    {θ θ' : Params (L + 1) d}
    (hstep :
      TexGenericStepClauses L d (TexGeneric L d)
  ...
  texMatchingLocalPatchRegularQuadricFrecProviderPLift_of_providerData
    (L := L) (d := d) (r := r) hL hr hstep endpoint D localRegion
    (texMatchingLocalPatchRegularQuadricFrecProviderData_of_realizedTailMem_tailPath
      (L := L) (d := d) (r := r) hL hr hstep endpoint D localRegion
      θfull FullPaths hPaths realized hunprimed hprimed)

/-- Recursive swept-path `PLift` constructor from direct realized-tail membership and
split lower-depth analytic/limit data. -/
def texMatchingLocalPatchRegularQuadricFrecProviderPLift_of_realizedTailMem_lowerDepthTailPath
    {L d r : Nat} (hL : 0 < L) (hr : 2 <= r)
    {θ θ' : Params (L + 1) d}
    (hstep :
      TexGenericStepClauses L d (TexGeneric L d)
...
/-- Recursive swept-path `PLift` constructor from direct realized-tail membership,
lower-depth analytic data, and selected tail-path asymptotics. -/
def texMatchingLocalPatchRegularQuadricFrecProviderPLift_of_realizedTailMem_lowerDepthTailPath_of_tailPath
    {L d r : Nat} (hL : 0 < L) (hr : 2 <= r)
    {θ θ' : Params (L + 1) d}
    (hstep :
      TexGenericStepClauses L d (TexGeneric L d)
  ...
  texMatchingLocalPatchRegularQuadricFrecProviderPLift_of_providerData
    (L := L) (d := d) (r := r) hL hr hstep endpoint D localRegion
    (texMatchingLocalPatchRegularQuadricFrecProviderData_of_realizedTailMem_lowerDepthTailPath_of_tailPath
      (L := L) (d := d) (r := r) hL hr hstep endpoint D localRegion
      θfull FullPaths hPaths realized analytic hunprimed hprimed)

/-- Universal-path tail-path provider constructor, composed directly to the exposed
closed-recursion limit obligation through the packed `Frec` provider surface. -/
theorem
    texMatchingLocalPatchClosedRecursionLimitObligation_of_paths_univ_tailPathProvider
    {L d r : Nat} (hL : 0 < L) (hr : 2 <= r)
    {θ θ' : Params (L + 1) d}
    (hstep :
      TexGenericStepClauses L d (TexGeneric L d)
...
/-- Realized-tail tail-path provider constructor, composed directly to the exposed
closed-recursion limit obligation through the packed `Frec` provider surface. -/
theorem
    texMatchingLocalPatchClosedRecursionLimitObligation_of_realizedTailMem_tailPathProvider
    {L d r : Nat} (hL : 0 < L) (hr : 2 <= r)
    {θ θ' : Params (L + 1) d}
    (hstep :
      TexGenericStepClauses L d (TexGeneric L d)
...
/-- Non-universal local-patch provider constructor from selected canonical dial
membership and the direct regular-quadric limit theorem.

This is the route intended for recursive arbitrary-path calls: path availability is
explicit, and no already-built `FirstLayerMatchedData θ θ'` is needed. -/
noncomputable def texMatchingRegionProviderFromLocalPatchData_of_regularQuadricLimit
    {L d r : Nat} (hL : 0 < L) (hr : 2 <= r)
    {θ θ' : Params (L + 1) d}
    (hstep :
      TexGenericStepClauses L d (TexGeneric L d)
...
/-- Local-patch provider constructor from explicit dial membership and direct Frec
asymptotics, with no matched-tail provider and no `FirstLayerMatchedData θ θ'`. -/
noncomputable def texMatchingRegionProviderFromLocalPatchData_of_regularQuadricFrecProvider
    {L d r : Nat} (hL : 0 < L) (hr : 2 <= r)
    {θ θ' : Params (L + 1) d}
    (hstep :
      TexGenericStepClauses L d (TexGeneric L d)
    ...
    TexMatchingRegionProviderFromLocalPatchData
      hL hr hstep endpoint D localRegion :=
  texMatchingRegionProviderFromLocalPatchData_of_regularQuadricLimit
    (L := L) (d := d) (r := r) hL hr hstep endpoint D localRegion
    provider.dial_mem
    (texMatchingLocalPatchRegularQuadricLimitObligation_of_closedRecursionLimits
      (L := L) (d := d) (r := r) hL hr hstep endpoint D localRegion
...
/-- Universal-path local-patch region provider from split lower-depth analytic and
limit data.  Dial membership and selected effective-tail path membership are derived
internally from `D.Paths = Set.univ`. -/
noncomputable def
    texMatchingRegionProviderFromLocalPatchData_of_paths_univ_lowerDepthTailPath
    {L d r : Nat} (hL : 0 < L) (hr : 2 <= r)
    {θ θ' : Params (L + 1) d}
    (hstep :
      TexGenericStepClauses L d (TexGeneric L d)
    ...
    TexMatchingRegionProviderFromLocalPatchData
      hL hr hstep endpoint D localRegion :=
  texMatchingRegionProviderFromLocalPatchData_of_regularQuadricFrecProvider
    (L := L) (d := d) (r := r) hL hr hstep endpoint D localRegion
    (texMatchingLocalPatchRegularQuadricFrecProviderData_of_paths_univ_lowerDepthTailPath
      (L := L) (d := d) (r := r) hL hr hstep endpoint D hPaths localRegion
      analytic limits)

/-- The pointwise regular-quadric dial theorem discharges the canonical local-patch
universal-path limit obligation. -/
theorem texMatchingLocalPatchUnivLimitObligation_of_regularQuadricLimit
    {L d r : Nat} (hL : 0 < L) (hr : 2 <= r)
    {θ θ' : Params (L + 1) d}
    (hstep :
      TexGenericStepClauses L d (TexGeneric L d)
    ...
    (hlimit :
      TexMatchingLocalPatchRegularQuadricLimitObligation
        hL hr hstep endpoint D localRegion) :
    TexMatchingLocalPatchUnivLimitObligation
      hL hr hstep endpoint D localRegion := by
  intro p hp t ht T0 hobs
  exact hlimit p hp t ht T0 (by
    simpa [texMatchingDialPathData_of_product] using hobs)

/-- Universal-path constructor for the reduced local-patch provider.  It fills the
canonical `DialPathData` choices and their membership in `D.Paths`; the remaining
analytic input is the pointwise limit theorem along `DialPathData.ofRegularQuadric`. -/
noncomputable def texMatchingRegionProviderFromLocalPatchData_of_univPaths
    {L d r : Nat} (hL : 0 < L) (hr : 2 <= r)
    {θ θ' : Params (L + 1) d}
    (hstep :
      TexGenericStepClauses L d (TexGeneric L d)
  ...
  let hlimitExact :
      TexMatchingLocalPatchUnivLimitObligation
        hL hr hstep endpoint D localRegion :=
    texMatchingLocalPatchUnivLimitObligation_of_regularQuadricLimit
      (L := L) (d := d) (r := r) hL hr hstep endpoint D localRegion hlimit
  exact
    { bridge :=
        texMatchingDialPathLimitBridgeData_of_univPaths
          (L := L) (d := d) (r := r) D hPaths signRegion T N S hlimitExact }
...
/-- Local-patch provider constructor for arbitrary IDL path classes.  The caller
supplies membership of the selected regular-quadric dials in `D.Paths` and the direct
regular-quadric limit theorem. -/
noncomputable def texMatchingRegionProviderData_of_localPatch_regularQuadricLimit
    {L d r : Nat} (hL : 0 < L) (hr : 2 <= r)
    {θ θ' : Params (L + 1) d}
    (hstep :
      TexGenericStepClauses L d (TexGeneric L d)
    ...
    (hlimit :
      TexMatchingLocalPatchRegularQuadricLimitObligation
        hL hr hstep endpoint D localRegion) :
    TexMatchingRegionProviderData hr hstep endpoint D :=
...
/-- Local-patch provider constructor in the universal-path case. -/
noncomputable def texMatchingRegionProviderData_of_localPatch_univPaths
    {L d r : Nat} (hL : 0 < L) (hr : 2 <= r)
    {θ θ' : Params (L + 1) d}
    (hstep :
      TexGenericStepClauses L d (TexGeneric L d)
  ...
  texMatchingRegionProviderData_of_localPatch
    (L := L) (d := d) (r := r) hL hr hstep endpoint D localRegion
    (texMatchingRegionProviderFromLocalPatchData_of_univPaths
      (L := L) (d := d) (r := r) hL hr hstep endpoint D hPaths
      localRegion hlimit)

/-- Local-patch region provider constructor in the universal-path case from split
lower-depth analytic and selected limit-identification data. -/
noncomputable def texMatchingRegionProviderData_of_localPatch_univPaths_lowerDepthTailPath
    {L d r : Nat} (hL : 0 < L) (hr : 2 <= r)
    {θ θ' : Params (L + 1) d}
    (hstep :
      TexGenericStepClauses L d (TexGeneric L d)
...
/-- Compile the non-universal local-patch regular-quadric route to the analytic package
consumed by the first-layer matching endpoint. -/
noncomputable def texFirstLayerMatchingAnalyticData_of_localPatch_regularQuadricLimit
    {L d r : Nat} (hL : 0 < L) (hr : 2 <= r)
    {θ θ' : Params (L + 1) d}
    (hstep :
      TexGenericStepClauses L d (TexGeneric L d)
  ...
  texFirstLayerMatchingAnalyticData_of_provider
    (L := L) (d := d) (r := r) hr hstep endpoint D
    (texMatchingRegionProviderData_of_localPatch_regularQuadricLimit
      (L := L) (d := d) (r := r) hL hr hstep endpoint D localRegion
      hdial_mem hlimit)

/-- Compile the local-patch universal-path provider to the analytic package consumed by
the first-layer matching endpoint. -/
noncomputable def texFirstLayerMatchingAnalyticData_of_localPatch_univPaths
    {L d r : Nat} (hL : 0 < L) (hr : 2 <= r)
    {θ θ' : Params (L + 1) d}
    (hstep :
      TexGenericStepClauses L d (TexGeneric L d)
  ...
  texFirstLayerMatchingAnalyticData_of_provider
    (L := L) (d := d) (r := r) hr hstep endpoint D
    (texMatchingRegionProviderData_of_localPatch_univPaths
      (L := L) (d := d) (r := r) hL hr hstep endpoint D hPaths
      localRegion hlimit)

/-- Compile the universal local-patch lower-depth provider to the analytic package
consumed by the first-layer matching endpoint. -/
noncomputable def texFirstLayerMatchingAnalyticData_of_localPatch_univPaths_lowerDepthTailPath
    {L d r : Nat} (hL : 0 < L) (hr : 2 <= r)
    {θ θ' : Params (L + 1) d}
    (hstep :
      TexGenericStepClauses L d (TexGeneric L d)
  ...
  texFirstLayerMatchingAnalyticData_of_provider
    (L := L) (d := d) (r := r) hr hstep endpoint D
    (texMatchingRegionProviderData_of_localPatch_univPaths_lowerDepthTailPath
      (L := L) (d := d) (r := r) hL hr hstep endpoint D hPaths
      localRegion analytic limits)
...
/-- Top-level constructor for the matching support package when all full-depth paths are
available.  The caller supplies the sign region, product neighborhood, actual dial
paths, and the analytic limit theorem; membership in `D.Paths` is filled from
`D.Paths = Set.univ`. -/
noncomputable def texFirstLayerMatchingAnalyticData_of_univDialPathLimit
    {L d r : Nat} (hr : 2 <= r)
    {θ θ' : Params (L + 1) d}
    (hstep :
      TexGenericStepClauses L d (TexGeneric L d)
...
/-- First-layer matching endpoint from a cascade-builder trichotomy, universal IDL
paths, and canonical actual gates. -/
theorem firstLayerMatched_of_texGenericStep_of_IDLData_cascadeBuilder_paths_univ_lowerDepthSelectedTail
    {L d r : Nat} (hr : 2 <= r)
    {θ θ' θB : Params (L + 1) d}
    (hstep :
      TexGenericStepClauses L d (TexGeneric L d)
...
/-- First-layer matching endpoint from an induction-provider trichotomy, universal IDL
paths, and canonical actual gates. -/
theorem firstLayerMatched_of_texGenericStep_of_IDLData_inductionProvider_paths_univ_lowerDepthSelectedTail
    {L d r : Nat} (hr : 2 <= r)
    {θ θ' θB : Params (L + 1) d}
    (hstep :
      TexGenericStepClauses L d (TexGeneric L d)
...
/-- First-layer matching endpoint from the local-region patch in the universal-path
case.  The dial paths are the canonical regular-quadric dials through the selected
product patch; the remaining input is the pointwise regular-quadric dial limit
theorem. -/
theorem firstLayerMatched_of_texGenericStep_of_IDLData_localPatch_univPaths
    {L d r : Nat} (hL : 0 < L) (hr : 2 <= r)
    {θ θ' : Params (L + 1) d}
    (hstep :
      TexGenericStepClauses L d (TexGeneric L d)
  ...
  exact
    firstLayerMatched_of_texGenericStep_of_IDLData
      (L := L) (d := d) (r := r) hr hstep endpoint D
      (texFirstLayerMatchingAnalyticData_of_localPatch_univPaths
        (L := L) (d := d) (r := r) hL hr hstep endpoint D hPaths
        localRegion hlimit)

/-- First-layer matching endpoint from the local-region patch in the universal-path
case, using lower-depth convergence over `Set.univ` path classes and selected
limit-vector identifications. -/
theorem firstLayerMatched_of_texGenericStep_of_IDLData_localPatch_univPaths_lowerDepthTailPath
    {L d r : Nat} (hL : 0 < L) (hr : 2 <= r)
    {θ θ' : Params (L + 1) d}
    (hstep :
      TexGenericStepClauses L d (TexGeneric L d)
  ...
  exact
    firstLayerMatched_of_texGenericStep_of_IDLData
      (L := L) (d := d) (r := r) hr hstep endpoint D
      (texFirstLayerMatchingAnalyticData_of_localPatch_univPaths_lowerDepthTailPath
        (L := L) (d := d) (r := r) hL hr hstep endpoint D hPaths
        localRegion analytic limits)

/-- Top-level specialization of the new first-layer matching endpoint when
`D.Paths = Set.univ`. -/
theorem firstLayerMatched_of_texGenericStep_of_univDialPathLimit
    {L d r : Nat} (hr : 2 <= r)
    {θ θ' : Params (L + 1) d}
    (hstep :
      TexGenericStepClauses L d (TexGeneric L d)
  ...
  exact
    firstLayerMatched_of_texGenericStep_of_IDLData
      (L := L) (d := d) (r := r) hr hstep endpoint D
      (texFirstLayerMatchingAnalyticData_of_univDialPathLimit
        (L := L) (d := d) (r := r) hr hstep endpoint D hPaths signRegion N
        dial dial_base dial_target hlimit)

/-- Proposition `trichotomy` plus the coefficient-comparison endpoint from the TeX Step 2
proof.  This is the missing analytic bridge that ties the selected gate asymptotics back
to the actual layer streams; the downstream matching code is only formal packaging. -/
noncomputable def texMatchingData_of_signRegion
    {L d r : Nat} (hr : 2 <= r)
    {θ θ' : Params (L + 1) d}
    (hstep :
      TexGenericStepClauses L d (TexGeneric L d)
    ...
    (signRegion :
      SignRegionData (L := L + 1) (d := d) θ' D.O
        (Params.headAttention θ') (Real.log (r : ℝ)))
    (C : TexMatchingConstructionData θ θ' (Real.log (r : ℝ)) signRegion.U) :
    Σ unprimed : GateAlongBase d, Σ primed : GateAlongBase d,
      Σ Ustar : Set (ProbePair d × ℝ),
        TexMatchingData θ θ' (Real.log (r : ℝ)) signRegion.U Ustar
          unprimed primed := by
  let T := C.T
  let N := C.N
  let S := C.S
  let coefficientComparison : MatchingCoefficientComparisonData θ θ' :=
    { D := S.D
      skipProduct_eq :=
        texMatching_skipProduct_eq_of_saturatedContribution
          (L := L) (d := d) (r := r) hr hstep endpoint D signRegion T N S
      ...
      K_maps_headSkip :=
        texMatching_K_maps_headSkip_of_saturatedContribution
          ...
          C.limitAgreement }
  exact
    ⟨T.unprimed, T.primed, T.Ustar,
      { trichotomy := T.trichotomy
        coefficientComparison := coefficientComparison }⟩

/-- Full TeX Step 2 matching package over the selected sign region. -/
noncomputable def matchingData_of_signRegion
    {L d r : Nat} (hr : 2 <= r)
    {θ θ' : Params (L + 1) d}
    (hstep :
      TexGenericStepClauses L d (TexGeneric L d)
    ...
    Σ unprimed : GateAlongBase d, Σ primed : GateAlongBase d,
      Σ Ustar : Set (ProbePair d × ℝ),
        TexMatchingData θ θ' (Real.log (r : ℝ)) signRegion.U Ustar
          unprimed primed :=
  texMatchingData_of_signRegion hr hstep endpoint D signRegion C

/-- Raw trichotomy projection from the stronger TeX matching package.  This keeps the
constructor-level trichotomy interface available without pretending it contains the
coefficient comparison by itself. -/
noncomputable def trichotomyData_of_signRegion
    {L d r : Nat} (hr : 2 <= r)
    {θ θ' : Params (L + 1) d}
    (hstep :
      TexGenericStepClauses L d (TexGeneric L d)
    ...
    Σ unprimed : GateAlongBase d, Σ primed : GateAlongBase d,
      Σ Ustar : Set (ProbePair d × ℝ),
        TrichotomyData (L := L + 1) (d := d) (Real.log (r : ℝ)) signRegion.U Ustar
          unprimed primed := by
  rcases matchingData_of_signRegion (L := L) (d := d) (r := r) hr hstep endpoint
      D signRegion C with
    ⟨unprimed, primed, Ustar, matchingStep⟩
  exact ⟨unprimed, primed, Ustar, matchingStep.trichotomy⟩

/-- TeX Proposition `matching`, isolated at the affine coefficient-comparison stage
before the formal `K = I` package. -/
noncomputable def matchingCoefficientComparisonData_of_matchingStep
    {L d r : Nat} (hr : 2 <= r)
    {θ θ' : Params (L + 1) d}
    (hstep :
      TexGenericStepClauses L d (TexGeneric L d)
    ...
    (signRegion :
      SignRegionData (L := L + 1) (d := d) θ' D.O
        (Params.headAttention θ') (Real.log (r : ℝ)))
    {unprimed primed : GateAlongBase d} {Ustar : Set (ProbePair d × ℝ)}
    (matchingStep :
      TexMatchingData θ θ' (Real.log (r : ℝ)) signRegion.U Ustar
        unprimed primed) :
    MatchingCoefficientComparisonData θ θ' := by
  have _ := hr
  have _ := hstep
  have _ := endpoint
  have _ := D
  have _ := signRegion
  exact matchingStep.coefficientComparison

/-- Proposition `matching` up to the coefficient-comparison endpoint immediately before
the abstract `K = I` cancellation. -/
noncomputable def matchingLimitData_of_matchingStep
    {L d r : Nat} (hr : 2 <= r)
    {θ θ' : Params (L + 1) d}
    (hstep :
      TexGenericStepClauses L d (TexGeneric L d)
    ...
    MatchingLimitData θ θ' := by
  exact
    (matchingCoefficientComparisonData_of_matchingStep
      (L := L) (d := d) (r := r) hr hstep endpoint D signRegion
        matchingStep).toMatchingLimitData

/-- The already-formalized `K = I` extraction turns matching-limit data into the
first-layer matching package used by the rest of Step 2. -/
def firstLayerMatchingData_of_matchingLimitData
    {L d : Nat} {θ θ' : Params (L + 1) d}
    (limitData : MatchingLimitData θ θ') :
    FirstLayerMatchingData θ θ' :=
  limitData.toFirstLayerMatchingData

/-- The coefficient/limit endpoint is the minimal Step 2 output needed to identify the
first value matrix once Step 1 has identified first attention. -/
theorem firstLayerMatched_of_matchingLimitData
    {L d : Nat} {θ θ' : Params (L + 1) d}
    (endpoint :
      FirstLayerEndpointData
        (Params.headAttention θ) (Params.headAttention θ')
    ...
    FirstLayerMatchedData θ θ' := by
  let matching :=
    firstLayerMatchingData_of_matchingLimitData (θ := θ) (θ' := θ') limitData
  refine FirstLayerMatchedData.ofEndpointAndHeadValueEq endpoint ?_
  simpa [Params.headValue, Params.headLayer] using matching.headValue_eq

/-- Variant of `firstLayerMatched_of_matchingLimitData` phrased in the local
coefficient-comparison package produced by TeX Proposition `matching`. -/
theorem firstLayerMatched_of_coefficientComparisonData
    {L d : Nat} {θ θ' : Params (L + 1) d}
    (endpoint :
      FirstLayerEndpointData
        (Params.headAttention θ) (Params.headAttention θ')
        (Params.headValue θ))
    (C : MatchingCoefficientComparisonData θ θ') :
    FirstLayerMatchedData θ θ' :=
  firstLayerMatched_of_matchingLimitData endpoint C.toMatchingLimitData

/-- If the coefficient-comparison output of TeX matching is already built, the
selected-dial matched-tail provider discharges the local-patch closed-recursion limits.

The `TexMatchingData` input supplies `FirstLayerMatchedData` through its
coefficient-comparison field; the provider supplies selected dial membership, the
primed matched-tail `Frec` asymptotic, and the saturated/telescoped endpoint equality. -/
theorem
    texMatchingLocalPatchClosedRecursionLimits_of_matchingData_matchedTailProvider
    {L d r : Nat} (hL : 0 < L) (hr : 2 <= r)
    {θ θ' : Params (L + 1) d}
    {signU Ustar : Set (ProbePair d × ℝ)}
    {unprimed primed : GateAlongBase d}
    ...
    (localRegion : TexRegionConstructionDataOfIDLDataObligation hL D)
    (matchingStep :
      TexMatchingData θ θ' (Real.log (r : ℝ)) signU Ustar
        unprimed primed)
    (provider :
      TexMatchingLocalPatchMatchedTailPathFrecProviderData
        hL hr hstep endpoint D localRegion) :
    TexMatchingLocalPatchRegularQuadricClosedRecursionLimitObligation
      hL hr hstep endpoint D localRegion :=
  texMatchingLocalPatchRegularQuadricClosedRecursionLimitObligation_of_matchedTailPathProvider
    (L := L) (d := d) (r := r) hL hr hstep endpoint D localRegion
    (firstLayerMatched_of_coefficientComparisonData endpoint
      matchingStep.coefficientComparison)
    provider
...
/-- Legacy explicit construction package compiled to the newer `MatchingLimitData`
endpoint.  This wires the current region/trichotomy/matching interfaces as far as they
go without pretending that `IDLData` alone already supplies the missing analytic
construction facts. -/
noncomputable def matchingLimitData_of_IDLData_of_texConstruction
    {L d r : Nat} (hr : 2 <= r)
    {θ θ' : Params (L + 1) d}
    (hstep :
      TexGenericStepClauses L d (TexGeneric L d)
    ...
    (endpoint :
      FirstLayerEndpointData
        ...
        (Params.headValue θ))
    (D : IDLData (L + 1) d r θ θ')
    (C : TexFirstLayerMatchingConstructionData θ θ' D) :
    MatchingLimitData θ θ' := by
  let signRegion :=
    signRegionData_of_texGenericStep (L := L) (d := d) (r := r) hr hstep endpoint
      D C.region
  rcases matchingData_of_signRegion (L := L) (d := d) (r := r) hr hstep endpoint
      D signRegion C.matching with
    ⟨unprimed, primed, Ustar, matchingStep⟩
  exact
    matchingLimitData_of_matchingStep (L := L) (d := d) (r := r) hr hstep endpoint
      D signRegion matchingStep

/-- The saturation/matching theorem: from the Step 1 endpoint and the TeX generic step
clauses, identify the first value matrix as well. -/
theorem firstLayerMatched_of_texGenericStep
    {L d r : Nat} (hr : 2 <= r)
    {θ θ' : Params (L + 1) d}
    (hstep :
      TexGenericStepClauses L d (TexGeneric L d)
    ...
    FirstLayerMatchedData θ θ' := by
  let limitData :=
    matchingLimitData_of_IDLData_of_texConstruction (L := L) (d := d) (r := r)
      hr hstep endpoint D C
  exact firstLayerMatched_of_matchingLimitData endpoint limitData
...
/-- An exponentially-close gate converges to its saturation value: the existing
`EventuallyExpClose` data (the `TrichotomyData` saturation shape) yields topological
convergence. -/
theorem eventuallyExpClose_tendsto {f : ℝ -> ℝ} {a : ℝ}
    (h : EventuallyExpClose f a) :
    Filter.Tendsto f Filter.atTop (nhds a) := by
  have hsub : Filter.Tendsto (fun τ : ℝ => f τ - a) Filter.atTop (nhds 0) := by
    have hbound :
        ∀ᶠ τ : ℝ in Filter.atTop, ‖f τ - a‖ ≤ h.coeff * Real.exp (-h.rate * τ) := by
      filter_upwards [Filter.eventually_ge_atTop h.start] with τ hτ
      simpa [Real.norm_eq_abs] using h.bound τ hτ
    have hzero :
        Filter.Tendsto (fun τ : ℝ => h.coeff * Real.exp (-h.rate * τ))
          Filter.atTop (nhds 0) := by
      have hmul : Filter.Tendsto (fun τ : ℝ => h.rate * τ) Filter.atTop Filter.atTop :=
        Filter.Tendsto.const_mul_atTop h.rate_pos Filter.tendsto_id
      have hexp :
          Filter.Tendsto (fun τ : ℝ => Real.exp (-h.rate * τ)) Filter.atTop (nhds 0) := by
...
/-- Componentwise exponential closeness gives convergence of the vector function to its
limit vector in the product topology on `Fin d -> ℝ`. -/
theorem tendsto_of_eventuallyExpClose_pi {d : Nat}
    {f : ℝ -> Fin d -> ℝ} {a : Fin d -> ℝ}
    (h : ∀ i : Fin d, EventuallyExpClose (fun τ => f τ i) (a i)) :
    Filter.Tendsto f Filter.atTop (nhds a) := by
  rw [tendsto_pi_nhds]
  intro i
  exact (h i).tendsto

/-- A `Frec`-valued path converges to a chosen limit vector once each coordinate of the
recursion is exponentially close to the corresponding coordinate of the limit.

This is the bridge from the upstream gate-saturation data (`EventuallyExpClose`, the
output shape of `TrichotomyData`) to the `Tendsto` shape required by the universal
lower-depth analytic record. -/
theorem tendsto_frec_of_eventuallyExpClose {L d : Nat} (r : Nat)
    (θ : Params L d) (P : ProbePath d) (limit : Fin d -> ℝ)
    (h : ∀ i : Fin d,
      EventuallyExpClose (fun τ => Frec r θ (P τ).1 (P τ).2 τ i) (limit i)) :
    Filter.Tendsto (fun τ : ℝ => Frec r θ (P τ).1 (P τ).2 τ)
      Filter.atTop (nhds limit) :=
  tendsto_of_eventuallyExpClose_pi h
...
/-- Compile the precise componentwise-saturation obligation to the universal
lower-depth analytic record consumed by the matching boundary. -/
def texMatchingRegularQuadricLowerDepthUnivTailPathFrecAnalyticData_of_saturationObligation
    {L d r : Nat} {θtail θtail' : Params L d}
    (H : TexMatchingLowerDepthUnivFrecSaturationObligation (r := r) θtail θtail') :
    TexMatchingRegularQuadricLowerDepthUnivTailPathFrecAnalyticData
      (r := r) θtail θtail' where
  unprimedLimit := H.unprimedLimit
  primedLimit := H.primedLimit
  unprimed_tendsto := fun P =>
    tendsto_frec_of_eventuallyExpClose r θtail P (H.unprimedLimit P) (H.unprimed_expClose P)
  primed_tendsto := fun P =>
    tendsto_frec_of_eventuallyExpClose r θtail' P (H.primedLimit P) (H.primed_expClose P)

/-- End-to-end: from the precise componentwise-saturation obligation plus the two
selected dial-limit identifications, build the active universal analytic-limit boundary
consumed by the matching `Frec` provider.

The saturation obligation supplies the genuine universal lower-depth convergence; the
two equalities pin the chosen limit functions to the saturated/telescoped vectors on the
selected dials.  No recursive `tailReduced` data are used. -/
def texMatchingLocalPatchRegularQuadricUnivAnalyticLimitData_of_saturationObligation
    {L d r : Nat} (hL : 0 < L) (hr : 2 <= r)
    {θ θ' : Params (L + 1) d}
    (hstep :
      TexGenericStepClauses L d (TexGeneric L d)
    ...
    (D : IDLData (L + 1) d r θ θ')
    (localRegion : TexRegionConstructionDataOfIDLDataObligation hL D)
    (H : TexMatchingLowerDepthUnivFrecSaturationObligation
          (r := r) (Params.tail θ) (Params.tail θ'))
    (limits :
      let region :=
        texRegionConstructionData_of_IDLData_of_localPatch
      ...
      TexMatchingRegularQuadricLowerDepthLimitIdentificationData
        ...
        (texMatchingRegularQuadricLowerDepthTailPathFrecAnalyticData_of_univData
          (r := r)
          (texMatchingRegularQuadricLowerDepthUnivTailPathFrecAnalyticData_of_saturationObligation
            (r := r) H))) :
    TexMatchingLocalPatchRegularQuadricUnivAnalyticLimitData
      hL hr hstep endpoint D localRegion :=
  texMatchingLocalPatchRegularQuadricUnivAnalyticLimitData_of_lowerDepthLimits
